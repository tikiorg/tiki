...
if (isset($_REQUEST['action'])) {
	...
	if ($_REQUEST['action']=='book') {
		if ($stackId==0) {
			// new entry
			$result=$accountinglib->stackBook($bookId, $_REQUEST['stackDate'], $_REQUEST['stackDescription'],
			...
			// modified old entry
			$result=$accountinglib->stackUpdate($bookId, $stackId, $_REQUEST['stackDate'], $_REQUEST['stackDescription'],
					$_REQUEST['debitAccount'], $_REQUEST['creditAccount'],
					$_REQUEST['debitAmount'], $_REQUEST['creditAmount'],
					$_REQUEST['debitText'],$_REQUEST['creditText']);
		}
...
if (is_array($result)) {
	...
	if ($stackId!=0) {
	    ...
	    $debitAmount=array();
	    $debitText=array();
	    for($i=0;$i<sizeof($stackEntry['debit']);$i++) {
	    ...
	    $creditAmount=array();
	    $creditText=array();
	    for($i=0;$i<sizeof($stackEntry['credit']);$i++) {
