...
class SearchLib extends TikiLib
{
	...
	function refresh_lru_wordlist($syllable) {
		...
		$query = 'select `searchword`, sum(`count`) as `cnt`'
						. ' from `tiki_searchindex`'
						. ' where `searchword` like ? group by `searchword`'
						. ' order by `cnt` desc'
						;
		
		$result = $this->query($query, array('%' . $syllable . '%'), $prefs['search_max_syllwords']);
		// search_max_syllwords: how many different searchwords that contain the syllable are taken into account?. Sortet by number of occurences.
		...
		$now = time();
		$this->query('insert into `tiki_searchsyllable`(`syllable`, `lastUsed`, `lastUpdated`) values (?, ?, ?)',	array($syllable, (int) $now, (int) $now));
	...
	function find_part_all($words, $offset, $maxRecords) {
		...
		global $prefs, $tiki_p_view, $tiki_p_view_directory, $tiki_p_view_image_gallery, $tiki_p_view_file_gallery;
		global $tiki_p_read_article, $tiki_p_forum_read, $tiki_p_read_blog, $tiki_p_view_faqs, $tiki_p_view_trackers, $tiki_p_download_files;
	...
	function find_exact_all($words, $offset, $maxRecords) {
		global $prefs, $tiki_p_view, $tiki_p_view_directory, $tiki_p_view_image_gallery;
		global $tiki_p_view_file_gallery, $tiki_p_read_article, $tiki_p_forum_read, $tiki_p_read_blog;
		global $tiki_p_view_faqs, $tiki_p_view_trackers, $tiki_p_download_files;
	...
	function find_exact_trackers($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_trackers'] == 'y' && count($words) >0 ) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, t.`description`, t.`lastModif`, t.`name`'
								. ' from `tiki_searchindex` s, `tiki_trackers` t'
								. ' where'
			...
			if ($cant1 < $offset+$maxRecords) {
				...
				$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, t.`lastModif`, t.`trackerId`, t.`status`'
									. ' from `tiki_searchindex` s, `tiki_tracker_items` t'
									.	' where `searchword` in (' . implode(',', array_fill(0, count($words), '?')) . ')'
											.	' and s.`location`=\'trackeritem\''
											. " and s.`page` like concat(t.`itemId`, '#%')"
											;
				$result = $this->query($query, $words, $maxRecords, $offset);
				$cant2 = 0;
				while ($res = $result->fetchRow()) {
	...
	function find_exact_imggals($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_galleries'] == 'y' && count($words) >0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, g.`description`, g.`hits`, g.`lastModif`, g.`name`'
	...
	function find_exact_img($words,$offset, $maxRecords) {
		...
		if ($prefs['feature_galleries'] == 'y' && count($words) > 0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, g.`description`, g.`hits`, g.`created`, g.`name`, g.`galleryId`'
	...
	function find_exact_blogs($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_blogs'] == 'y' && count($words) > 0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, b.`description`, b.`hits`, b.`lastModif`, b.`title`'
	...
	function find_exact_blog_posts($words, $offset, $maxRecords) {
		global $prefs, $user;
		if ($prefs['feature_blogs'] == 'y' && count($words) >0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, bp.`data`, b.`hits`, b.`title` as `btitle`, bp.`created`, b.`title`, b.`blogId`'
	...
	function find_exact_articles($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_articles'] == 'y' && count($words) > 0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, a.`heading`, a.`nbreads`, a.`publishDate`, a.`title`, a.`topicId`'
	...
	function find_exact_directory($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_directory'] == 'y' && count($words) > 0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, d.`description`, d.`hits`, d.`name`'
	...
	function &find_exact_directory_sites($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_directory'] == 'y' && count($words) > 0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, d.`description`, d.`hits`, d.`name`, d.`lastModif`, cs.`categId`'
	...
	function find_exact_faqs($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_faqs'] == 'y' && count($words) > 0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, f.`description`, f.`hits`, f.`created`, f.`title`'
	...
	function find_exact_faqquestions($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_faqs']== 'y' && count($words) > 0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, f.`question`, faq.`hits`, faq.`created`, faq.`title`, f.`answer`, f.`faqId`'
	...
	function find_exact_forums($words, $offset, $maxRecords, $filter='') {
		...
		if ($prefs['feature_forums']== 'y' && count($words) > 0) {
			...
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, f.`description`, f.`hits`, f.`lastPost`, f.`name`'
	...
	function find_exact_forumcomments($words, $offset, $maxRecords, $filter) {
		...
		if ($prefs['feature_forums'] == 'y' && count($words) > 0) {
			...
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, f.`data`, f.`hits`, f.`commentDate`, f.`object`, f.`title`, fo.`name`'
	...
	function find_exact_files($words, $offset, $maxRecords) {
		...
		if ($prefs['feature_file_galleries'] == 'y' && count($words) > 0) {
			$query = 'select distinct s.`page`, s.`location`, s.`last_update`, s.`count`, f.`search_data`, f.`data`, f.`lastModif`, f.`filename`, f.`hits`, f.`description`, f.`name`, g.`name` as `galName`'
						. ' from `tiki_searchindex` s, `tiki_files` f, `tiki_file_galleries` g'
						. ' where `searchword` in (' . implode(',', array_fill(0, count($words), '?')) .')'
							. ' and s.`location`=\'file\''
