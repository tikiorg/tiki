...
function displayPicker( closeTo, list, area_id, isSheet, styleType ) {
	...
	pickerDiv = $('<div class="toolbars-picker"/>')
	...
	for( var i in window.pickerData[list] ) {
		...
		pickerDiv.append(prepareLink( i, chr ));
	}
				...
				if (field) {
					...
					if (! field.attr('disabled')) {
						map.events.register('click', map, function (e) {
							...
							field.val(lonlat.lat + ',' + lonlat.lon + ',' + map.getZoom()).change();
					...
					if (matching) {
						var lat = parseFloat(matching[1]);
						var lon = parseFloat(matching[3]);
