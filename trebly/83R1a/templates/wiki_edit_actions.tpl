{* $Id: wiki_edit_actions.tpl 35443 2011-07-17 08:55:18Z changi67 $ *}
...
{if $page|lower neq 'sandbox' or $tiki_p_admin eq 'y'}
	{if ! isset($page_badchars_display) or $prefs.wiki_badchar_prevent neq 'y'}
		...
		{else}
			...
			{/if}
			<input type="submit" class="wikiaction" title="{tr}Save the page.{/tr}" name="save" value="{tr}Save{/tr}" onclick="needToConfirm=false;" />
		{/if}
	{/if}
