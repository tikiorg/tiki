	...
	if (document.getElementById(id).style.display == "none") {
		...
		if (zeromargin == 'left') {
			...
			if (!document.getElementById(maincol).style.marginLeft) {
				document.getElementById(maincol).style.marginLeft = $jq("#"+id).width() + "px";
			}
			...
			if (!document.getElementById(maincol).style.marginRight) {
				document.getElementById(maincol).style.marginRight = $jq("#"+id).width() + "px";
			}
			setSessionVar(showit,'y');
		}
	} else {
...
function chgArtType() {
	...
	if (typeof articleCustomAttributes != 'undefined') {
		var propertyList = propertyList.concat(articleCustomAttributes);
	}
...
function setSelectionRange(textarea, selectionStart, selectionEnd) {
	if (typeof textarea.setSelectionRange != 'undefined') {
		...
		range.collapse();
		if (selectionEnd > 1) {
			range.moveEnd('character', selectionEnd - 2);
		}
...
function getTASelection( textarea ) {
	if (typeof $jq(textarea).attr("selectionStartSaved") === 'string' && $jq(textarea).attr("selectionStartSaved")) {	// forgetful firefox
		return textarea.value.substring($jq(textarea).attr("selectionStartSaved"), $jq(textarea).attr("selectionEndSaved"));
...
function saveTASelection( elem ) {
	...
	if (typeof elem.selectionStart != 'undefined') {
        $jq(elem).attr("selectionStartSaved", elem.selectionStart);
        $jq(elem).attr("selectionEndSaved", elem.selectionEnd);
	}
}
...
function restoreTASelection( elem ) {
	...
	if ($jq(elem).attr("selectionStartSaved")) {
		if (elem.selectionStart != $jq(elem).attr("selectionStartSaved")) {
			elem.selectionStart = $jq(elem).attr("selectionStartSaved");
			elem.selectionEnd = $jq(elem).attr("selectionEndSaved");
		}
        $jq(elem).removeAttr("selectionStartSaved");
        $jq(elem).removeAttr("selectionEndSaved");
	}
}
...
function popup_plugin_form( area_name, type, index, pageName, pluginArgs, bodyContent, edit_icon )
{
	...
	if (!bodyContent) { 
		if (document.getTASelection) {
			bodyContent = document.getTASelection();
...
function checkPasswordsMatch(in1, in2, el) {
	if ($jq(in1).val().length && $jq(in1).val() == $jq(in2).val()) {
		$jq(el).html("<img src='pics/icons/accept.png' style='vertical-align:middle' alt='Secure' /><em>Passwords match</em>");
		return true;
	} else {
		$jq(el).html("");
		return false;
