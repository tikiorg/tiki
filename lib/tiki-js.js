...
function insertAt(elementId, replaceString, blockLevel, perLine, replaceSelection) {
	...
	var lines, startoff = 0, endoff = 0, newIEoffset = 0;
	if ($textarea[0].createTextRange) {		// up to IE8 the length of $textarea[0].value !== val here but this seems to have stopped working in IE10
		...
		if (lines !== null) {
			startoff -= lines.length;	// remove one char per line for old IE
		} else {
			lines = val.substring(0, selection.start).match(/\n/g);		// IE10 has unix style linends but still puts the replacement in the wrong place
			if (lines) {
				newIEoffset -= lines.length;		// needs offsets on all selection points
			}
		}
	}
	var selectionStart = selection.start + newIEoffset;
	var selectionEnd = selection.end + newIEoffset;
