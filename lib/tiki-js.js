...
function insertAt(elementId, replaceString, blockLevel, perLine, replaceSelection) {
	...
	if ($('#cke_contents_' + elementId).length !== 0) {
		// get selection from ckeditor
		var cked = CKEDITOR ? CKEDITOR.instances[elementId] : null;
		if (cked) {
			...
			if (isPlugin) {
				...
				if (com.is("body")) {		// hmm, not very useful
					var r = sel.getRanges();
					if (r.length) {
						com = r[0].startContainer;
						while (!com.hasClass("tiki_plugin") && com !== r[0].endContainer && $(com.$).find(".tiki_plugin").length === 0) {
