...
function getCookie(name, section, defval) {
	if ( jqueryTiki.no_cookie && (window.XMLHttpRequest || window.ActiveXObject) && typeof tiki_cookie_jar != "undefined" && tiki_cookie_jar.length > 0) {
...
function build_plugin_form_row(row, name, label_name, requiredOrSpecial, value, description, paramDef, selectedMod, savedArgs)
{
	...
	if (paramDef && paramDef.type == 'image') {
		...
		$(field).append(
			$("<img />").attr("src", paramDef.icon)
				.addClass("icon")
				.css("cursor", "pointer")
					.attr("title", tr("Upload or record media"))
				.click(function() {openMediaUploader(
						"<input type=\"hidden\" name=\"from\" value=\"picker\" />" +
						"<input type=\"hidden\" name=\"area\" value=\"" + paramDef.area + "\" />"
					); return false;})
		);
	}
...
function openMediaUploader( formAdditions, formAction ) {
	if (jqueryTiki.ui) {

		formAdditions = formAdditions !== undefined ? formAdditions : "";
		formAction = formAction !== undefined ? formAction : "";

		var $d = $("<div id=\"kaltura_upload_dialog\" style=\"display:none\"></div>")
		...
		$d.dialog({
			...
			create: function(event, ui) {
				$.get("tiki-kaltura_upload.php?full=n", function (data, status) {
					...
					if (data) {
						data = data.replace(/<script.*?><\/script>/gi, "");	// remove scripts
						data = data.replace(/<link.*?\/>/gi, "");			// remove css
						$d.append(data);
						$(".navbar .button:first", $d).remove();

						$("#kcw").append($(formAdditions));
						if (formAction) {
							$("#kcw").attr("action", "tiki-wikiplugin_edit.php");
						}
					}
					ajaxLoadingHide();
				});
		...
		alert(tr("You need to enable feature_jquery"));
	}
