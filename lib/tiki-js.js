...
function addCodeMirrorEditorRelation(editor, $input, fullscreen) {
	window.codeMirrorEditor = (window.codeMirrorEditor ? window.codeMirrorEditor : []);
	window.codeMirrorEditor.push(editor);
	
	if (fullscreen) {
		$input
			.attr('codeMirrorRelationshipFullscreen', window.codeMirrorEditor.length - 1)
		...
		if ($.fn.resizable) {
			...
			codeWrapper.resizable({
				start: function() {
					codeWrapper.find('iframe').hide();
				},stop: function() {
					codeWrapper.find('iframe').show();
				}
			});
		}
