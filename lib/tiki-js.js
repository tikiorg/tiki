...
/**
 * New version of icntoggle function
...
function adjustRating(element, data) {
	...
	for (var i=0; i < $sibs.length; i++) {
		var v = $($sibs[i]).attr("onclick"), path = "";	// get value from onclick script
		if (v) {
			v = v.match(/,([-\d]+)\)/);
			if (v && v.length === 2) {
				v = v[1];
				path = $("img", $sibs[i]).attr("src");
				if (path) {
					path = path.match(/^(.*?)img\/icons\//);
					if (path.length > 1) {
						path = path[1];
					}
				}
				if (v <= data[0].voteavg && data[0].numvotes > 0) {
					if (data[0].result && data[0].my_rate == v) {
						$("img", $sibs[i]).attr("src", path + "img/icons/star_selected.png");
					} else {
						$("img", $sibs[i]).attr("src", path + "img/icons/star.png");
					}
				} else {
					if (data[0].result && data[0].my_rate == v) {
						$("img", $sibs[i]).attr("src", path + "img/icons/star_grey_selected.png");
					} else {
						$("img", $sibs[i]).attr("src", path + "img/icons/star_grey.png");
					}
				}
			}
		}
