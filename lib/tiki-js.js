...
function restoreTASelection( elem ) {
	...
	if (elem && $jq(elem).attr("selectionStartSaved")) {
...
function insertAt(elementId, replaceString, blockLevel, perLine, replaceSelection) {
	...
	if (selectionStart < 0 || (selectionStart == textarea.value.length && selectionStart == selectionEnd)) {	// couldn't get textarea selection via jq
		if (typeof $jq(textarea).attr("selectionStartSaved") === 'string' && $jq(textarea).attr("selectionStartSaved")) {	// forgetful firefox
			selectionStart = $jq(textarea).attr("selectionStartSaved");
			selectionEnd = $jq(textarea).attr("selectionEndSaved");
		} else {
...
function checkPasswordsMatch(in1, in2, el) {
	if ($jq(in1).val().length && $jq(in1).val() == $jq(in2).val()) {
		$jq(el).html("<img src='pics/icons/accept.png' style='vertical-align:middle' alt='Secure' /><em>" + tr("Passwords match") + "</em>");
		return true;
	} else {
		$jq(el).html("");
		return false;
