...
function build_plugin_form_row(row, name, label_name, requiredOrSpecial, value, description, paramDef)
{
	...
	if (paramDef.options) {
		...
		if (paramDef.filter && jqueryTiki.autocomplete) {
			if (paramDef.filter == "pagename") {
				$jq(input).autocomplete("tiki-listpages.php?listonly",
				...
				$jq(input).autocomplete("tiki-ajax_services.php?listonly=groups",
					{extraParams: {"httpaccept": "text/javascript"},
					 dataType: "json",
					 multiple: true,
					 multipleSeparator: "|",
				...
				$jq(input).autocomplete("tiki-ajax_services.php?listonly=users",
					{extraParams: {"httpaccept": "text/javascript"},
					 dataType: "json",
					 multiple: true,
					 multipleSeparator: "|",
