...
function toggle_autosaved(editorId, autoSaveId) {
	...
	if ($("#"+editorId+"_original").length === 0) {	// no save version already?
		...
		$.ajax({
			...
			success: function(data) {
				...
				$("#"+editorId).parents("form:first").
					append($("<input type='hidden' id='"+editorId+"_original' value='"+encodeURIComponent($("#"+editorId).val())+"' />"));
				if (cked) {
					cked.setData(output);
		...
		output = decodeURIComponent($("#"+editorId+"_original").val());
		if (cked) {
	...
	if (editorId) {
		...
		if (!ajaxPreviewWindow) {
			if (inPage) {
				...
				ajaxPreviewWindow = window.open('tiki-auto_save.php?editor_id=' + editorId + '&autoSaveId=' + encodeURIComponent(autoSaveId), '_blank', features);
			}
		} else {
