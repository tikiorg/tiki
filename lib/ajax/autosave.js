...
function toggle_autosaved(editorId, autoSaveId) {
	...
	if ($("#"+editorId+"_original").length === 0) {	// no save version already?
		...
		$.ajax({
			...
			success: function(data) {
				...
				$("#"+editorId).parents("form:first").
					append($("<input type='hidden' id='"+editorId+"_original' value='"+tiki_encodeURI($("#"+editorId).val())+"' />"));
				if (cked) {
					cked.setData(output);
		...
		output = tiki_decodeURI($("#"+editorId+"_original").val());
		if (cked) {
...
function auto_save( editorId, autoSaveId ) {
	...
	if ( !auto_save_submit && editorId && autoSaveId && !sending_auto_save) {
		...
		if (auto_save_data[editorId] !== data || $('#' + editorId).attr("old_allowhtml") != allowHtml) {
			...
			$.ajax({
				url: 'tiki-auto_save.php',
				data: 'command=auto_save&editor_id=' + editorId + '&data=' + tiki_encodeURIComponent(data) + '&referer=' + autoSaveId
									+ "&allowHtml=" + allowHtml,
				type: "POST",
				// good callback
				success: function(data) {
	...
	if (editorId) {
		...
		if (!ajaxPreviewWindow) {
			if (inPage) {
				...
				ajaxPreviewWindow = window.open('tiki-auto_save.php?editor_id=' + editorId + '&autoSaveId=' + tiki_encodeURI(autoSaveId), '_blank', features);
			}
		} else {
			if (typeof ajaxPreviewWindow.get_new_preview === 'function') {
				...
				ajaxPreviewWindow.open('tiki-auto_save.php?editor_id=' + editorId + '&autoSaveId=' + tiki_encodeURI(autoSaveId));
			}
		}
	} else {
		if (auto_save_debug) {
