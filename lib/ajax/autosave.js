...
function toggle_autosaved(editorId, autoSaveId) {
	...
	if ($("#"+editorId+"_original").length === 0) {	// no save version already?
		...
		$.ajax({
			...
			success: function(data) {
				...
				$("#"+editorId).parents("form:first").
					append($("<input type='hidden' id='"+editorId+"_original' value='"+encodeURI($("#"+editorId).val())+"' />"));
				if (cked) {
					cked.setData(output);
		...
		output = decodeURI($("#"+editorId+"_original").val());
		if (cked) {
	...
	if (editorId) {
		...
		if (!ajaxPreviewWindow) {
			if (inPage) {
				...
				ajaxPreviewWindow = window.open('tiki-auto_save.php?editor_id=' + editorId + '&autoSaveId=' + encodeURI(autoSaveId), '_blank', features);
			}
		} else {
			if (typeof ajaxPreviewWindow.get_new_preview === 'function') {
				...
				ajaxPreviewWindow.open('tiki-auto_save.php?editor_id=' + editorId + '&autoSaveId=' + encodeURI(autoSaveId));
			}
		}
	} else {
		if (auto_save_debug) {
