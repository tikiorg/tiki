...
function ajax_preview(id) {
	...
	if (auto_save_id.length >= id) {
		...
		ajaxPreviewWindow = window.open('tiki-auto_save.php?editor_id=' + auto_save_id[0] + '&autoSaveId=' + escape(autoSaveId),'_blank',features);
		} else {
			if (typeof ajaxPreviewWindow.get_new_preview == 'function') {
				ajaxPreviewWindow.get_new_preview();
				ajaxPreviewWindow.focus();
			} else {
				ajaxPreviewWindow.open('tiki-auto_save.php?editor_id=' + auto_save_id[0] + '&autoSaveId=' + escape(autoSaveId));
			}
		}
	}
