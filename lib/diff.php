	...
	function reverse ($from_lines)
   {
		...
		for (reset($this->edits), $currentedits = current($this->edits); $edit = $currentedits; next($this->edits))
		{
			if (is_array($edit)) { // Was an add, turn it into a delete.
				...
				$edit = -$nadd;
			}elseif ($edit > 0){ // Was a copy --- just pass it through.	      }
				$x += $edit;
	      }else if ($edit < 0){ // Was a delete, turn it into an add.
				$ndelete = -$edit;
				$edit = array();
				while ($ndelete-- > 0)
					$edit[] = "" . $from_lines[$x++];
		...
		while ($left || $right)
		{
			...
			if (is_array($op) && is_array($newop)){
				...
				for ($i = 0, $sizeof_newop = count($newop); $i < $sizeof_newop; $i++)
					$op[] = $newop[$i];
			}elseif (($op > 0 && $newop > 0) || ($op < 0 && $newop < 0)){ // $op and $newop are both either deletes or copies.
				$op += $newop;
			}else {
				$comp->edits[] = $op;
				$op = $newop;
...
class WikiDiffFormatter
{
	...
	function _format ($edits, $from_lines)
	{
		...
		{
			if (!is_array($edit) && $edit >= 0){ // Edit op is a copy.
				$ncopy = $edit;
			}else {
				$ncopy = 0;
				if (empty($hunk)){
				...
				if (is_array($edit)){ // Edit op is an add.
					$y += count($edit);
					$hunk[$this->do_reverse_diff ? 'd' : 'a'] = $edit;
				}else { // Edit op is a delete
					$deletes = array();
					while ($edit++ < 0)
	...
	function _emit_diff ($xbeg,$xlen,$ybeg,$ylen,$hunks)
	{
		...
		$color = array('c' => '#ffffff','a' => '#ccffcc','d' => '#ffcccc');
