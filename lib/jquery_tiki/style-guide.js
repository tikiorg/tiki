...
function getSelector(el) {
	var selector = "";
	if (el.attr('data-selector')) selector = el.attr('data-selector');
	selector = selector.split(',');
	return selector[selector.length - 1].trim();
}
...
function checkIV(el, selector) {
	...
	if ($(selector).css(bfa).indexOf('rgb(') > -1) {
		css = $(selector).css(bfa);
		part1 = css.slice(0, css.indexOf('rgb('));
		part2 = css.slice(css.indexOf('rgb('));
		el.val( part1 + toHex(part2) );
	} else if (bfa === 'padding') {
		if ($(selector).css('padding-top') === $(selector).css('padding-left') &&
				$(selector).css('padding-top') === $(selector).css('padding-bottom') &&
				$(selector).css('padding-top') === $(selector).css('padding-right')) {
			el.val( $(selector).css('padding-top') );
		} else {
			el.val( $(selector).css('padding-top') + ' ' + $(selector).css('padding-right') + ' ' + $(selector).css('padding-bottom') + ' ' + $(selector).css('padding-left') );
		}
	} else if (bfa === 'border-radius') {
		if ($(selector).css('border-top-left-radius') === $(selector).css('border-top-right-radius') &&
				$(selector).css('border-top-left-radius') === $(selector).css('border-bottom-right-radius') &&
				$(selector).css('border-top-left-radius') === $(selector).css('border-bottom-left-radius')) {
			el.val( $(selector).css('border-top-left-radius') );
		} else {
			el.val( $(selector).css('border-top-left-radius') + ' ' + $(selector).css('border-top-right-radius') + ' ' + $(selector).css('border-bottom-right-radius') + ' ' + $(selector).css('border-bottom-left-radius') );
		}
	} else if (bfa === 'border') {
		el.val( $(selector).css('border-top-width') + ' ' + $(selector).css('border-top-style') + ' ' + toHex($(selector).css('border-top-color')) );
	} else {
		el.val( $(selector).css(bfa) );
	}
...
function updateInputValue(el) {
	...
	if (keep) saveCustomCSS();
}



// save custom CSS to cookies
function saveCustomCSS() {
	var mid = parseInt(Object.keys(inputJSON).length * 0.5);
	var obj1 = {};
	var obj2 = {};
	for (var key in inputJSON) {
		mid > key ? obj1[key] = inputJSON[key] : obj2[key] = inputJSON[key];
	}

	setCookie('input-data1', JSON.stringify(obj1, null, 0), { expires: 365 });
	setCookie('input-data2', JSON.stringify(obj2, null, 0), { expires: 365 });
	setCookie('custom-data', JSON.stringify(customJSON, null, 0), { expires: 365 });
}



// load custom CSS from cookies
function loadCustomCSS() {
	var obj1 = $.getJSON(getCookie('input-data1'));
	var obj2 = $.getJSON(getCookie('input-data2'));
	customJSON = $.getJSON(getCookie('custom-data'));
	$.extend(inputJSON, obj1, obj2);
}
...
function deleteCookies() {
	deleteCookie('input-data1');
	deleteCookie('input-data2');
	deleteCookie('custom-data');
}



// stuff to fix for SG to work with all themes
function themeFix() {
	var $body = $('body');
...
//			console.log(JSON.stringify(baseJSON, null, 2));
	...
	// set some values if cookies exist
	if (getCookie('save-changes')) {
		keep = true;
	...
	// generate bootstrap variables when clicked
	...
	// cookies behaviour when checking/unchecking
	$('.keep-changes').on('change', function() {
		...
		setCookie("save-changes", keep, "", 365);
	});
