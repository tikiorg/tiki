...
SG.prototype = {
	...
	generateLessVars: function(sources) {
		// add generated variables based on # of times same color, font, size appears
		if (sources.constructor !== Array) sources = [sources];
		this.data = "/*\n";
		this.data += "==========================================================================\n";
...
function saveCustomCSS() {
	...
	setCookie('input-data1', JSON.stringify(obj1, null, 0));
	setCookie('input-data2', JSON.stringify(obj2, null, 0));
	setCookie('custom-data', JSON.stringify(customJSON, null, 0));
}
...
function loadCustomCSS() {
	var obj1 = JSON.parse(getCookie('input-data1'));
	var obj2 = JSON.parse(getCookie('input-data2'));
	customJSON = JSON.parse(getCookie('custom-data'));
...
//			console.log(JSON.stringify(baseJSON, null, 2));
	...
	// generate bootstrap variables when clicked
	$('.generate-less').click(function() {
		$("textarea[name=header_custom_less]").val(
			sg.generateLessVars([inputJSON])
		);
		return false;
	});
