	...
	obtainExecutor = function (container) {
		...
		return executors[url] = delayedExecutor(5000, function () {
			...
			$(containers).each(function () {
				$(this).modal($(this).width() > 30 ? tr("Saving...") : " ");
			});
			...
			$.post(url, parts.join('&'), 'json')
				.success(function () {
					$(containers).
						...
						each(function () {
							var $this = $(this),
								url = $(this).data('fieldFetchUrl') + "&mode=output";	// use the url for the field input in output mode
		...
		if (url) {
			$.get(url)
				.success(function (data) {
					...
					if ($radios.length) {
						var itemId = $(container).data("field-fetch-url").match(/itemId=(\d+)/);
						if (itemId) {
							itemId = itemId[1];
						}
