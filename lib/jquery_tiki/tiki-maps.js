		...
		this.each(function () {
			...
			setTimeout(function () {
				...
				if (layers.length > 1 && -1 !== $.inArray('controls', desiredControls)) {
					...
					defaultMode.controls.push(new OpenLayers.Control.PanZoom());
				}

				if (layers.length > 1 && -1 !== $.inArray('layers', desiredControls)) {
					defaultMode.controls.push(new OpenLayers.Control.LayerSwitcher());
				}
				...
				container.controlModes.addMode(defaultMode);
