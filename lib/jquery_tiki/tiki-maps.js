			...
			setTimeout(function () {
				...
				container.defaultStyleMap = new OpenLayers.StyleMap({
					...
					"select": new OpenLayers.Style({
						externalGraphic: "lib/openlayers/img/marker-gold.png",
						graphicWidth: 21,
						graphicHeight: 25,
						graphicXOffset: -10.5,
						graphicYOffset: -25,
				...
				if (jqueryTiki.googleStreetView) {
					...
					if (jqueryTiki.googleStreetViewOverlay) {
						container.streetview.overlay = new OpenLayers.Layer.XYZ(
							"StreetView Overlay",
							currentProtocol + "//cbk0.google.com/cbk?output=overlay&zoom=${z}&x=${x}&y=${y}&cb_client=api",
						...
						$.post('tiki-searchindex.php?filter~geo_located=y', $(this).serialize(), function (data) {
							...
							$.each(data, function (k, i) {
								...
								if (i.geo_location) {
									...
									var format, new_layer;
									var files = i.geo_file.split(",");
									...
									for (var f = 0; f < files.length; f++) {
										new_layer = new OpenLayers.Layer.Vector(i.title + " (" + i.geo_file_format + " file)", {
											//projection: new OpenLayers.Projection('EPSG:900913'),
											// N.B. Currently only supporting default EPSG:4326 projection files (12.x)
											strategies: [new OpenLayers.Strategy.Fixed()],
											style: {
												fillColor: "#aa6633", fillOpacity: 0.3,
												strokeColor: "#ffa500", strokeWidth: 2, strokeOpacity: 0.8,
												pointRadius: 5,
												cursor: "pointer"
											},
										...
										new_layer.events.register("loadend", new_layer, function (e) {
											$.each(this.features, function (k, feature) {

												feature.attributes.itemId = i.object_id;
												if (!feature.attributes.color) {
													feature.attributes.color = '#FF9600';	// currently not active
												}
											...
											// hook up the new layer to the pre-defined events (to open popups)
											vectorLayerList.push(this);
											setupLayerEvents(this);
										...
										container.map.addLayer(new_layer);
									}
								}
							});
						}, 'json');
