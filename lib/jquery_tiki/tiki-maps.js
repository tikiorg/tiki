...
(function () {
	...
	function fixLayerOrder(container) {
		var map = container.map;
		$('.olLayerDiv', container).each(function () {
			if (this.id.indexOf('OpenLayers.Layer.Markers') > -1) {
	...
	$.fn.createMap = function () {
		this.each(function () {
			...
			setTimeout(function () {
				...
				var selectControl;
				defaultMode.controls.push(selectControl = new OpenLayers.Control.SelectFeature(container.vectors, {
				...
				setInterval(function () {
					var target = container.activePopup, w = 0,h = 0;
					if (target) {
						...
						if (target.contentSize.w < w || target.contentSize.h < h) {
							target.setSize(new OpenLayers.Size(w, h));
							target.contentSize.w = w;
							target.contentSize.h = h;
							target.panIntoView();
						}
					}
				}, 1000);
			...
			container.markerIcons.createMarker(iconModel, lonlat, function (marker) {
				...
				if (options.content) {
					marker.events.register('click', marker, function () {
						...
						if (! loaded) {
							popup = new OpenLayers.Popup.FramedCloud('marker', lonlat, null, options.content, marker.icon, true, function () {
		...
		if (options.type && options.object && $.inArray(options.type,  jqueryTiki.infoboxTypes) !== -1) {
			...
			this.each(function () {
				...
				$.get($.service('object', 'infobox', {
					...
					newPopup = new OpenLayers.Popup.FramedCloud('marker', options.lonlat, null, data, options.hook, true, close);
