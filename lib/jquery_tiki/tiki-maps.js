...
(function () {
	...
	$.fn.addMapMarker = function (options) {
		this.each(function () {
			...
			if (options.layer) {
				if (! container.layers[options.layer]) {
					container.layers[options.layer] = new OpenLayers.Layer.Markers(options.layer);
					container.map.addLayer(container.layers[options.layer]);
				}

				markerLayer = container.layers[options.layer];
			}
						...
						if (options.type && options.object && $.inArray(options.type,  jqueryTiki.infoboxTypes) !== -1) {
							$.get($.service('object', 'infobox', {
								...
								if (container.activePopup === popup) {
									container.map.removePopup(container.activePopup);
									container.activePopup = new OpenLayers.Popup('marker', lonlat, null, data);
									container.activePopup.autoSize = true;
									container.map.addPopup(container.activePopup);
								}
							}, 'html');
