				...
				$(container).closest('.tab, #appframe, #tiki-center').find('form.search-box')
					...
					.submit(function () {
						...
						$.post('tiki-searchindex.php?filter~geo_located=y', $(this).serialize(), function (data) {
							var layer = $(form).data('result-layer'), destination;

							destination = container.getLayer(layer);
							if (form.priorList) {
							...
							$.each(data, function (k, i) {
								...
								if (i.geo_location) {
								...
								} else if (i.geo_feature && destination.vectors) {
									...
									destination.vectors.addFeatures(features);
								}
							});
						}, 'json');
						return false;
