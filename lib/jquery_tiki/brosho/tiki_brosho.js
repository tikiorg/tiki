...
function toggle_brosho() {
	...
	$jq("#brosho-selector").prepend($jq("<img src='pics/icons/close.png' style='float:right;' />").click(function(){
	...
	$jq("#brosho-controls").append($jq("<li id='brosho-copy' />").text("Copy to Custom CSS: ").append($jq("<a href='#'>Copy</a>")).click(function(){
		$jq("textarea[name=header_custom_css]").val($jq("textarea[name=header_custom_css]").val() + "\n" + brosho_get_code());
	}));
	$jq("#brosho-generate, #brosho-position").remove();
	if ($jq.ui) {
		var h = $jq("#brosho-wrapper").height();
		$jq("#brosho-wrapper").resizable({maxHeight:h,minHeight:h}).draggable();
	}
	return false;
}
...
function brosho_get_code() {
	...
	$jq('body *').each(function() { //check every element for changed css
		var el = $jq(this); //used several times
		if (el.length) {
			...
			if(brosho_css !== undefined || brosho_css) { //does this element have changed css
				var temp_css = $jq.fn.brosho.extractCssSelectorPath(el) + ' {\n'; //generate css selector path
