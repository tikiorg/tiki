...
elFinder.prototype.commands.info = function() {
	...
	this.exec = function(hashes) {
		...
		// start tiki
		// TODO perms checking here?
		var disabledCommands = ['editdesc'],
			viewDesc = $.inArray('desc', disabledCommands) == -1 ? true : false,
			editDesc = $.inArray('editdesc', disabledCommands) == -1 ? true : false,
		 	viewUser = $.inArray('user', disabledCommands) == -1 ? true : false,
		 	viewHits = $.inArray('hits', disabledCommands) == -1 ? true : false,
		 	viewSyntax = $.inArray('syntax', disabledCommands) == -1 ? true : false;
		// end tiki
		...
		if (cnt == 1) {
			...
			fm.request({
				...
				.done(function (data) {
					...
					fieldDesc.empty().html(data.info.description || '');
				});
			// end tiki
		} else {
