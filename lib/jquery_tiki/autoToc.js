		...
		if ($headers.length > 1) {
			...
			if (!jqueryTiki.autoToc_inline) { 
				...
				if (jqueryTiki.autoToc_pos === "left" || jqueryTiki.autoToc_pos === "top") {
					$tocDiv.append($list).insertBefore('#page-data');
				} else {
					$tocDiv.append($list).insertAfter('#page-data');
				}
				...
				$("> .nav", "#autotoc").affix({
					offset: {
						...
						bottom: function () {
							return (this.bottom = $(document).height() - $('#page-data').offset().top - $('#page-data').height())
						}
					}
				})
