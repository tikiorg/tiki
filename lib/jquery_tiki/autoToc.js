...
$.buildAutoToc = function () {
	...
	if (document.getElementById("page-data") && !document.getElementById("toc-off") && document.location.href.indexOf("tiki-print.php") == -1) {
		...
		if (headers.length > 1) {
			...
			tocString = "<div id='outerToc' contenteditable='false'> <a href='#'> Table of Contents, or Click for Page Top</a><div id='innerToc' contenteditable='false'></div></div><div id='outerToc-static' contenteditable='false'><div id='toc-static'></div></div>";
			...
			for (i = 0; i < headers.length; i++) {
				...
				if (headerLevel >= largestHeader && headerLevel <= smallestHeader) {
					...
					//aText = $.trim(currentHeader.innerHTML); //trim incase inner html is text (if <hx>text</hx>) //enable this line and comment out line below if you want to carry text formatting and images used in headers instead of converting them to text
					...
					if (!currentHeader.id){
						...
						//set the id on the elmeent
						currentHeader.setAttribute("id", id);
					}

					//if the element already has an Id, just use the one it has for processing	
					else {
						id = currentHeader.id;
					}
