		...
		var layers = [], tiles = jqueryTiki.mapTileSets, factories = {
			...
			mapquest_aerial: function () {
				return new ol.layer.Tile({
					...
					name: 'MapQuestSatellite'
				});
			},
			mapquest_hybrid: function () {
				return new ol.layer.Tile({
					source: new ol.source.MapQuest({ layer: 'hyb' }),
					name: 'MapQuestHybrid'
				});
			}
/* coo, no google layers in ol3? :(
			google_street: function () {
				return new ol.Layer.Google(
					"Google Streets",
					{numZoomLevels: 20}
				);
			},
			...
			blank: function () {
				...
				var layer = new ol.Layer.OSM(tr('Blank'));
				layer.isBlank = true;
				return layer;*/
			/* Needs additional testing
			},
			visualearth_road: function () {
				return new ol.Layer.VirtualEarth(
					"Virtual Earth Roads",
...
*/
	...
	$.fn.createMap = function () {
		this.each(function () {
			...
			setTimeout(function () {
				ol.ImgPath = "img/openlayers/dark/";
...
*/
				...
				container.vectors = new ol.layer.Vector({
...
/* tooltips & highlighting TODO

				if ($(container).data("tooltips")) {
					defaultMode.controls.push(highlightControl = new ol.control.SelectFeature(vectorLayerList, {
						hover: true,
						highlightOnly: true,
						...
						eventListeners: {
							...
							featurehighlighted: function (evt) {
								...
								var popup = new ol.Popup.Anchored(
									'myPopup',
									lonlat,
									new ol.Size(150, 18),
									"<small>" + evt.feature.get("content") + "</small>",
				...
				defaultMode.controls.push(selectControl = new ol.control.SelectFeature(vectorLayerList, {
					onSelect: function (feature) {
						if (container.tooltipPopup) {
							map.removePopup(container.tooltipPopup);
							container.tooltipPopup = null;
						}
						if (feature.get("url") === container.markerIcons.loadedMarker["default"].url) {
							feature.set("url", container.markerIcons.loadedMarker.selection.url);
						...
						if (! loaded && feature.get("content")) {
							var popup = new ol.Popup.FramedCloud('feature', lonlat, null, feature.get("content"), null, true, function () {
								$(container).setMapPopup(null);
							});
							popup.autoSize = true;
					...
					onUnselect: function (feature) {
						if (feature.get("url") === container.markerIcons.loadedMarker.selection.url) {
							feature.set("url", container.markerIcons.loadedMarker["default"].url);
							feature.layer.redraw();
						}
					}
				}));
...
/*							container.map.setLayerZIndex(vectors, vectorLayerList.length * 1000);
				...
				if (central) {
					container.resetPosition = function () {
						map.setView(new ol.View({
							center: [central.lon, central.lat],
							zoom: central.zoom
						}));
					};
				...
				$(container).on('search', function (e, data) {
					function markLocation (lat, lon, bounds) {
						...
						if (typeof zoomToFoundLocation != 'undefined') {
							...
							if (zoomFactor < 0) {
								...
								map.setCenter(lonlat);
								map.zoomToScale(zoomFactor * ol.INCHES_PER_UNIT.m);
							}
						} else {
					...
					function markGoogleLocation(result)
					{
						...
						osw = new ol.LonLat(sw.lng(), sw.lat()).transform(
							new ol.proj.Projection("EPSG:4326"),
							map.getProjectionObject()
						);
						one = new ol.LonLat(ne.lng(), ne.lat()).transform(
							new ol.proj.Projection("EPSG:4326"),
							map.getProjectionObject()
						);
						...
						markLocation(loc.lat(), loc.lng(), new ol.Bounds(left, bottom, right, top));
					}
	...
	$.fn.addMapMarker = function (options) {
		this.each(function () {
			...
			if (options.lat && options.lon) {
				lonlat = new ol.geom.Point([options.lon, options.lat]);
			}
			...
			container.markerIcons.createMarker(iconModel, lonlat, function (feature) {
				...
				markerLayer.getSource().addFeatures([feature]);
	...
	$.fn.loadInfoboxPopup = function (options) {
		if (options.type && options.object && $.inArray(options.type,  jqueryTiki.infoboxTypes) !== -1) {
			...
			this.each(function () {
				...
				popup = new ol.Popup('marker', options.lonlat, null, options.content + '<img src="img/spinner.gif"/>');
				popup.autoSize = true;
				$(container).setMapPopup(popup);
