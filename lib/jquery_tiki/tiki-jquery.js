...
(function($) {
	...
	function loadSelectorData(filter, /* optional */ count, success) {
		...
		if (count) {
			params.maxRecords = count;
		}
		
		url = $.service('search', 'lookup', params);
	...
	$.fn.object_selector = function (prepareOptionFn, value, title) {
		...
		this.each(function () {
			var input = this, $simple = $(this).prev(), filter = $(input).data('filters'), threshold = $(input).data('threshold');
			$(input).hide();
			$simple.hide();
			...
			var $spinner = $(this).parent(),
				...
				$results = $filter.closest('.object-selector').find('.results'),
				$noresults = $filter.closest('.object-selector').find('.no-results');

			if (prepareOptionFn === 'set') {
				$select.val(value);
			...
			$spinner.tikiModal(" "),
			loadSelectorData(filter, threshold, function (data) {
				$select
					.empty()
					.append('<option/>');

				$selection.text($(input).data('label'));
				...
				if (data.resultset.count <= threshold) {
					...
					$search.click(function (e) {
						...
						this.ajax = loadSelectorData($.extend(filter, {title: $filter.val()}), function (data) {
							$results.empty();
							$noresults.removeClass('hidden');
				...
				$(input).object_selector('set', $(input).val(), $(input).data('label'));
			});
	...
	$.fn.object_selector_multi = function (action) {
		...
		this.each(function () {
			var $textarea = $(this).hide()
				...
				, $results = $container.find('.results').first()
				, $noresults = $container.find('.no-results').first()
			...
			function getList()
			{
				return $.makeArray($container.find(':checkbox:checked').map(function () {
					return $(this).val();
				}));
			}
			...
			$search.click(function (e) {
				...
				$spinner = $filter.parent().tikiModal(" ");
				loadSelectorData($.extend(filter, {title: $filter.val()}), function (data) {
					$noresults.removeClass('hidden');
					$results.find(':checkbox:not(:checked)')
						.closest('.checkbox')
						.remove();

					var current = getList();
					$.each(data.resultset.result, function (key, value) {
						var thisItem = value.object_type + ':' + value.object_id,
							item = $('<div class="checkbox"><label><input type="checkbox"> </label></div>');

						if (-1 !== $.inArray(thisItem, current)) {
							return;
						}
...
$(document).on('shown.bs.modal', '.modal.fade', function () {
	$(this).next('.modal').css('z-index', $(this).css('z-index') + 10);
	$('.modal-backdrop').last().css('z-index', $(this).css('z-index') - 1);
});
