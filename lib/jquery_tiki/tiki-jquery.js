	...
	if (jqueryTiki.sheet) {
		...
		$.sheet.get_sheet_json = function(sheetInstance) {	// diverged from jQuery.sheet 1.1 / Tiki 6
			...
			jQuery(sheetClone).each(function() {
				...
				//This preserves the width for postback
				var table = jQuery(this);
				var trFirst = table.find('tr:first');
				table.find('col').each(function(i){ 
					trFirst.find('td').eq(i).css('width', jQuery(this).css('width'));
				});
	...
	var prepareLink = function( link, ins, disp ) {
		...
		if ( isSheet ) {
			link.onclick = function() {
				$.sheet.instance[0].cellChangeStyle(styleType, $(link).children().first().css('background-color'));
