					...
					case "jscalendar":	// replacements for jscalendar
						...
						opts = {
							...
							onClose: function(dateText, inst) {
								...
								if (inst.settings && inst.settings.timepicker) {	// if it's a datetimepicker add on the time
								...
								$(inst.settings.altField).val(timestamp);
							}
						};
						break;
					default:
...
(function($) {
	...
	$.fn.loadService =  function (data, options) {
		...
		$dialog.load($.service(controller, action), data, function () {
			...
			$dialog.find('form:not(.no-ajax)').submit(function (e) {
				var form = this;
				$.ajax($.service(controller, action), {
					type: 'POST',
					dataType: 'json',
					data: $(form).serialize(),
					success: function (data) {
	...
	$.fn.object_selector = function (filter, threshold) {
		var input = this;
		this.each(function () {
			var $spinner = $(this).parent().modal(tr("Loading"));
					...
					filterField.keypress(function (e) {
						...
						this.searching = setTimeout(function () {
							...
							if ((loaded === "*" || loaded.length >= 3) && loaded !== $select.data('loaded')) {
								$spinner = $(field).parent().modal(tr("Loading"));
