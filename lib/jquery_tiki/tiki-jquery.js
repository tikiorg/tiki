...
$(document).ajaxComplete(function (e, jqxhr) {
	...
	if (feedback) {
		tikiFeedback.fadeIn(200, function() {
			...
			//it's important to not compare positions until after the tikiFeedback object has been manipulated
			
			//place tikifeedback div into window view if necessary
			if (tikiFeedback.offset().top < $(window).scrollTop()) {
				$(window).scrollTop(tikiFeedback.offset().top);
			}
		});
	}
