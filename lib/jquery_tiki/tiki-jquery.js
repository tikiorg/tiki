...
(function($) {
	...
	$.fn.createMap = function () {
		...
		this.each(function () {
			...
			setTimeout(function () {
				container.map = new OpenLayers.Map(id);
				container.layer = new OpenLayers.Layer.OSM();
				container.markers = new OpenLayers.Layer.Markers();
				container.map.addLayer(container.layer);
				container.map.addLayer(container.markers);
				container.map.zoomToMaxExtent();
				...
				if (field) {
					...
					if (! field.attr('disabled')) {
						var MyClick = OpenLayers.Class(OpenLayers.Control, {
							...
							trigger: function(e) {
								var lonlat = container.map.getLonLatFromViewPortPx(e.xy).transform(
									container.map.getProjectionObject(),
									new OpenLayers.Projection("EPSG:4326")
								);
								field.val(lonlat.lat + ',' + lonlat.lon + ':' + container.map.getZoom()).change();
								if (container.locationMarker) {
									container.markers.removeMarker(container.locationMarker);
								}

								container.locationMarker = new OpenLayers.Marker(container.map.getLonLatFromViewPortPx(e.xy), markerIcon.clone());
								container.markers.addMarker(container.locationMarker);
							}
						});

						var click = new MyClick();
						container.map.addControl(click);
						click.activate();
					}
					...
					if (matching) {
						...
						container.map.setCenter(lonlat, zoom);
