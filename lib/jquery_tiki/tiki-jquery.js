...
//			widgets: ['zebra'],							// stripes (coming soon)
	...
	if (jqueryTiki.colorbox && !jqueryTiki.mobile) {
		...
		/****************** NOTE ******************
		 * Use of rel attribute below is generally not valid HTML (especially since we haven't defined in a profile URI
		 * in the head element) and should not be used going forward. Use data-box instead. Selectors below accept both
		 * for backwards compatibility until prior uses can be converted
		 * */
		
		//for every link containing 'shadowbox', 'colorbox', 'box', etc. in rel or data-box attribute
		$("a[rel*='box'], a[data-box*='box']").colorbox({
			rel: function(){
		...
		$("a[rel*='box'][rel*='type=img'], a[rel*='box'][rel!='type='], a[data-box*='box'][data-box*='type=img'], a[data-box*='box'][data-box!='type=']").colorbox({
			photo: true
		});
		// rel containg slideshow (this one must be without #col1)
		$("a[rel*='box'][rel*='slideshow'], a[data-box*='box'][data-box*='slideshow']").colorbox({
			photo: true,
			slideshow: true,
			slideshowSpeed: 3500,
			preloading: false,
		...
		//same as above but using data-box - these should be kept in sync
		...
		$("#col1 a[rel*='box'][href^='ftp://'], #col1 a[rel*='box'][href^='ftps://'], #col1 a[data-box*='box'][href^='ftp://'], #col1 a[data-box*='box'][href^='ftps://']").colorbox({
			iframe: true,
			width: "95%",
			height: "95%"
		});
		// rel containg type=flash
		$("#col1 a[rel*='box'][rel*='type=flash'], #col1 a[data-box*='box'][data-box*='type=flash']").colorbox({
			inline: true,
			width: "60%",
			height: "60%",
			href: function () {
		...
		$("#col1 a[rel*='box'][rel*='type=iframe'], #col1 a[data-box*='box'][data-box*='type=iframe']").colorbox({
			iframe: true
		});
		// inline content: hrefs starting with #
		$("#col1 a[rel*='box'][href^='#'], #col1 a[data-box*='box'][href^='#']").colorbox({
			inline: true,
			width: "50%",
			height: "50%",
			href: function(){ 
		...
		$("a[rel*='box'][title], a[data-box*='box'][title]").colorbox({
			title: function(){ 
				return $(this).attr('title');
			}
		});
		// but prefer the title from title attribute of a wrapped image if any (in all columns)
		$("a[rel*='box'] img[title], a[data-box*='box'] img[title]").colorbox({
			title: function(){ 
				return $(this).attr('title');
			},
		...
		$("#col1 a[rel*='box'][rel*='title='], #col1 a[data-box*='box'][data-box*='title=']").colorbox({
			title: function () {
				re = /(title=([^;\"]+))/i;
		...
		$("#col1 a[rel*='box'][rel*='height='], #col1 a[data-box*='box'][data-box*='height=']").colorbox({
			height: function () {
				re = /(height=([^;\"]+))/i;
				ret = $(this).attr("rel").match(re);
				return ret[2];
		...
		$("#col1 a[rel*='box'][rel*='width='], #col1 a[data-box*='box'][data-box*='width=']").colorbox({
			width: function () {
				re = /(width=([^;\"]+))/i;
				ret = $(this).attr("rel").match(re);
				return ret[2];
		...
		if (jqueryTiki.colorbox) {
			$("a[rel^='shadowbox[colorbox'], a[data-box^='shadowbox[colorbox']").each(function () {$(this).attr('savedTitle', $(this).attr('title'));});
			$("a[rel^='shadowbox[colorbox'], a[data-box^='shadowbox[colorbox']").colorbox({
				title: function() {
	...
	if (jqueryTiki.zoom) {
		$("a[rel*=zoom], a[data-box*=zoom]").each(function () {
			$(this)
