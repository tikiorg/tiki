...
(function($) {
	...
	$.fn.object_selector = function (filter, threshold) {
		var input = this;
		this.each(function () {
			var $spinner = $(this).parent().modal(true);
					...
					filterField.keypress(function (e) {
						...
						this.searching = setTimeout(function () {
							...
							if ((loaded === "*" || loaded.length >= 3) && loaded !== $select.data('loaded')) {
								$spinner = $(field).parent().modal(true);
		...
		$(this).each(function() {
			...
			function visibilityHelper() {
				if (!me.is(':visible')) {
					...
					me
						.trigger('visible')
						.data('visibilityHelper', false);
				}
			}
			
			if (!me.data('visibilityHelper')) {
				me.data('visibilityHelper', true);
				visibilityHelper();
			}
		});
	} else {
...
$.uiIcon = function(type) {
	return $('<div class="ui-state-default ui-corner-all" style="width: 1.4em; height: 1.4em; margin: .2em; display: inline-block; cursor: pointer;">' +
