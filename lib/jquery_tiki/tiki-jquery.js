...
//			widgets: ['zebra'],							// stripes (coming soon)
	...
	if (jqueryTiki.colorbox) {
		...
		// links generated by the {COLORBOX} plugin - FIXME: seems to loose the description on the first image, but better than having <br /> tags showing in tooltip
		$jq("a[rel^='shadowbox[colorbox']").cluetip({ splitTitle: '<br />', cluezIndex: 400, width: 'auto', fx: {open: 'fadeIn', openSpeed: 'fast'}, clickThrough: true, attribute: false });
	}
	...
	if (jqueryTiki.sheet) {
		...
		jS.get_sheet_json = function() {	// function coming soon to jQuery.sheet
	...
	switch (func) {
		...
		case "sheet":
			if (jqueryTiki.sheet) {
				...
				var sheet_theme = jqueryTiki.ui ? "lib/jquery/jquery-ui/themes/" + jqueryTiki.ui_theme + "/jquery-ui.css" : "lib/jquery/jquery.sheet/theme/jquery-ui-1.8.custom.css";
...
//		 						$jq("#jSheetUI .jSheetBarTopParent, #jSheetUI .sheetPane, #jSheetUI .jSheetEditPane")
//		 							.width($jq(this).width() - $jq("#jSheetUI .jSheetBarCornerParent").width());
//		 						$jq("#jSheetUI .barLeft>div, #jSheetUI .sheetPane, #jSheetUI .jSheetEditPane")
//		 							.height($jq(this).height() - $jq("#jSheetUI .jSheetBarCornerParent").height() - $jq("#jSheetControls").height());
//		 					}});
