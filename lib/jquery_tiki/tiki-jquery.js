...
function toggleFullScreen(area_name) {
	var $ta = $jq("#" + area_name);
	var $diff = $jq("#diff_outer"), $edit_form, $edit_form_innards;

	if (!$ta.length) {
		$ta = $jq(getElementById(area_name));
		area_name = $ta.attr("id");
	}
	...
	if (fullScreenState[area_name]) {	// leave full screen
		...
		if ($diff.length) {
			...
			fullScreenState[area_name]["edit_form"]["el"] = $edit_form[0];
		...
		if ($diff.length) {
			...
			$grippy = $jq("<div id='fs_grippy' />").css({"background-image": "url(pics/icons/shading.png)",
