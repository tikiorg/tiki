...
$(document).ajaxComplete(function (e, jqxhr) {
	...
	if (feedback) {
		tfb.fadeIn(200, function() {
			...
			var	title = tfb.find('span.rboxtitle').text(),
				content = tfb.find('div.rboxcontent').text();
			//use decode and escape methods to display UTF8 properly in the title and content from remarksbox.tpl
			title = decodeURIComponent(customEscape(title.trim()));
			tfb.find('span.rboxtitle').text(title);
			content = decodeURIComponent(customEscape(content.trim()));
			tfb.find('div.rboxcontent').text(content);
			//place tikifeedback div into window view if necessary
			//it's important to not define wintop and compare positions until after the tfb object has been manipulated
			var wintop = $(window).scrollTop();
			if (tfb.offset().top < wintop) {
				tfb.offset({'top': wintop});
...
/**
 * Encodes a string. Mimics the native JavaScript escape function which is deprecated
 * When possible, encodeURI() or encodeURIComponent() should be used instead. This function is used when those functions
 * do not produce the same result as escape and that result is necessary
 *
 * @param str			String to be escaped
 * @returns {string}
 */
function customEscape(str) {
	return str.toString().replace(/[^\w @\*\-\+\.\/]/g, function(char) {
		return escapeHash(char);
	});
}

/**
 * Decodes a string. Mimics the native JavaScript unescape function which is deprecated
 * When possible, decodeURI() or decodeURIComponent() should be used instead. This function is used when those functions
 * do not produce the same result as unescape and that result is necessary
 *
 * @param str			String to be decoded
 * @returns {string}
 */
function customUnEscape(str) {
	return str.toString().replace(/%(u[\da-f]{4}|[\da-f]{2})/gi, function(seq) {
		return escapeHash(seq);
	});
}

/**
 * Encode/decode single characters. Utility used in customEscape() and customUnescape() functions
 *
 * @param char				Character being encoded/decoded
 * @returns {*}
 */
function escapeHash(char) {
	var ret = escapeHash[char];
	if(!ret) {
		if(char.length - 1) {
...
/**
 * Remove ajax tikifeedback div placed at the window top once the user starts scrolling and place empty div back in
 * the standard position (at the top of div#col1).
 * Use the throttle function to limit firing to once every 2 seconds
 */
$(window).scroll(throttle(function() {
	var tfb = $('#tikifeedback') || false,
		coltop = $('div#col1').offset().top;
	if (tfb) {
		var divtop = tfb.offset().top;
		//make sure the div is not in the standard position and that the window has moved before removing
		if (divtop !== coltop && divtop !== $(window).scrollTop()) {
			tfb.fadeOut();
			tfb.empty();
			tfb.fadeIn();
			//move empty div back to the standard position
			tfb.offset({'top': coltop});
			tfb.css('z-index', '');
		}
	}
...
/**
 * Utility to limit firing of a function to every specified number of milliseconds
 *
 * @param fn				The function that should be run at each interval
 * @param threshhold		Minimum interval in milliseconds before function will run again; default is 250
 * @param scope				Element the function is bound to; default is this
 * @returns {Function}
 */
