...
//						}
		...
		case "colorpicker":
			if ($.fn.ColorPicker) {
				jqueryTiki.colorPickerListItem = null;
				jqueryTiki.colorPickerPreviews = null;
//				switch (type) {
//					default:	// more modes one day?
				opts = {
					// tiki options
					colorSelector: '',		// jq selector for colour swatch
					colorLabel: '',			// jq selector for visible label
					colorInput: '',			// jq selector for form input
					onAfterChange: null,	// can be function for further processing

					color: '#888',
					onBeforeShow: function () {
						jqueryTiki.colorPickerListItem = this;	 // store for later (maybe there's a better way?)

						var c = $.trim($(colorLabel, this).text());
						...
						if ($("#livePreview:checked").length) {
							...
							//$(this).attr("colorPickerType", m[0]);
							jqueryTiki.colorPickerPreviews = [];
							var zall = $(":visible");
							for (var t in m) {
								for (var i = 0; i < zall.length; i++) {
									var $el = $(zall[i]);
									if ($el.length && $el.css(m[t]) && $.Color($el.css(m[t])).toHEX() == c) {
										//$el.attr("colorPickerType", m[t]);
										jqueryTiki.colorPickerPreviews.push( [ m[t], $el[0] ] );
									}
								}
							}
						} else {
					...
					onChange: function (hsb, hex, rgb) {
						if (colorSelector) {
							$(colorSelector, jqueryTiki.colorPickerListItem).css('backgroundColor', '#' + hex);
						}
						if (colorLabel) {
							$(colorLabel, jqueryTiki.colorPickerListItem).text('#' + hex);
						}
						if (colorInput) {
							$(colorInput, jqueryTiki.colorPickerListItem).val('#' + hex);
						}
						$(jqueryTiki.colorPickerListItem).addClass('changed');
						
						if ($("#livePreview:checked").length) {
							var type = $(jqueryTiki.colorPickerListItem).attr("colorPickerType");
							$(jqueryTiki.colorPickerPreviews).each(function () {
...
//				}
				...
				var colorSelector = opts.colorSelector;
				var colorLabel = opts.colorLabel;
				var colorInput = opts.colorInput;
