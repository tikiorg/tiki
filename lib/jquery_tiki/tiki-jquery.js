...
(function($) {
				...
				$('.comment-form a', this).click(function () {
					$(this).parent().empty().removeClass('button').load($(this).attr('href'), function () {
						$('form', this).submit(function () {
							...
							$.post($.service('comment'), $(this).serialize(), function (data, st) {
								if (data.threadId) {
	...
	$.service = function (controller, action) {
		if (action) {
			return 'tiki-' + controller + '-' + action;
		} else {
			return 'tiki-' + controller;
		}
	};
	...
	$.fn.loadService =  function (data, options) {
		var $dialog = this;
		$dialog.load($.service(data.controller, data.action), data, function () {
			$dialog.find('form').submit(function (e) {
				var form = this;
				$.ajax($.service(data.controller, data.action), {
					type: 'POST',
					dataType: 'json',
					data: $(form).serialize(),
