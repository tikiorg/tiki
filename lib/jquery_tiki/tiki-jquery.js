...
//			widgets: ['zebra'],							// stripes (coming soon)
	...
	if (jqueryTiki.sheet) {
		...
		$jq.sheet.saveSheet = function( index, redirect ) {
...
//			getElementById(area_name).focus(); // unsuccesfull attempt to get Fx3.5/win to keep selection info
		...
		$jq(this).dialog("close");
		$jq('div.plugin input[name="type"][value="' + type + '"]').parent().parent().remove();
...
function maintain_trackerfilter_state( myId, otherId ) {
	
	$jq("#trackerFilter" + myId + " form").submit( function () {
		var oform = $jq("#trackerFilter" + otherId + " form").serialize();
		$jq(this).append("<input type='hidden' name='other_filters[]' value='" + $jq(this).serialize() + "' />")
				 .append("<input type='hidden' name='other_filters[]' value='" + oform + "' />");
		return true;
	});
}
	...
	switch (func) {
		...
		case "sheet":
			if (jqueryTiki.sheet) {
				...
				opts = $jq.extend({urlBaseCss: 	"lib/jquery/jquery.sheet/jquery.sheet.css",
		 		...
		 		return this.each(function() {
					var sh;
		 			if (jqueryTiki.ui) {
						if ($jq(this).attr('style') && $jq(this).attr('style').toLowerCase().indexOf('height') > -1) {
							$jq(this).height($jq(this).find("table:first").height() + 6); //a little extra padding?
						} else {
							$jq(this).height($jq(this).height() + 0);
						}
		...
		case "s5":
			if (jqueryTiki.jqs5) {
				$jq(this).s5($jq.extend({
					menu: function() {
						return (
							'<a href="#" onclick="go(\'first\'); return false;" title="First"><img src="lib/jquery/jquery.s5/images/resultset_first.png" alt="First" /></a> ' + 
							'<a href="#" onclick="go(\'prev\'); return false;" title="Prev"><img src="lib/jquery/jquery.s5/images/resultset_previous.png" alt="Prev" /></a> ' + 
							'<a href="#" onclick="go(\'next\'); return false;" title="Next"><img src="lib/jquery/jquery.s5/images/resultset_next.png" alt="Next" /></a> ' + 
							'<a href="#" onclick="go(\'last\'); return false;" title="Last"><img src="lib/jquery/jquery.s5/images/resultset_last.png" alt="Last" /></a> ' +
							'<a href="#" onclick="return false;" title="Regular View"><img src="lib/jquery/jquery.s5/images/application_view_list.png " alt="Regular View" /></a> ' +
							'<a href="#" onclick="return false;" title="Jump To Slide"><img src="lib/jquery/jquery.s5/images/layers.png" alt="Jump To Slide" /></a> ' +
							'<a href="#" onclick="autoPlay(true); return false;" title="Play"><img src="lib/jquery/jquery.s5/images/control_play_blue.png" alt="Play" /></a> ' +
							'<a href="#" onclick="jQuery.s5.settings.pause = true; return false;" title="Pause"><img src="lib/jquery/jquery.s5/images/control_pause_blue.png" alt="Pause" /></a> ' +
							'<a href="#" onclick="jQuery.s5.settings.pause = true; return false;" title="Stop"><img src="lib/jquery/jquery.s5/images/control_stop_blue.png" alt="Stop" /></a> ' +
							'<a href="#" onclick="getNote(); return false;" title="Notes"><img src="lib/jquery/jquery.s5/images/note.png" alt="Notes" /></a> '
						);
					},
