...
(function($) {
	...
	$.fn.object_selector = function (filter, threshold, valueFunction) {
		var input = this;
		if (valueFunction == undefined) {
			valueFunction = function (value) {
				return value.object_type + ':' + value.object_id;
			};
		}
		this.each(function () {
			...
			$.getJSON('tiki-searchindex.php', {
				...
				$.each(data, function (key, value) {
					$select.append($('<option/>').attr('value', valueFunction(value)).text(value.title));
				});
