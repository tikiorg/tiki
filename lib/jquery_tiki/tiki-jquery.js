...
$(document).ajaxComplete(function (e, jqxhr) {
	var feedback = decodeURIComponent(jqxhr.getResponseHeader('X-Tiki-Feedback')), // decodeURIComponent reverses rawurlencode().
		tikiFeedback = $('#tikifeedback'),
		coltop = $('div#col1').offset().top;
	if (feedback) {
		tikiFeedback.fadeIn(200, function() {
			//place html from ajax X-Tiki-Feedback into the div#tikifeedback
			tikiFeedback.html($($.parseHTML(feedback)).filter('#tikifeedback').html());
			var	title = tikiFeedback.find('span.rboxtitle').text(),
				content = tikiFeedback.find('div.rboxcontent').text();
			
			title = title.trim();
			tikiFeedback.find('span.rboxtitle').text(title);
			
			content = content.trim();
			tikiFeedback.find('div.rboxcontent').text(content);
			
			//place tikifeedback div into window view if necessary
			//it's important to not define wintop and compare positions until after the tikiFeedback object has been manipulated
			var wintop = $(window).scrollTop();
			if (tikiFeedback.offset().top < wintop) {
				tikiFeedback.offset({'top': wintop});
				tikiFeedback.css('z-index', 3000);
			}
		});
	}
	tikiFeedback.find('.clear').on('click', function () {
		$(tikiFeedback).empty();
		tikiFeedback.offset({'top': coltop});
		tikiFeedback.css('z-index', '');
		return true;
	});
});
 ...
 */
// Is this not superfluous? Chealer 2017-06-20
$(window).scroll(throttle(function() {
	var tikiFeedback = $('#tikifeedback') || false,
		coltop = $('div#col1').offset().top;
	if (tikiFeedback) {
