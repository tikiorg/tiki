...
function toggleFullScreen(area_name) {
	...
	if (fullScreenState[area_name]) {	// leave full screen
		...
		for (var i = 0; i < fullScreenState[area_name]["changed"].length; i++) {
			...
			$el.css("margin", fullScreenState[area_name]["changed"][i]["margin"])
				.css("padding", fullScreenState[area_name]["changed"][i]["padding"])
		...
		$ta.parents().each(function() {
			...
			var ob = [];
			ob["el"] = this;
			ob["margin"] = $jq(this).css("margin");
			ob["padding"] = $jq(this).css("padding");
