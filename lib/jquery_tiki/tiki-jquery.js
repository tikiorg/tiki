...
function toggleFullScreen(area_id) {
	...
	if (textareaEditor) {
		...
		if ($ta.hasClass('fullscreen')) {
			...
			var fullscreenParent = $('div.' + area_id);
			var parent = $('#' + fullscreenParent.attr('parentId'));
			
			parent
				.prepend($ta)
				.prepend(toolbar);
			var code = textareaEditor.getCode();
			removeCodeMirrorEditorRelation($ta);
			$('#codemirrorContainer' + area_id).remove();
			...
			var codeMirrorContainer = $('<div id="codemirrorContainer' + area_id + '" class="' + area_id + ' CodeMirror-fullscreen" parentId="' + $ta.parent().attr('id') + '"  />')
				.appendTo('form')
				.append(toolbar)
				.append($ta)
			...
			var newEditor = CodeMirror.fromTextArea($ta[0], {
				height: ($(window).height() - (toolbar.height() + 5)) + 'px',
