...
function convertOverlib(element, tip, params) {	// process modified overlib event fn to cluetip from {popup} smarty func
	...
	if (element.tipWidth) {
		...
		$jq(el).css('position', 'absolute').css('left', '9999px').css('visibility', 'hidden').css('display', 'none');
		$jq(document.body).append(el);
...
//			widgets: ['zebra'],							// stripes (coming soon)
	...
	if (jqueryTiki.sheet) {
		...
		$jq.sheet.saveSheet = function( redirect ) {
			...
			s = "s=" + $jq.toJSON(s).replace(/\+/g,"%2B");	// convert to JSON and replace +'s with 0x2B hex value
