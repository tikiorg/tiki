...
$.fn.modal = function(msg, s) {
	var obj = $(this);
	var hasModal = obj.attr('hasModal');
	
	s = $.extend({
		isEverthing: false
	}, s);
	
	var modal = obj.find('#boxes');
	...
	if (!msg) {
		...
		obj.removeAttr('modals');
		return;
	}
	
	obj.attr('hasModal', 'true');
	...
	} else { //if the modal is just for a single element
		size = {
	...
	dialog
		.append(spinner)
		.append(msg)
		.css('top',  (size.middle - (dialog.height() / 2)) + 'px')
		.css('left', (size.center - (dialog.width() / 2)) + 'px')
