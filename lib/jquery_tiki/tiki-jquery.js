	...
	if (textareaParent.hasClass('TextArea-fullscreen')) {
		...
		win.resize(function() {
			if (win.data('cm-resize') && screen) {
				...
				var innerHeight = win.height() - actions.height();

				if (diff.length) {
					swidth = (screen.width() / 2) + "px";
					innerHeight -= msg.height() -msg.css("paddingTop").replace("px", "") - msg.css("paddingBottom").replace("px", "");
					msg.css("width", (screen.width() / 2 - msg.css("paddingLeft").replace("px", "") - msg.css("paddingRight").replace("px", "")) + "px");
					diff.css({
						width: swidth,
						height: (win.height() - actions.height()) + 'px'
					});
				...
				textarea.css('height', (innerHeight - toolbar.height()) + "px");
			}
		});
