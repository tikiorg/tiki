...
$.fn.rangy = function(fn) {
	...
	$.rangySupported(function() {
		me.mouseup(function(e) {
			...
			if (text.length > 3) {
				var serial = rangy.serializeSelection(selection, false, me[0]);
...
$.fn.rangyRestore = function(serial, fn) {
	var me = $(this);
	$.rangySupported(function() {
		rangy.deserializeSelection(serial, me[0]);
		rangy.createCssClassApplier("rangy-selection new").toggleSelection();
		
		var selection = $('.rangy-selection.new')
			.removeClass('new')
			.data('serial', serial);
...
$.fn.rangyRestorePhrase = function(phrase, fn) {
	var me = $(this);
	$.rangySupported(function() {
		rangy.setPhrase(phrase, me[0]);
		rangy.createCssClassApplier("rangy-selection new").toggleSelection();
	...
	me.serviceDialog({
		...
		load: function () {
			...
			drawing.loadDraw({
				...
				drawing.parent().dialog('close');
				document.location = document.location + '';
			});
			
			$('#tiki_draw').submit(function() {
				$.modal(tr('Saving...'));
				$('#tiki_draw').saveDraw();
				return false;
			});
