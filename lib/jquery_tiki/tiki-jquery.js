...
//			widgets: ['zebra'],							// stripes (coming soon)
	...
	if (jqueryTiki.colorbox) {
		...
		// the line above seems to be breaking images with links - assuming that all images contain '&display' in the href seems to restrictive
		$jq("a[rel*='box'][rel*='type=iframe']").colorbox({
			iframe: true,
			width: "95%"
		});
		...
		// get title from the title attribute of the link
		...
		/* shadowbox params compatibility functions */
...
function popupPluginForm(area_name, type, index, pageName, pluginArgs, bodyContent, edit_icon){
    ...
    if (!pluginArgs && !bodyContent) {
		...
		if (sel.substring(0, 1) == "{") {	// whole plugin selected
			var l = type.length;
	...
	var btns = {};
	var closeText = "Close";
	btns[closeText] = function() { $jq(this).dialog("close"); textarea.focus(); };
	btns[(!replaceText) ? "Insert" : "Replace"] = function() {
