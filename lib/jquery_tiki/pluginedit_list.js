...
$(document).ready(function () {
	...
	jqueryTiki.plugins.list = {
		...
		addPlugin: function (plugin, parentPlugin) {
						...
						$("<div>")
							...
							.append(
								...
								gui.argsSelector(pluginName)
									.css({width: "auto", fontWeight: "normal", marginLeft: "2rem"})
									.prepend($("<option>")
										.text(tr("Add"))
										.prop("selected", true)
									)
									.change(function () {
										$li.append(
											gui.buildArg($li.data("name"), $(this).val(), "", plugin)
										);
			...
			for (var argName in plugin.args) {
				if (plugin.args.hasOwnProperty(argName)) {
					...
					$argsDivs.append(this.buildArg(pluginName, argName, value, parentPlugin));
				}
			}
		 ...
		 * @return {*|jQuery}
		 */
		buildArg: function (pluginName, argName, value, parentPlugin) {
