...
$(document).ready(function () {
	...
	jqueryTiki.plugins.list = {
			...
			var findPlugins = function ($element) {
					...
					$element.find(".plugin").each(function () {
						...
						$plugin.find("> .args > div").each(function () {
							var $arg = $(this),
								argName = $arg.find(".arg-name > span").text();
		...
		addPlugin: function (plugin, parentPlugin) {
			...
			var gui = this,
				argsDef = parentPlugin.args ? parentPlugin.args : gui.plugins[pluginName],
				...
				$li = $("<li>")
					...
					.append(
						...
						$("<div>")
							...
							.text(pluginName)
							.append(
								gui.buildDropDown(gui.arrayKeys(argsDef), tr("Add argument"), function () {
									if ($(this).data("value")) {
										$li.find("> .args").append(
		...
		buildArg: function (pluginName, argName, value, parentPlugin, otherArgs) {
			var argDef, gui = this, $input, $moreArgsDropDown = "";
				...
				if (typeof argDef === "object") {	// array or object
					...
					if (argDef && typeof argDef.length === "undefined") {	// object
						...
						var clickFunction = function () {
							var argName = $(this).parents(".arg-name").parent().find(".form-control").val(),
								parentArgs = gui.plugins[pluginName][aName][argName][$(this).data("value")],
								$plug = gui.addPlugin({
									name: $(this).data("value"),
									args: {},
									plugins: []
						...
						$moreArgsDropDown = gui.buildDropDown(
							gui.arrayKeys(argDef[value]),
							tr("Other Plugins"),
							clickFunction
						);

						$input.change(function () {
							$moreArgsDropDown.empty();
							$moreArgsDropDown.replaceWith(
								gui.buildDropDown(
									gui.arrayKeys(argDef[$input.val()]),
									tr("Other Plugins"),
									clickFunction
								)
							);
						...
						if (argDef.length > 1) {
							...
							$moreArgsDropDown = this.buildDropDown(
								argDef[1],
								tr("Other Arguments"),
								function () {
									$input.parents(".args").append(
										gui.buildArg(pluginName, $(this).data("value"), "", parentPlugin, otherArgs)
									);
								}
							);
						}
