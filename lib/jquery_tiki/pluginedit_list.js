			...
			if (!paramsDef) {
				for (pa in parentPlugin.params) {
					if (parentPlugin.params.hasOwnProperty(pa) && gui.plugins[parentPlugin.name].params[pa].options) {
						if (typeof gui.plugins[parentPlugin.name].params[pa].options[parentPlugin.params[pa]].plugins[pluginName] !== "undefined") {
		...
		buildToolBar: function (plugins, parent, clickFunction) {
			...
			for (var plugin in plugins) {
				if (plugins.hasOwnProperty(plugin) && plugins[plugin].icon &&
					...
					$div.append(
						this.buildDropDown(this.arrayKeys(this.plugins[plugin].params), tr("Add") + " " + plugin, clickFunction, plugins[plugin].icon, plugin)
					);
				}
			}
