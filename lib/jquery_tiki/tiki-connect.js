...
$("#connect_feedback_cbx").click(function(){
	if ($("#connect_feedback_cbx:checked").length > 0) {
		$(".adminoptionbox .tikihelp, .adminoptionbox .icon:not(.connectVoter), .adminoptionbox .pref_dependency").hide();
		$(".connectVoter").show();
	} else {
		$(".adminoptionbox .tikihelp, .adminoptionbox .icon:not(.connectVoter), .adminoptionbox .pref_dependency").show();
		$(".connectVoter").hide();
	}
});

var connectVote = function(pref, vote, el) {
	var spinner = $(el).parent().showBusy();
	if ($(el).data("newVote")) {
