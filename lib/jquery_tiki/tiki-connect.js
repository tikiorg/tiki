...
$("#connect_feedback_cbx").click(function(){
	if ($("#connect_feedback_cbx:checked").length > 0) {
		$(".adminoptionbox .tikihelp, .adminoptionbox .icon:not(.connectVoter)").hide("fast");
		$(".connectVoter").show("fast");
	} else {
		$(".adminoptionbox .tikihelp, .adminoptionbox .icon:not(.connectVoter)").show("fast");
		$(".connectVoter").hide("fast");
	}
});
