...
function initThemeGenDialog() {    // closure for colorpicker code
	...
	if (jqueryTiki.ui) {
		$(function () {
			var $div;
			$(".tgSize", "#themegenerator_container").focus(
				function () {
		...
		var getUnit = function ($el) {
			...
			if (m) {
				...
				return "px";	// default to px if no unit specified
			}
		}
	}
...
function openThemeGenDialog( themename ) {
	if (themename) {
		...
		themename = getCookie("name", "themegen", tr("no theme"));
	}
	...
	var p = { my: "center", at: "center", of: window };

	if (l !== null && t !== null) {
		p = { my: "left top", at: "left+" + parseInt(l, 10) + " top+"+ parseInt(t, 10), of: window };
	}

	var $themegeneratorContainer = $("#themegenerator_container");
	if ($themegeneratorContainer.length === 0) {
		$("body").append($("<div id='themegenerator_container'><form action='tiki-admin.php?page=look&cookietab=6' method='post'></form></div>"));
	}

	var $form = $('form', $themegeneratorContainer);

	$.post("tiki-admin.php?page=look", {tg_open_dialog: true, tg_css_file: $("#tg_css_file", "#themegenerator_container").val()}, function(data) {
			if (!$form.length) {
				$themegeneratorContainer = $("#themegenerator_container");
				$form = $("form", $themegeneratorContainer);
			}
			$form.html(data);
			initThemeGenDialog();
			ajaxLoadingHide();
		...
		.dialog({
			...
			buttons: {
				...
				Preview: function () {
					...
					$form.append($("<input type='hidden' name='tg_preview' value='dialog' />"))
						.submit();
					//$(this).dialog('close');
				}
			},
