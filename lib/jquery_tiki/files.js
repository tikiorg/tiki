...
(function ($) {
			...
			$(window).queue('process-upload', function () {
				reader.onloadend = function (e) {
					...
					if (window.FormData) {
						...
						//checking if image is uploaded to call resize function
					    if(file.type.substring(0,6) == 'image/') {
							if(image_max_size_x=='')
							  image_max_size_x=1024;
							if(image_max_size_y=='')
							  image_max_size_y=768;
							  
                            sendData.data.append('data', resizeFile(file,image_max_size_x,image_max_size_y),file.name);
						}
						else
    ...
    function resizeFile(file,image_x,image_y) {
                    ...
                    img.src = imageData;
                        ...
                        if (img.width > image_x || img.height >image_y) {
                            ...
                            imageData = dataURItoBlob(canvas.toDataURL('image/jpeg'));
