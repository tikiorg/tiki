...
Contact author at: hermanV@subdimension.com
*/
...
class graph {
  ...
  var $parameter         =   array(        // input parameters
    ...
    'title_size'         =>  18,           // title text point size
    ...
    'axis_font'          => 'ARIALBD.TTF', // axis text font. don't forget to set 'path_to_fonts' above.
...
function draw_set($order, $set, $offset) {
  ...
  foreach ($this->x_data as $index => $x) {
    ...
    if (($bar!='none') && (string)$thisY != 'none') $this->bar($thisX, $thisY, $bar, $barSize, $colour, $offset, $set);
...
function bar($x, $y, $type, $size, $colour, $offset, $index) {
