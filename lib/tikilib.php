...
class TikiLib {
    ...
    foreach($merge as $img) {
      ...
      if($fp) {
        ...
        if(strlen($data)>0) {
          ...
          if(fcuntion_exists("ImageCreateFromString")&&defined(IMG_PNG)&&(!strstr($type,"gif"))) {
  ...
  function assign_module($name,$title,$position,$order,$cache_time=0,$rows=10)
  {
    $query = "delete from tiki_modules where name='$name'";
    $result = $this->db->query($query);
    if(DB::isError($result)) $this->sql_error($query,$result);
    $query = "insert into tiki_modules(name,title,position,ord,cache_time,rows) values('$name','$title','$position',$order,$cache_time,$rows)";
    $result = $this->db->query($query);
    if(DB::isError($result)) $this->sql_error($query,$result);
    return true;
  }
  ...
  {
    $query = "select name,title,position,ord,cache_time,rows from tiki_modules where position='$position' order by ord asc";
    $result = $this->db->query($query);
    if(DB::isError($result)) $this->sql_error($query,$result);
    $ret = Array();
    while($res = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
  ...
  function insert_image($galleryId,$name,$description,$filename, $filetype, $data, $size, $xsize, $ysize, $user,$t_data,$t_type) 
  {
    ...
    $t_data = addslashes($t_data);
    $now = date("U");
    $query = "insert into tiki_images(galleryId,name,description,filename,filetype,filesize,data,xsize,ysize,user,created,t_data,t_type)
                          values($galleryId,'$name','$description','$filename','$filetype',$size,'$data',$xsize,$ysize,'$user',$now,'$t_data','$t_type')";
    $result = $this->db->query($query);
    if(DB::isError($result)) $this->sql_error($query,$result);
  ...
  function replace_gallery($name, $description, $theme, $user,$maxRows,$rowImages,$thumbSizeX,$thumbSizeY,$public) 
  {
    ...
    if($result->numRows()) {
      ...
      // Create a new record
      $query =  "insert into tiki_galleries(name,description,theme,created,user,lastModif,maxRows,rowImages,thumbSizeX,thumbSizeY,public) 
                                    values ('$name','$description','$theme',$now,'$user',$now,$maxRows,$rowImages,$thumbSizeX,$thumbSizeY,'$public')";
      $result = $this->db->query($query);
      if(DB::isError($result)) $this->sql_error($query,$result);
    }
    return true;
  ...
  function find_pages($words='') 
  {
    if(!$words) {
      $query='select * from tiki_pages order by hits desc';
    } else {
      $vwords = split(' ',$words);
      $parts = Array();
      ...
      $query='select * from tiki_pages where '.$part.' order by hits desc'; 
    }
    $result = $this->db->query($query);
    if(DB::isError($result)) $this->sql_error($query,$result);
    if(!$result->numRows()) return false;
  ...
  function update_page($pageName,$edit_data,$edit_comment, $edit_user, $edit_ip) 
  {
    ...
    $result = $this->db->query($query);
    if(DB::isError($result)) $this->sql_error($query,$result);
    // If we have too many versions then
