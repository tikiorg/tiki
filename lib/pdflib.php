    ...
    function _parseHTML(&$html)
	{
	   ...
	   //replacing temp table with sorted content
			for($i=0;$i<count($sortedContent);$i++)
			{
				...
				$html=cleanContent($html,array(array("input","tablesorter-filter","class"),array("select","tablesorter-filter","class"),array("select","pvtRenderer","class"),array("select","pvtAggregator","class"),array("td","pvtCols","class"),array("td","pvtUnused","class"),array("td","pvtRows","class"),array("div","plot-container","class"),array("div","js-plotly-plot","class"),array("td","pvtVals","class")));
	...
	function sortContent(&$table,&$tempValue,&$sortedContent,$tag)
	{
	   ...
	   $tid= $table->getAttribute("id");
		   if(file_exists("temp/#".$tid."_".session_id().".txt"))
           {
			   ...
			   $tableTag.=">";
               //checking if pivottable, need to include css in case of table renderers
			   if(strpos($tid,"pivottable") && !strpos($content,"plot-container"))
			   {
				   $cssfile="<style>".file_get_contents("vendor/nicolaskruchten/pivottable/dist/pivot.css")."</style>";
				   }
			   $content=$tableTag.$cssfile.$content.'</'.$tag.'>';
