<?php
// $Header: /cvsroot/tikiwiki/lib/html2pdf/box.table.php,v 1.1.1.1 2006-02-06 15:38:18 nikchankov Exp $
...
//     }
  ...
  function normalize_rhc() {
    ...
    for ($i=0; $i < count($this->content); $i++) {
      ...
      for ($j=0; $j < count($row->content); $j++) {
        ...
        $cell->put_height_constraint(new HCConstraint(null, null, null));
      };
    };
  }
  ...
  function normalize() {
    ...
    for ($i=0; $i<count($this->content); $i++) {
      ...
      for ($j=0; $j<count($row->content); $j++) {
        ...
        $row->content[$j]->rowspan = min($row->content[$j]->rowspan,
                                         count($this->content) - $i);

        if ($row->content[$j]->rowspan > 1) {

          // Note that min($i + $row->content[$j]->rowspan, count($this->content)) is 
          // required, as we cannot be sure that table actually contains the number
          // of rows used in rowspan 
          //
          for ($k=$i+1; $k<min($i + $row->content[$j]->rowspan, count($this->content)); $k++) {

            // Note that if rowspanned cell have a colspan, we should insert SEVERAL fake cells!
            //
            for ($cs = 0; $cs < $row->content[$j]->colspan; $cs++) {
              $this->content[$k]->add_fake_cell_before($j);
            };
          }
        }
      }
  ...
  function _row_heights($minheight) {
    ...
    for ($i=0; $i<count($this->content); $i++) {
      ...
      //
      $hc = $this->get_rhc($i);
      $cheights[] = $hc->apply($heights[$i], $this->content[$i]);
    };
...
//     };
  ...
  function to_ps(&$psdata) {
    $psdata->write("box-table-create\n");

    $psdata->write("dup [");
    for ($i=0; $i<$this->cols_count(); $i++) {
      $cwc = $this->get_cwc($i);
      $psdata->write($cwc->to_ps()." ");
    };
    $psdata->write("] box-table-put-cwc-list\n");

    $psdata->write("dup [");
    for ($i=0; $i<$this->rows_count(); $i++) {
      $rhc = $this->get_rhc($i);
      $psdata->write($rhc->to_ps()." ");
    };
    $psdata->write("] box-table-put-rhc-list\n");
