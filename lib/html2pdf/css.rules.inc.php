<?php
// $Header: /cvsroot/tikiwiki/lib/html2pdf/css.rules.inc.php,v 1.1.1.1 2006-02-06 15:38:27 nikchankov Exp $
...
class CSSRule {
  ...
  function apply(&$root) {
    return apply_css_rule_obj($this->body, $this->baseurl, null, $root);
  }
  ...
  function CSSRule($rule) {
    ...
    foreach (array_keys($this->body) as $key) {
      ...
      if ($handler) { 
        ...
        global $g_baseurl;
        $old_baseurl = $g_baseurl;
        $g_baseurl = $this->baseurl;
        $this->body[$key] = $handler->parse($value); 
