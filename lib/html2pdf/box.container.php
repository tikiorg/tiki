<?php
// $Header: /cvsroot/tikiwiki/lib/html2pdf/box.container.php,v 1.1.1.1 2006-02-06 15:39:32 nikchankov Exp $
...
class GenericContainerBox extends GenericBox {
   ...
   */
  function create_content(&$root) {
    // Initialize content
    $child = $root->first_child();
    while ($child) {
      $box_child =& create_pdf_box($child);
  ...
  function reflow_anchors(&$viewport, &$anchors) {
    GenericBox::reflow_anchors($viewport, $anchors);
    for ($i=0; $i<count($this->content); $i++) {
