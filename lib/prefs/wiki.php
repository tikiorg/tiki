...
function prefs_wiki_list($partial = false)
{
	...
	return array(
		...
		'wiki_cache' => array(
			...
			'warning' => tra("Wiki cache reduces server load but can cause some empty pages and other issues when using wiki plugins. Use only if you reall need it, and it's perfaps better to use Individual wiki cache on only the pages that require it."),
			'type' => 'list',
			'options' => array(
				0 => tra('no cache'),
				60 => '1 ' . tra('minute'),
