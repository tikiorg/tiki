...
$.sheet = $.extend({
	saveSheet: function( tikiSheet, fn ) {
		...
		window.showFeedback(tr("Saving"), 10000);
		
		$.post(url, {s: $.toJSON(sheetData.sheets), t: $.toJSON(sheetData.trackers)}, function(data) {
			jS.setDirty(false);
			if (fn) {
				if($.isFunction(fn)) {
					fn();
	...
	exportSheet: function(jS) {	// diverged from jQuery.sheet 1.1 / Tiki 6
		...
		$(sheetClone).each(function() {
			if (!$(this).hasClass('readonly')) {
				...
				var trackerId = $(this).attr('trackerId');
				if ($(this).attr('trackerId')) {
