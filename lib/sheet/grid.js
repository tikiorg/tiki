...
$.sheet = $.extend({
	...
	saveSheet: function( fn ) {
		...
		$.post(url, {
			s: $.toJSON(sheetData.sheets),
			t: $.toJSON(sheetData.trackers),
			o: $.toJSON(sheetData.order) //order not yet working
		}, function(data) {
			jS.setDirty(false);
			if (fn) {
				if($.isFunction(fn)) {
	...
	deleteSheet: function() {
		var jS = jST.getSheet();
		var trackerId = jS.obj.sheet().data('trackerid');
		var sheetId = jS.obj.sheet().data('sheetid');
					...
					switch (type) {
						case "spreadsheet":
							$.get("tiki-view_sheets.php", {
								...
								$.sheet.manageState(jST, true, "edit");
							});
							break;
						case "file":
							break;
