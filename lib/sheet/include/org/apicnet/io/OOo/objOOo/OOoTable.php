...
class OOoTable extends absOOo {
	...
	function colMaxNextSpan($argArray, $curentCol){
		$max    = $curentCol*1000000;	// un nombre tr�s grand
	...
	/**
	 * OOoTable::colSpan(), m�thode de fusion de colonnes. Le num�ro des colonnes commence � 0.
	 * 
	 * @param $argSpan = array(
	 			"lignSPan"	=> 1,
	 			"colDeb"	=> 1,
	...
	/**
	 * OOoTable::rowSpan(), m�thode de fusion de lignes. Le num�ro des lignes commence � 0.
	 * 
	 * @param $argSpan = array(
	 			"lignDeb"	=> 1,
				"colDeb"	=> 1,
    ...
    /**
     * table::addcellData(), m�thode d'ajout de donn�es dans une cellule.
     * 
     * @param  $argData  = array(
	 			"data"			=> DATA
				"img"			=> array(
	...
	function addcellData($column, $row, $argData){
		if (is_array($argData)) {
			...
			$this -> ErrorTracker(4, "argData doit �tre un tableau", 'colSpan', __FILE__, __LINE__);
		}
    }
	...
	function run(&$nodeContent, &$nodeStyle, $dir){
		...
		for ($i = 0; $i < $this->_row; $i++ ){
			...
			for($j = 0; $j < $this->_column; $j++){
				...
				/**********************Cr�ation du style de la colonne*********************************/
				...
				/*********************Fin de la Cr�ation du style de la colonne**************************/
			   
				/*****************************Cr�ation du style de la cellule************************/
