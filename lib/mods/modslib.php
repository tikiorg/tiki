	...
	function _decodedeps(&$array, $str)
	{
		...
		foreach ($am as $dep) {
			...
			foreach ($tests[0] as $test) {
				$moddep->tests[] = ['test' => preg_replace('/[0-9.]*$/', '', $test),
...
class TikiModInfo extends TikiModAvailable
{
	...
	{
		if (! is_file($mods_path . '/Installed/' . $this->type . '-' . $this->name . '.conf.txt')) {
			return false;
		}
...
class ModsLib
{
	...
	{
		if ($path and $path != '/' and ! is_dir(dirname($path))) {
			$this->prepare_dir(dirname($path));
		}
		if (! is_dir($path) and substr($path, 0, 1) != '.') {
			mkdir($path, 02777);
		}
	}
		...
		if ($fp) {
			while (! feof($fp)) {
				...
				if (trim($line) and substr(trim($line), 0, 1) == "'") {
					...
					if (count($str) < 4) {
						continue; // line must have at least 4 elements
					}
					if (empty($str[0]) || empty($str[1]) || empty($str[2])) {
						continue; // theses field must be not empty
					}
	...
	function get_depend_available($moddep, $list, $check_revision = false)
	{
		if (isset($list[$moddep->type]) && isset($list[$moddep->type][$moddep->name])) {
			...
			if (! $check_revision || $moddep->isitin($mod)) {
				return $mod;
			}
		}
		return null;
	...
	function find_deps($modspath, $mods_server, $modnames)
	{
		...
		$repos = ['installed' => $this->read_list($modspath . "/Installed/00_list.txt", 'installed'),
		...
		foreach ($deps['toinstall'] as $k => $toinstall) {
			if (isset($repos['installed'][$toinstall->type])
			   && isset($repos['installed'][$toinstall->type][$toinstall->name])) {
				$meat = ['from' => $repos['installed'][$toinstall->type][$toinstall->name],
						'to' => $toinstall];
...
*/
	...
	function _find_deps_remove($repos, $modtoremove, &$deps)
	{
		...
		foreach ($repos['installed'] as $meat) {
			foreach ($meat as $mod) {
				if (isset($deps['toremove'][$mod->modname])) {
					continue;
				}
				if (! is_array($mod->requires)) {
					continue;
				}
	...
	function install($path, $mod, $from = null, $upgrade = false)
	{
		...
		if ($upgrade and is_array($info->sql_upgrade) and count($info->sql_upgrade)) {
			uksort($info->sql_upgrade, [$this, 'revision_compare']);
			global $tikilib;
