...
jQuery.fn.extend({
	flexibleSyntaxHighlighter: function(s) {
		...
		jQuery(this).each(function() {
			jQuery(this).hide(); //we have to hide all the textareas or pre objects to know how much space we can take up
		});
				...
				if (!modeDef.mime) {
					...
					if (modeLoc.js) {
						jQuery.getScript(modeLoc.js, function(e) {
							...
							o.hide(); //we hide after the load, makes the page jump around much less
							syntaxHighlighter.remove(textarea);
