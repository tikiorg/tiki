...
jQuery.fn.extend({
	flexibleSyntaxHighlighter: function(settings) {
		...
		jQuery(this).each(function() {
			...
			if (!$.isArray(settings.mode)) settings.mode = [settings.mode];
			
			jQuery('style.tiki-codemirror-style').remove();
			var modeStack = 0;
			jQuery(settings.mode).each(function() {
				mode = (this + '');
				var modeDef = settings.modes[mode];
				if (!modeDef) return;
				...
				if (modeLoc.js) {
					jQuery.getScript(modeLoc.js, function(o) {
						modeStack--;
						if (modeStack < 1) {
...
var syntaxHighlighter = {
	ready: function(textarea, settings, modeDef) {
		var changeCount = 0;
		...
		if (!settings.readOnly) {
			...
			'</div>').insertAfter(textarea.next()).click(function(){
				...
				for(mode in settings.modes) {
					options += '<input class="cm-mode" type="checkbox" value="' + mode + '" ' + (settings.mode == mode ? 'checked="true"' : '') + '/>' + mode + '<br />';
				}
