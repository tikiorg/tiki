...
var syntaxHighlighter = {
	...
	ready: function(textarea, settings, modeDef, currentMode) {
		...
		if (settings.readOnly) {
			$(editor.getWrapperElement()).addClass('codelisting');
		}
		
		if (settings.height) {
			$(editor.getScrollerElement()).parent().height(settings.height);
		}
		
		if (settings.width) {
			$(editor.getScrollerElement()).parent().width(settings.width);
		}
