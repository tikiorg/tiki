	...
	if (dialogName == 'link') {
		...
		dialogDefinition.onOk = function() {	// switch the link type to normal url, no protocol and class wiki
			...
			if (this.getContentElement('info','linkType').getValue() == "tikilink") {
				...
				if (this._.selectedElement && this._.selectedElement.$ && this._.selectedElement.$.nodeName === "A") {
					this._.selectedElement.$.textContent = this.getContentElement('info','tikilinkLabel').getValue();
				}
			}
		...
		dialogDefinition.onShow = function() {	// check for class=wiki and fill tikilink form if so
			...
			if (this.getContentElement('advanced','advCSSClasses').getValue() === 'wiki') {
				...
				this.getContentElement('info','tikilinkLabel').setValue(this._.selectedElement.$.textContent);
			} else {
				...
				if (selection.getType() === CKEDITOR.SELECTION_TEXT) {
					if (r[0].startContainer.$ === r[0].endContainer.$) {
						var seltext = r[0].startContainer.$.nodeValue.substring(r[0].startOffset, r[0].endOffset);
