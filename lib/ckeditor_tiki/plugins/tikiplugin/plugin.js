...
//							rng = rng[0];
//						}
//						//rng[0].setStartAt(element);
//						var el = sel.getStartElement();
//						if (typeof el !== 'undefined' && el && el.$) {
//							while (el.$.nextSibling && el.$ !== rng.endContainer.$) { // loop through selection if multiple elements
//								el = new CKEDITOR.dom.element(el.$.nextSibling);
//								if ($(el.$).hasClass("tiki_plugin")) { // found it
...
//						element.children[ i ].attributes.onclick = function ( evt ) {
//							alert("qwe");
//							var element = asplugin.getPluginParent( evt.data.element );
//							if (element) {
//								evt.cancel();
//								evt.stop();
//								evt.data.dialog = null;
