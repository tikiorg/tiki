...
$(document).ready(function() {
		...
		$("#page_list_container li").each(function () {
			...
			$(this).draggable({
				...
				stop:function (event, ui) {
					...
					function (event, ui) {	// sortable gets muddled when dragging so disable it
	...
	$(".save_structure").click(function(){
		...
		$.post($.service("wiki", "save_structure"), {data: $.toJSON(ary)}, function (data) {
			$sortable.modal();
			if (data) {
				$sortable.parent().html(data.html);
				setupStructure();
				$(".save_structure").hide();
				tocDirty = false;
			}
