...
class CalRecurrence extends TikiLib {
	...
	public function createEvents() {
		...
		$dates = array();
		$start = date('Y/m/d',$this->getStartPeriod());
		$start = explode("/",$start);
		if ($this->getNbRecurrences() > 0) {
			$nbRec = $this->getNbRecurrences();
			if ($this->isWeekly()) {
				$startWeekday = date('w',$this->getStartPeriod()); // 0->Sunday, 6->Saturday
				$firstEventDate = $this->getWeekday() - $startWeekday;
				if ($firstEventDate < 0)
					$firstEventDate += 7;
				for ($i=0 ; $i < $nbRec ; $i++) {
					$dates[] = date('U',mktime(0,0,0,$start[1],$start[2] + $firstEventDate + ($i * 7),$start[0]));
				}
				...
				for ($i=0 ; $occurrences < $nbRec ; $i++) {
					$nbDaysInMonth = date('t',mktime(0,0,0,$startMonth + $i,1,$start[0]));
					if ($this->getDayOfMonth() > $nbDaysInMonth)
						continue;
					$dates[] = date('U',mktime(0,0,0,$startMonth + $i,$this->getDayOfMonth(),$start[0]));
					$occurrences++;
				}
			} elseif ($this->isYearly()) {
				$yymm = date('md',$this->getStartPeriod());
				...
				for ($i=0 ; $occurrences < $nbRec ; $i++) {
					if ($isLeapDay) {
						if (date('L',mktime(0,0,0,1,1,$start[0] + $offset + $i)) == 0)
							continue;
					}
					$dates[] = date('U',mktime(0,0,0,substr($dt,0,2),substr($dt,-2),$start[0] + $offset + $i));
					$occurrences++;
				}
			} else {
			...
			if ($this->isWeekly()) {
				...
				if ($firstEventDate < 0)
					$firstEventDate += 7;
				$currDate = date('U',mktime(0,0,0,$start[1],$start[2] + $firstEventDate,$start[0]));
				...
				$firstIsNextMonth = ($this->getDayOfMonth() < $start[2]);
				$startMonth = $firstIsNextMonth ? $start[1] + 1 : $start[1];
				$currDate = date('U',mktime(0,0,0,$startMonth,$this->getDayOfMonth(),$start[0]));
				$i = 0;
				while ($currDate < $this->getEndPeriod()) {
					$nbDaysInMonth = date('t',mktime(0,0,0,$startMonth + $i,1,$start[0]));
					if ($this->getDayOfMonth() > $nbDaysInMonth) {
						$i++;
						$currDate = date('U',mktime(0,0,0,substr($dt,0,2),substr($dt,-2),$start[0] + $offset + $i));
						continue;
					}
					$dates[] = $currDate;
					$i++;
					$currDate = date('U',mktime(0,0,0,$startMonth + $i,$this->getDayOfMonth(),$start[0]));
				}
			} elseif ($this->isYearly()) {
				$yymm = date('md',$this->getStartPeriod());
				$isLeapDay = ($this->getDateOfYear() == 229); // Feb 29th case.
				$offset = ($this->getDateOfYear() < $yymm) ? 1 : 0;
				$dt = str_pad($this->getDateOfYear(),4,"0",STR_PAD_LEFT);
				$currDate = date('U',mktime(0,0,0,substr($dt,0,2),substr($dt,-2),$start[0] + $offset));
				$i = 0;
				while ($currDate < $this->getEndPeriod()) {
					if ($isLeapDay) {
						if (date('L',mktime(0,0,0,1,1,$start[0] + $offset + $i)) == 0) {
			...
			foreach ($theEventsToBeChanged as $anEvtId) {
				...
				foreach($changedFields as $aField) {
					if (substr($aField,0,1) != "_")
						...
						$anEvtStart = date('Y/m/d',$anEvt['start']);
						...
						if ($aField == "_weekday") {
							...
							if (is_null($advanced)) {
								if ($this->getWeekday() < $oldRec->getWeekday()) {
									if (date('w',mktime()) <= $this->getWeekday()) {
										$offsetInSeconds = ($this->getWeekday() - date('w',mktime())) * dayInSeconds;
										$couldBeDay = date('U',mktime($newStartHour,$newStartMin,0,date('m'),date('d'),date('Y'))) + $offsetInSeconds;
							...
							if (!is_null($advanced)) {
								$daysOffsetEvent = $this->getWeekday() - date('w',mktime(0,0,0,$anEvtStart[1],$anEvtStart[2],$anEvtStart[0]));
								$tmp[] = "start='" . date('U',mktime($newStartHour,$newStartMin,0,$anEvtStart[1],$anEvtStart[2] + $daysOffsetEvent,$anEvtStart[0])) . "'";
								$tmp[] = "end='" . date('U',mktime($newEndHour,$newEndMin,0,$anEvtStart[1],$anEvtStart[2] + $daysOffsetEvent,$anEvtStart[0])) . "'";
							}
						}
						// - for monthly events :
						...
						elseif ($aField == "_dayOfMonth") {
							...
							if (is_null($advanced)) {
								if ($this->getDayOfMonth() < $oldRec->getDayOfMonth()) {
									if (date('d',mktime()) <= $this->getDayOfMonth()) {
										$offsetInSeconds = ($this->getDayOfMonth() - date('d',mktime())) * dayInSeconds;
										$couldBeDay = date('U',mktime(0,0,0,date('m',mktime()),date('d',mktime()),date('Y',mktime()))) + $offsetInSeconds;
										if ($couldBeDay >= $this->getStartPeriod()) {
											$advanced = true;
										}
									}
							...
							if ($advanced) {
								// on est sur le meme mois
								$tmp[] = "start='" . date('U',mktime($newStartHour, $newStartMin, 0, $anEvtStart[1], $this->getDayOfMonth(), $anEvtStart[0])) . "'";
								$tmp[] = "end='" . date('U',mktime($newEndHour, $newEndMin, 0, $anEvtStart[1], $this->getDayOfMonth(), $anEvtStart[0])) . "'";
							} else {
								// si le nouveau jour est après l'ancien, on est sur le meme mois; sinon, on est sur le mois suivant.
								$offsetMonth = 0;
								if ($this->getDayOfMonth() < ($oldRec->getDayOfMonth()))
									$offsetMonth = 1;
								$tmp[] = "start='" . date('U',mktime($newStartHour, $newStartMin, 0, $anEvtStart[1] + $offsetMonth, $this->getDayOfMonth(), $anEvtStart[0])) . "'";
								$tmp[] = "end='" . date('U',mktime($newEndHour, $newEndMin, 0, $anEvtStart[1] + $offsetMonth, $this->getDayOfMonth(), $anEvtStart[0])) . "'";
							}
						}
						// - for yearly events :
						...
						elseif ($aField == "_dateOfYear") {
							...
							if (is_null($advanced)) {
								...
								if ($this->getDateOfYear() < $oldRec->getDateOfYear()) {
									if (time() <= date('U',mktime($newStartHour,$newStartMin,0,substr($thisdate,0,2),substr($thisdate,-2),date('Y')))) {
										$offsetInSeconds = date('U',mktime($newStartHour,$newStartMin,0,substr($thisdate,0,2),substr($thisdate,-2),date('Y'))) - $this->getStartPeriod();
							...
							if ($advanced) {
								$tmp[] = "start='" . date('U',mktime($newStartHour, $newStartMin, 0,substr($thisdate,0,2),substr($thisdate,-2), $anEvtStart[0])) . "'";
								$tmp[] = "end='" . date('U',mktime($newEndHour, $newEndMin, 0, substr($thisdate,0,2),substr($thisdate,-2), $anEvtStart[0])) . "'";
							} else {
								$offsetYear = 0;
								if ($this->getDateOfYear() < $oldRec->getDateOfYear())
									$offsetYear = 1;
								$tmp[] = "start='" . date('U',mktime($newStartHour, $newStartMin, 0, substr($thisdate,0,2),substr($thisdate,-2), $anEvtStart[0] + $offsetYear)) . "'";
								$tmp[] = "end='" . date('U',mktime($newEndHour, $newEndMin, 0, substr($thisdate,0,2),substr($thisdate,-2), $anEvtStart[0] + $offsetYear)) . "'";
							}
				...
				if (in_array("_start",$changedFields) && $doWeChangeTimeIfNeeded) {
					...
					$newStartHour = floor($this->getStart()/100);
					$newStartMin = $this->getStart() - 100*$newStartHour;
					$tmp[] = "start='" . date('U',mktime($newStartHour, $newStartMin, 0, $anEvtStart[1], $anEvtStart[2], $anEvtStart[0])) . "'";
				}
				if (in_array("_end",$changedFields) && $doWeChangeTimeIfNeeded) {
					...
					$newEndHour = floor($this->getEnd()/100);
					$newEndMin = $this->getEnd() - 100*$newEndHour;
					$tmp[] = "end='" . date('U',mktime($newEndHour, $newEndMin, 0, $anEvtStart[1], $anEvtStart[2], $anEvtStart[0])) . "'";
				}
				if (count($tmp) > 0) {
	...
	public function compareFieldsOfEvent($evt,$oldRec) {
		...
		if ($evt['calendarId'] != $oldRec->getCalendarId())
			$result[] = "calendarId";
		if (date('Hi',$evt['start']) != $oldRec->getStart())
			$result[] = "start";
		// checking the end is double check : is it the right hour ? is it the same day ?
		if ((date('Hi',$evt['end']) != $oldRec->getEnd()) || (date('Ymd',$evt['start']) != date('Ymd',$evt['end'])))
			$result[] = "end";
		...
		if ($evt['description'] != $oldRec->getDescription())
			$result[] = "description";
		if (date('Hi',$evt['start']) != str_pad($oldRec->getStart(),4,"0",STR_PAD_LEFT))
			$result[] = "_start";
		if (date('Hi',$evt['end']) != str_pad($oldRec->getEnd(),4,"0",STR_PAD_LEFT))
			$result[] = "_end";
		if ($oldRec->isWeekly()) {
			if (date('w',$evt['start']) != $oldRec->getWeekday())
				$result[] = "_weekday";
		} elseif ($oldRec->isMonthly()) {
			if (date('d',$evt['start']) != $oldRec->getDayOfMonth())
				$result[] = "_dayOfMonth";
		} elseif ($oldRec->isYearly()) {
			if (date('md',$evt['start']) != $oldRec->getDateOfYear())
				$result[] = "_dateOfYear";
		}
		return $result;
	}
