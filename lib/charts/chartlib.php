...
class ChartLib extends TikiLib {
	...
	function generate_new_ranking($chartId) {
		...
		if ($info['lastChart'] + $info['frequency'] < $now) {
			...
			while ($res = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
				...
				if ($maxPeriod) {
					$lastPosition
						= $this->getOne("select position from tiki_charts_rankings where itemId=$itemId and chartId=$chartId and period=$maxPeriod");
				} else {
					$lastPosition = 0;
				}
	...
	function max_dif($chartId, $period) {
		return
			$this->getOne("select max(lastPosition-position) from tiki_charts_rankings where chartId=$chartId and period=$period");
	}
	...
	function get_chart_item($itemId) {
		...
		if ($period) {
			...
			// Last position
			$res['lastPosition']
				= $this->getOne("select lastPosition from tiki_charts_rankings where itemId=$itemId and period=$period");
			// Best position
			$res['best'] = $this->getOne("select min(position) from tiki_charts_rankings where itemId=$itemId");
			$res['bestdate']
				= $this->getOne("select timestamp from tiki_charts_rankings where itemId=$itemId and position=" . $res['best']);
