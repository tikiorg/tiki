...
class WikiParser {
	...
	*/
	...
	function performAction($thisSS, $yytext, $yyleng, $yylineno, $yy, $yystate, $SS, $S) {
		$O = count($S) - 1;
		$r = 0;
		$thisS = 0;
	switch (yystate) {
	...
	case 3:$thisS = $S[$O];
	break;
	case 4:$thisS = ($S[$O-1] ? $S[$O-1] : '') . ($S[$O] ? $S[$O] : '');
	break;
	case 5:$thisS = ($S[$O-2] ? $S[$O-2] : '') . ($S[$O-1] ? $S[$O-1] : '') . ($S[$O] ? $S[$O] : '');
	break;
	case 6:$thisS = plugin($S[$O]);
	break;
	case 7:
			$S[$O]->body = $S[$O-1];
			$thisS = plugin($S[$O]);
	...
	case 9:$thisS = $S[$O];
	break;
	case 10:$thisS = $S[$O-1] . $S[$O];
	break;
	case 11:$thisS = $S[$O-1] . $S[$O];
	break;
	case 12:$thisS = $S[$O];
	break;
	case 13:$thisS = isHtmlPermissible($S[$O]);
	break;
	case 14:$thisS = $S[$O];
	break;
	case 15:$thisS = $S[$O];
	...
	case 16:$thisS = $S[$O];
	break;
	case 17:$thisS = "<b>" . $S[$O-1] . "</b>";
	break;
	case 18:$thisS = "<div style='border: solid 1px black;'>" . $S[$O-1] . "</div>";
	break;
	case 19:$thisS = "<center>" . $S[$O-1] . "</center>";
	break;
	case 20:
			$text = $S[$O-1]->split(':');
			$thisS = "<span style='color: #" . $text[0] . ";'>" . $text[1] . "</span>";
		
	break;
	case 21:$thisS = "<i>" . $S[$O-1] . "</i>";
	break;
	case 22:$thisS = "<h6>" . $S[$O-1] . "</h6>";
	break;
	case 23:$thisS = "<h5>" . $S[$O-1] . "</h5>";
	break;
	case 24:$thisS = "<h4>" . $S[$O-1] . "</h4>";
	break;
	case 25:$thisS = "<h3>" . $S[$O-1] . "</h3>";
	break;
	case 26:$thisS = "<h2>" . $S[$O-1] . "</h2>";
	break;
	case 27:$thisS = "<h1>" . $S[$O-1] . "</h1>";
	break;
	case 28:
			$link = $S[$O-1]->split('|');
			$href = $S[$O-1];
			...
			$thisS = "<a href='" . $href . "'>" . $text  . "</a>";
		
	break;
	case 29:$thisS = "<span style='text-decoration: line-through;'>" . $S[$O-1] . "</span>";
	break;
	case 30:
			$tableContents = '';
			$rows = $S[$O-1]->split('<br />');
			...
			$thisS = "<table style='width: 100%;'>" . $tableContents . "</table>";
		
	break;
	case 31:$thisS = "<div class='titlebar'>" . $S[$O-1] . "</div>";
	break;
	case 32:$thisS = "<u>" + $S[$O-1] + "</u>";
	break;
	case 33:
...
class WikiParserLexer {
	...
	function next() {
		...
		for ($i = 0; $i < count($rules); $i++) {
			preg_match($rules[$i], $this->_input, $match);
			...
			if ( isset($match) && isset($match[0]) ) {
				...
				$this->yylloc = (object)array(
					...
					"last_column"=> $lines ? strlen($lines[count($lines) - 1]) - 1 : $this->yylloc->last_column + count($match[0])
				);
				$this->yytext .= $match[0];
				$this->match .= $match[0];
				...
				$token = $this->performAction($this->S, $this->yy, $this->yyleng, $rules[$i], $this->conditionStack[count($this->conditionStack) - 1]);
	...
	function performAction($S, $yy, $yy_, $avoiding_name_collisions, $YY_START) {
...
print_r($wikiParser->parse("__bold__"));