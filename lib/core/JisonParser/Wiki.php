...
case 4:
		...
		$thisS = JisonParser_Wiki_Handler::join($S[$O-1], $S[$O]);
	
break;
case 5:
		
		$thisS = JisonParser_Wiki_Handler::join($S[$O-2], $S[$O-1], $S[$O]);
	
break;
case 6:
		
		$thisS = JisonParser_Wiki_Handler::plugin($S[$O]);
...
break;
case 11:
		$thisS = JisonParser_Wiki_Handler::html($S[$O]);
...
break;
case 14:
		$thisS = JisonParser_Wiki_Handler::bold($S[$O-1]);
		
	
break;
case 15:
		$thisS = JisonParser_Wiki_Handler::box($S[$O-1]);
	
break;
case 16:
		$thisS = JisonParser_Wiki_Handler::center($S[$O-1]);
	
break;
case 17:
		$thisS = JisonParser_Wiki_Handler::colortext($S[$O-1]);
	
break;
case 18:
		$thisS = JisonParser_Wiki_Handler::italics($S[$O-1]);
	
break;
case 19:
		$thisS = JisonParser_Wiki_Handler::header6($S[$O-1]);
	
break;
case 20:
		$thisS = JisonParser_Wiki_Handler::header5($S[$O-1]);
	
break;
case 21:
		$thisS = JisonParser_Wiki_Handler::header4($S[$O-1]);
	
break;
case 22:
		$thisS = JisonParser_Wiki_Handler::header3($S[$O-1]);
	
break;
case 23:
		$thisS = JisonParser_Wiki_Handler::header2($S[$O-1]);
	
break;
case 24:
		$thisS = JisonParser_Wiki_Handler::header1($S[$O-1]);
	
break;
case 25:
		$thisS = JisonParser_Wiki_Handler::link($S[$O-1]);
	
break;
case 26:$thisS = $S[$O-1];
break;
case 27:
		$thisS = JisonParser_Wiki_Handler::strikethrough($S[$O-1]);
	
break;
case 28:
		$thisS = JisonParser_Wiki_Handler::tableParser($S[$O-1]);
	
break;
case 29:
		$thisS = JisonParser_Wiki_Handler::titlebar($S[$O-1]);
	
break;
case 30:
		$thisS = JisonParser_Wiki_Handler::underscore($S[$O-1]);
	
break;
case 31:
		$thisS = JisonParser_Wiki_Handler::wikilink($S[$O-1]);
...
case 0:
		...
		$yy_->yytext = JisonParser_Wiki_Handler::inlinePlugin($yy_->yytext);
...
case 1:
		...
		$yy->pluginStack = JisonParser_Wiki_Handler::stackPlugin($yy_->yytext, $yy->pluginStack);
		
		if (JisonParser_Wiki_Handler::size($yy->pluginStack) == 1) {
			return 'PLUGIN_START';
		} else {
			return 'CONTENT';
		}
...
case 2:
		...
		if ($yy->pluginStack) {
			if (
				JisonParser_Wiki_Handler::size($yy->pluginStack) > 0 &&
				JisonParser_Wiki_Handler::substring($yy_->yytext, 1, -1) == $yy->pluginStack[JisonParser_Wiki_Handler::size($yy->pluginStack) - 1]->name
			) {
				if (JisonParser_Wiki_Handler::size($yy->pluginStack) == 1) {
					$yy_->yytext = $yy->pluginStack[JisonParser_Wiki_Handler::size($yy->pluginStack) - 1];
					$yy->pluginStack = JisonParser_Wiki_Handler::pop($yy->pluginStack);
					return 'PLUGIN_END';
				} else {
					$yy->pluginStack = JisonParser_Wiki_Handler::pop($yy->pluginStack);
					return 'CONTENT';
...
case 3:
		...
		$yy->npStack = JisonParser_Wiki_Handler::push($yy->npStack, true);
		$this->npOn = true;
...
case 4:
		...
		$this->npStack = JisonParser_Wiki_Handler::pop($yy->npStack);
		if (JisonParser_Wiki_Handler::size($yy->npStack) < 1) $yy->npOn = false;
		
		return 'NP_END';
	
break;
case 5:
		$yy_->yytext = JisonParser_Wiki_Handler::hr();
...
case 6:
		...
		$yy_->yytext = JisonParser_Wiki_Handler::substring($yy_->yytext, 2, -2);
		$yy_->yytext = JisonParser_Wiki_Handler::smile($yy_->yytext);
...
case 7:
		...
		$yy_->yytext = JisonParser_Wiki_Handler::substring($yy_->yytext, 2, -1);
...
case 8:
		...
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'BOLD_END');
	
break;
case 9:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'BOLD_START');
	
break;
case 10:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'BOX_END');
	
break;
case 11:
		
		this.begin('box');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'BOX_START');
	
break;
case 12:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'CENTER_END');
	
break;
case 13:
		
		this.begin('center');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'CENTER_START');
	
break;
case 14:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'COLORTEXT_END');
	
break;
case 15:
		
		this.begin('colortext');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'COLORTEXT_START');
	
break;
case 16:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER6_END');
	
break;
case 17:
		
		this.begin('header6');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER6_START');
	
break;
case 18:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER5_END');
	
break;
case 19:
		
		this.begin('header5');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER5_START');
	
break;
case 20:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER4_END');
	
break;
case 21:
		
		JisonParser_Wiki_Handler::begin('header4');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER4_START');
	
break;
case 22:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER3_END');
	
break;
case 23:
		
		this.begin('header3');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER3_START');
	
break;
case 24:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER2_END');
	
break;
case 25:
		
		this.begin('header2');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER2_START');
	
break;
case 26:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER1_END');
	
break;
case 27:
		
		this.begin('header1');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'HEADER1_START');
	
break;
case 28:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'ITALIC_END');
	
break;
case 29:
		
		this.begin('italic');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'ITALIC_START');
	
break;
case 30:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'LINK_END');
	
break;
case 31:
		
		this.begin('link');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'LINK_START');
	
break;
case 32:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'STRIKETHROUGH_END');
	
break;
case 33:
		
		this.begin('strikethrough');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'STRIKETHROUGH_START');
	
break;
case 34:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'TABLE_END');
	
break;
case 35:
		
		this.begin('table');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'TABLE_START');
	
break;
case 36:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'TITLEBAR_END');
	
break;
case 37:
		
		this.begin('titlebar');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'TITLEBAR_START');
	
break;
case 38:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'UNDERSCORE_END');
	
break;
case 39:
		
		this.begin('underscore');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'UNDERSCORE_START');
	
break;
case 40:
		
		JisonParser_Wiki_Handler::popState();
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'WIKILINK_END');
	
break;
case 41:
		
		this.begin('wikilink');
		return JisonParser_Wiki_Handler::npState($this->npOn, 'CONTENT', 'WIKILINK_START');
...
case 44:
		...
		if (JisonParser_Wiki_Handler::npState($this->npOn, false, true) == true) {
			$yy_->yytext = JisonParser_Wiki_Handler::formatContent($yy_->yytext);
		}
