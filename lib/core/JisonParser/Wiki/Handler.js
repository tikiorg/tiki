var WikiHandler = {
	...
	Parse: function(val, errors) {
		//try {
			if (this.inUse) {
				//var wiki = WikiFactory(WikiHandler);
	...
	closeOpenStates: function(content) {
		for(var i = this.lexer.conditionStack.length - 1; i > 0; i--) {
			var fn = this.lexer.conditionStack.pop();
			if (this[fn]) content = this[fn](content);
		}
