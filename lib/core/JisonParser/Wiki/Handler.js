		...
		if (plugin) {
			if (plugin.name) {
				...
				if (window[wikiPluginName]) {
					...
					if (thisOutput) {
						var newOutput = JisonParser_Wiki_Handler.parse(thisOutput);
...
var JisonParser_Wiki_Handler = {
	parse: function(val, errors) {
		try {
			function LexerClone () {}
			LexerClone.prototype = Parser.lexer;

			function ParserClone () {
				this.lexer = new LexerClone();
				this.yy = {};
			}
			ParserClone.prototype = Parser;
			
			var thisParser = (new ParserClone);
			
			return thisParser.parse(val);
		} catch (e) {
			if (errors) {
				return e;
			}
