			...
			build.dependancy(option, {
				start: function() {
					$('<a href="#"> Toggle Checked </a>')
						.toggle(function() {
							$(objs[option.objName]).find('.reportBuilderInput:checkbox').removeAttr('checked');
						}, function() {
							$(objs[option.objName]).find('.reportBuilderInput:checkbox').attr('checked', 'true');
						})
						.appendTo(option.obj);
				},
				values: function() {
					...
					objs[option.objName] = area.find('input');
			...
			if (values) {
				if (option.dependancy) {
					me.bind('reportReady', function() {
						$(objs[prefix + option.dependancy]).change(function() {
							option.obj.html('');
