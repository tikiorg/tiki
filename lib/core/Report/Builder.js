	...
	var importData = function(dataItem, trail, I) {
		for(type in dataItem) {
			if (type == 'value') {
				...
				console.log([input.val(), dataItem[type], trail]);
			} else if (type == 'join') {
				var thisTrail = (trail + "");
				var importTrail = (trail + '_' + (isNaN(type) ? type : '')).split('_').clean().join("_");
				...
				i++;
				if (i < 1000)
				importData(dataItem[type], (trail + '_' + (isNaN(type) ? type : '')).split('_').clean().join("_"), (!isNaN(type) ? type : ''));
			}
		}
	};
