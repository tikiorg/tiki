...
//					'URL'           => 									// URL to files (seems not to be REQUIRED)
		...
		if ($input->defaultGalleryId->int()) {
			...
			$opts['roots'][0]['startPath'] = $this->defaultGalleryId;	// root path just needs the id
			$opts['roots'][0]['accessControlData'] = array('startPath' => $this->defaultGalleryId, 'deepGallerySearch' => $input->deepGallerySearch->int(), 'galleryParentIds' => $galleriesParentIds);
		}
