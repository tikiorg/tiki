...
function wikiplugin_trackerlist_info()
{
	...
	return array(
		...
		'body' => '<br>' . tr('Additional information when using tablesorter:') . '<br>' .
			...
			. '<b>tsfilters</b> - '. tra('When server=y, for best results the date filter should only be applied to date field types.')
			. '<br><br>' . tra('Notice')
		,
		'format' => 'html',
	...
	if ($prefs['feature_trackers'] != 'y' || !isset($trackerId) || !($tracker_info = $trklib->get_tracker($trackerId))) {
		...
		if (count($passfields)) {
			...
			if (isset($col) && $items['cant'] > 1) {
				 ...
				 */
				if (in_array($fieldtype, ['f', 'j']) && $tsServer && isset($tsdatefilter) && $tsdatefilter === true) {
					...
					foreach ($items['data'] as $record) {
						...
						if (count($tsfilter) == 2) {
							...
							if ($dateval >= (int) substr($tsfilter[0], 0, 10)
								&& $dateval <= (int) substr($tsfilter[1], 0, 10))
							{
								$filtered[] = $record;
							}
