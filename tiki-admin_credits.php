...
if( isset( $_REQUEST['use_credit'] ) && $use_credit_userid = $tikilib->get_user_id($_POST['userfilter']) ) {
	$creditslib->useCredits( $use_credit_userid,
...
if( isset( $_REQUEST['restore_credit'] ) && $restore_credit_userid = $tikilib->get_user_id($_POST['userfilter']) ) {
	$creditslib->restoreCredits( $restore_credit_userid,
...
if( isset( $_REQUEST['update_types'] ) ) {
	...
	{
			$creditslib->updateCreditType($values['credit_type'],
				$values['display_text'],
				$values['unit_text'],
	...
	{
		$creditslib->updateCreditType($_POST['new_credit_type'],
				$_POST['display_text'],
				$_POST['unit_text'],
...
if( isset( $_REQUEST['userfilter'] ) )
{
	...
	if( $editing )
	{
		...
		if (isset($_REQUEST['startDate_Year']) || isset($_REQUEST['endDate_Year'])) {
			$smarty->assign( 'startDate', $tikilib->make_time(0, 0, 0, $_REQUEST['startDate_Month'], $_REQUEST['startDate_Day'], $_REQUEST['startDate_Year']) );
			$smarty->assign( 'endDate', $tikilib->make_time(23, 59, 59, $_REQUEST['endDate_Month'], $_REQUEST['endDate_Day'], $_REQUEST['endDate_Year']) );
		...
		if( isset( $_POST['save'], $_POST['credits'] ) ) // {{{
		{
			...
			if( !empty( $_POST['credit_type'] ) && !empty( $_POST['total_amount'] ) && in_array($_POST['credit_type'], array_keys($creditTypes)) )
			{
				$creditslib->addCredits( $editing['userId'], $_POST['credit_type'], $_POST['total_amount'], $_POST['expiration_date'], $_POST['creation_date'] );
			}
