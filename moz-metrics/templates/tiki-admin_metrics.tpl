...
<h1><a class="pagetitle" href="tiki-admin_metrics.php">{tr}Admin Metrics{/tr}</a>

{if $prefs.feature_help eq 'y'}
<a href="{$prefs.helpurl}Metrics+Admin" target="tikihelp" class="tikihelp" title="{tr}Admin Metrics{/tr}">
<img border='0' src='pics/icons/help.png' alt="{tr}Help{/tr}" width="16" height="16" /></a>{/if}

{if $prefs.feature_view_tpl eq 'y'}
<a href="tiki-edit_templates.php?template=tiki-admin_metrics.tpl" target="tikihelp" class="tikihelp" title="{tr}View template{/tr}: {tr}Admin Metrics Template{/tr}">
...
<!--a class="linkbut" href="tiki-admin_metrics.php?clear_cache=1">{tr}Clear Cache{/tr}</a-->
</div>
...
<td class="{cycle}"><a class="link" href="tiki-admin_metrics.php?metric_edit={$i|escape:'url'}#editcreate" title="{tr}Edit{/tr}"><img src="pics/icons/page_edit.png" border="0" width="16" height="16" alt='{tr}Edit{/tr}' /></a>
             ...
             <a class="link" href="tiki-admin_metrics.php?metric_remove={$i|escape:'url'}" title="{tr}Delete{/tr}"><img src="pics/icons/cross.png" border="0"  width="16" height="16" alt='{tr}Delete{/tr}' /></a></td>
</tr>
{/foreach}
{else}
...
<td class="{cycle}"><a class="link" href="tiki-admin_metrics.php?tab_edit={$i|escape:'url'}#editcreatetab" title="{tr}Edit{/tr}"><img src="pics/icons/page_edit.png" border="0" width="16" height="16" alt='{tr}Edit{/tr}' /></a>
             ...
             {if $use_memcache}<a class="link" href="tiki-admin_metrics.php?tab_clearcache={$i|escape:'url'}" title="{tr}Clear cache{/tr}"><img src="pics/icons/delete.png" border="0"  width="16" height="16" alt='{tr}Clear cache{/tr}' /></a>{/if}
</td>
</tr>
{/foreach}
{else}
...
<td class="{cycle}"><a class="link" href="tiki-admin_metrics.php?assign_metric_edit={$i|escape:'url'}#assign" title="{tr}Edit{/tr}"><img src="pics/icons/page_edit.png" border="0" width="16" height="16" alt='{tr}Edit{/tr}' /></a>
             <a class="link" href="tiki-admin_metrics.php?assign_remove={$i|escape:'url'}" title="{tr}Unassign{/tr}"><img src="pics/icons/cross.png" border="0"  width="16" height="16" alt='{tr}Delete{/tr}' /></a></td>
</tr>
{/foreach}
{else}
<tr><td colspan="6" class="odd">
...
<h2 id="assign">{if $assign_metric eq ''}
{tr}Assign metric{/tr}
{else}
...
<table class="normal">
<tr><td class="{cycle advance=false}">{tr}Metric Name{/tr}</td><td class="{cycle}">
...
<tr><td class="{cycle advance=false}">{tr}Tab for metric{/tr}</td><td class="{cycle}">
...
<!--tr><td class="{cycle advance=false}">{tr}Weight in tab{/tr}</td><td class="{cycle}"><input type="text" name="assign_metric_order" value="{$smarty.request.assign_metric_order|escape}" /></td></tr-->
...
<h2 id="editcreate">{if !isset($metric_id)}
...
<div class="rbox-data" name="tip"><ul>
...
<tr><td class="form odd">{tr}Name (must be unique){/tr}</td><td class="odd"><input type="text" name="metric_name" value="{$metric_name|escape}" /></td></tr>
<tr><td class="form even">{tr}Range{/tr}</td><td class="even">
...
<tr><td class="form {cycle advance=false}">{tr}Data type{/tr}</td><td class="{cycle}">
...
<tr><td class="form {cycle advance=false}">{tr}Query{/tr}
</td><td class="{cycle}">

<textarea id="metric_query" name="metric_query" rows="10">{$metric_query|escape}</textarea>

</td></tr>
<tr><td>&nbsp;</td><td><input type="submit" name="metric_submit" value="{if !isset($metric_id)}Create Metric{else}Edit Metric{/if}" /></td></tr>
</table>
<input type="hidden" name="metric_id" value="{$metric_id|escape}">
</form>


<h2 id="editcreatetab">{if !isset($tab_id)}
...
<tr><td class="form {cycle advance=false}">{tr}Name (must be unique){/tr}</td><td class="{cycle}"><input type="text" name="tab_name" value="{$tab_name|escape}" /></td></tr>
<tr><td class="form {cycle advance=false}">{tr}Weight (must be integer){/tr}</td><td class="{cycle}"><input type="text" name="tab_order" value="{$tab_order|escape}" /></td></tr>
<tr><td class="form {cycle advance=false}">{tr}Content{/tr}
</td><td class="{cycle}">

<textarea id="tab_content" name="tab_content" rows="10">{$tab_content|escape}</textarea>

</td></tr>
<tr><td>&nbsp;</td><td><input type="submit" name="tab_submit" value="{if !isset($tab_id)}Create Tab{else}Edit Tab{/if}" /></td></tr>
</table>
