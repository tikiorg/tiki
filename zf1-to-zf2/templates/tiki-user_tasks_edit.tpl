<form action="tiki-user_tasks.php" method="post">
	...
	<table class="formcolor">
		...
		<tr>
			...
			<td colspan="3">
				{if ($receive_users)}
					{if ($info.taskId > 0 and !$admin_mode)}
						<select name="task_user" {if ($info.taskId > 0 and !$admin_mode)} disabled="disabled"{/if}>
					...
					{else}
						<select name="task_user" >
							{section name=user_i loop=$receive_users}
								<option value="{$receive_users[user_i].login|escape}" {if ( $receive_users[user_i].login eq $info.user)} selected="selected" {/if}>
			...
			<tr>
				<td colspan="4">
					<div align="center">
						...
						<input type="submit" class="btn btn-default btn-sm" name="reload" value="{tr}Reload{/tr}">
					</div>
				</td>
			</tr>
		{else}
