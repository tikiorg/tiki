			...
			{if $diff_style eq 'sidediff' || $diff_style eq 'minsidediff'}
				<tr>
					{if $diffdata}
						<td>{$diffdata}</td>
					{else}
						<td>{tr}Versions are identical{/tr}</td>
					{/if}
				</tr>
			{/if}
...
<form action="tiki-map_history.php" method="get">
	...
	<div style="text-align:center;">
		<div class="panel panel-default"><div class="panel-body"><b>{tr}Legend:{/tr}</b> {tr}v=view{/tr} {if $prefs.default_wiki_diff_style eq "old"}, {tr}c=compare{/tr}, {tr}d=diff{/tr}{/if}</div>
	</div>
