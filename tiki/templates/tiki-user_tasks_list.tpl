...
{if $tasklist[task_i].deleted} 
	{if $tasklist[task_i].creator ne $user}
		{html_image file='img/icons2/delete_all.gif' width='16' height='16' border='0' alt='{tr}delete{/tr}'}
	{else}
	...
	{if ($tasklist[task_i].user eq $user) }
		...
		{else}
			{if ($tasklist[task_i].accepted_user eq '')}
				{html_image  file="$img_me_waiting" height="$img_me_waiting_height" width="$img_me_waiting_width" title='{tr}waiting for me{/tr}' border='0' alt='{tr}waiting for me{/tr}'}
			{else}
				...
				{else}
					{html_image file="$img_he_waiting"  height="$img_he_waiting_height" width="$img_he_waiting_width" border='0' alt='{tr}waiting for other user{/tr}' title='{tr}waiting for other user{/tr}'}
				{/if}
			{/if}
		{/if}
