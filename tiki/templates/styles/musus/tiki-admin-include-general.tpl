{* $Header: /cvsroot/tikiwiki/tiki/templates/styles/musus/tiki-admin-include-general.tpl,v 1.2 2004-01-13 19:16:52 musus Exp $ *}
  ...
  <div class="tiki-content">
    <form action="tiki-admin.php?page=general" method="post">
      ...
      <table class="admin"><tr>
        ...
        <td>{tr}Disallow access to the site (except for those with permission){/tr}:</td>
      ...
      <table class="admin"><tr>
        ...
        <td>{tr}Disallow access when load is above the threshold (except for those with permission){/tr}:</td>
        <td><input type="checkbox" name="use_load_threshold" {if $use_load_threshold eq 'y'}checked="checked"{/if}/>
      </td>
      </tr><tr>
        <td>{tr}Max average server load threshold in the last minute{/tr}:</td>
        <td><input type="text" name="load_threshold" value="{$load_threshold}" size="5" /></td>
      </tr><tr>
        <td>{tr}Message to display when server is too busy{/tr}:</td>
      ...
      <table class="admin"><tr>
        ...
        <tr>
        <td>{tr}Open external links in new window{/tr}:</td>
