...
{if count($galleries) > 0}
	...
	<table>
	<tr><td>{tr}File Title{/tr}:</td><td><input type="text" name="name" /></td></tr>
	<tr><td>{tr}File Description{/tr}:</td><td><textarea rows="5" cols="40" name="description"></textarea></td></tr>
	<tr><td>{tr}File Gallery{/tr}:</td><td> 
	...
	<tr><td>	{tr}Upload from disk:{/tr}</td>
	<td>
	...
	<tr><td>{tr}Batch upload{/tr}</td><td>
	...
	{if count($uploads) > 0}
		...
		{section name=ix loop=$uploads}
			<div align="center">
				...
				<div class="wikitext">
					{tr}You can download this file using{/tr}: <a href="{$url_browse}?fileId={$uploads[ix].fileId}">{$url_browse}?fileId={$uploads[ix].fileId}</a><br /><br />
