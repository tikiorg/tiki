...
{if $art_list_size eq 'y'}
	<th><a href="tiki-list_submissions.php?offset={$offset}&amp;sort_mode={if $sort_mode eq 'size_desc'}size_asc{else}size_desc{/if}">{tr}Size{/tr}</a></th>
{/if}
...
{/if}
{if $art_list_size eq 'y'}
	<td class="{cycle advance=false}">{$listpages[changes].size|kbsize}</td>
{/if}
{if $art_list_img eq 'y'}
...
<td class="{cycle}">
	{if $tiki_p_edit_submission eq 'y' or $listpages[changes].author eq $user}
		<a href="tiki-edit_submission.php?subId={$listpages[changes].subId}"><img src='img/icons/edit.gif' border='0' alt='{tr}Edit{/tr}' /></a>
	{/if}
	{if $tiki_p_remove_submission eq 'y'}
		<a href="tiki-list_submissions.php?offset={$offset}&amp;sort_mode={$sort_mode}&amp;remove={$listpages[changes].subId}"><img src='img/icons2/delete.gif' alt='{tr}Remove{/tr}' /></a>
	{/if}
	{if $tiki_p_approve_submission eq 'y'}
		<a href="tiki-list_submissions.php?approve={$listpages[changes].subId}"><img src='img/icons2/post.gif' alt='{tr}Approve{/tr}' /></a>
	{/if}
</td>
</tr>
{sectionelse}
