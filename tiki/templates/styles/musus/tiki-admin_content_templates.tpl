...
<!-- begin -->
<br />
{if $preview eq 'y'}<div class="wikitext">{$parsed}</div>{/if}
{if $templateId > 0}
<h2>{tr}Edit this template:{/tr} {$info.name}</h2>
<a href="tiki-admin_content_templates.php">{tr}Create new template{/tr}</a>
{else}<h2>{tr}Create new template{/tr}</h2>
{/if}
<form action="tiki-admin_content_templates.php" method="post">
<input type="hidden" name="templateId" value="{$templateId|escape}" />
<table>
<tr><td><label>{tr}name{/tr}:</label></td><td><input type="text" maxlength="255" size="40" name="name" value="{$info.name|escape}" /></td></tr>
{if $feature_cms_templates eq 'y'}
<tr><td><label>{tr}use in articles{/tr}:</label></td><td><input type="checkbox" name="section_cms" {if $info.section_cms eq 'y'}checked="checked"{/if} /></td></tr>
{/if}
{if $feature_wiki_templates eq 'y'}
<tr><td><label>{tr}use in wiki{/tr}:</label></td><td><input type="checkbox" name="section_wiki" {if $info.section_wiki eq 'y'}checked="checked"{/if} /></td></tr>
{/if}
{if $feature_newsletters eq 'y'}
<tr><td><label>{tr}use in newsletters{/tr}:</label></td><td><input type="checkbox" name="section_newsletters" {if $info.section_newsletters eq 'y'}checked="checked"{/if} /></td></tr>
{/if}
<tr><td><label>{tr}use in HTML pages{/tr}:</label></td><td><input type="checkbox" name="section_html" {if $info.section_html eq 'y'}checked="checked"{/if} /></td></tr>
<tr><td><label>{tr}template{/tr}:</label></td><td><textarea name="content" rows="25" cols="60">{$info.content|escape}</textarea></td></tr>
