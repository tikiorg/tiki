...
<tr>
<td class="{cycle advance=false}"><a href="backups/{$tikidomain}{$backups[user].filename}" title="{$backups[user].filename}">{$backups[user].filename|truncate:20:"...":true}</a></td>
...
<td class="{cycle}">
   <a href="tiki-backup.php?remove={$backups[user].filename}">{tr}remove{/tr}</a>
   <a href="tiki-backup.php?restore={$backups[user].filename}">{tr}restore{/tr}</a>
</td>
</tr>
{/section}
</table>
...
<br /><br />
<a href="tiki-backup.php?rrestore={$restorefile}">{tr}Click here to confirm restoring{/tr}</a>
</div>
<br />
{/if}
<div class="simplebox">
...
<tr><td>{tr}Upload backup{/tr}:</td><td>
