...
<div class="normal">
<div class="editblogform">{tr}Blog{/tr}</div><div class="editblogform">
<select name="blogId">
...
<div class="editblogform">{tr}Smileys{/tr}</div><div class="editblogform">
   {include file="tiki-smileys.tpl" area_name='blogedit'}
</div>
{/if}
{if $blog_data.use_title eq 'y' || !$blogId}
<div class="editblogform">{tr}Title{/tr}</div>
...
{if $postId > 0}
	<div class="editblogform" style="clear:left">{tr}Upload image for this post{/tr}
	<input type="hidden" name="MAX_FILE_SIZE" value="1000000000">
	<input name="userfile1" type="file">
	</div>
	{if count($post_images) > 0}
		<div class="editblogform">{tr}Images{/tr}</div>
		<div class="editblogform">
		{section name=ix loop=$post_images}
			...
			<a href="tiki-blog_post.php?postId={$postId}&amp;remove_image={$post_images[ix].imgId}"><img border='0' src='img/icons/trash.gif' alt='{tr}Trash{/tr}'/></a>	<textarea rows="2" cols="40">{$post_images[ix].link|escape}</textarea><br />
	...
	{/if}
{/if}
<div class="editblogform">{tr}Mark entry as private:{/tr} <input type="checkbox" name="blogpriv" {if $blogpriv eq 'y'}checked="checked"{/if} /></div>
<div class="editblogform">{tr}Send trackback pings to:{/tr}<small>{tr}(comma separated list of URIs){/tr}</small></div>
<div class="editblogform">
...
<div class="editblogform">{tr}Spellcheck{/tr}: <input type="checkbox" name="spellcheck" {if $spellcheck eq 'y'}checked="checked"{/if} /></div>
{/if}
{if $feature_freetags eq 'y'}
  {include file=freetag.tpl}
{/if}
<div class="editblogform">&nbsp;</div><div class="editblogform"><input type="submit" class="wikiaction" name="save" value="{tr}save{/tr}" />
