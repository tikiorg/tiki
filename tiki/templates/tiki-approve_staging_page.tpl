...
<table>
<tr>
<td class="odd">{$staging_info.lastModif|tiki_short_datetime}</td>
{if $tiki_p_wiki_view_author ne 'n'}<td class="odd">{$staging_info.user}</td>{/if}
{if $prefs.feature_wiki_history_ip ne 'n'}<td class="odd">{$staging_info.ip}</td>{/if}
<td class="odd">{if $staging_info.comment}{$staging_info.comment}{else}&nbsp;{/if}</td>
<td class="odd button">{$staging_info.version}<br />{tr}current{/tr}</td>
<td class="odd button">&nbsp;<a class="link" href="tiki-pagehistory.php?page={$staging_page|escape:"url"}&amp;preview={$staging_info.version}" title="{tr}View{/tr}" target="_blank">v</a>
