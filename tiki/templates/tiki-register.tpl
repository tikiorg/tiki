...
<tr><td class="formcolor">{tr}Password{/tr}:</td><td class="formcolor"><input id='pass1' type="password" name="pass" {if $feature_ajax eq'y'}onKeyUp="check_pass()"{/if}/></td></tr>

<tr><td class="formcolor">{tr}Repeat password{/tr}:</td><td class="formcolor"><input style="float:left" id='pass2' type="password" name="passAgain" {if $feature_ajax eq'y'}onKeyUp="check_pass()"{/if}/>{if $feature_ajax eq'y'}<div style="float:left" id="checkpass"></div>{/if}</td></tr>
