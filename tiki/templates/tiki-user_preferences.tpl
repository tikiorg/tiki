...
<tr id="1"><td class="heading" colspan="2">{tr}Personal Information{/tr}</td></tr>
  ...
  <tr><td class="{cycle advance=false}">{tr}Real Name{/tr}:</td><td class="{cycle}">{if $prefs.auth_ldap_nameattr eq '' || $prefs.auth_method ne 'auth'}<input type="text" name="realName" value="{$user_prefs.realName|escape}" />{else}{$user_prefs.realName}{/if}</td></tr>
...
<select name="country">
  ...
  <tr><td class="{cycle advance=false}">{tr}Longitude (WGS84/decimal degrees){/tr}:</td><td class="{cycle}"><input type="text" name="lon" value="{$user_prefs.lon|escape}" />
  ...
  <tr><td class="{cycle advance=false}">{tr}Latitude (WGS84/decimal degrees){/tr}:</td><td class="{cycle}"><input type="text" name="lat" value="{$user_prefs.lat|escape}" /></td></tr>
  {/if}

  <tr><td class="{cycle advance=false}">{tr}Avatar{/tr}:</td><td class="{cycle}">{$avatar} <a href="tiki-pick_avatar.php{if $userwatch ne $user}?view_user={$userwatch}{/if}" class="link">{tr}Pick user Avatar{/tr}</a></td></tr>
  <tr><td class="{cycle advance=false}">{tr}URL:{/tr}</td><td class="{cycle}"><input type="text" size="40" name="homePage" value="{$user_prefs.homePage|escape}" /></td></tr>
  {if $prefs.feature_wiki eq 'y' and $prefs.feature_wiki_userpage eq 'y'}
  <tr><td class="{cycle advance=false}">{tr}Your personal Wiki Page{/tr}:</td><td class="{cycle}">{if $userPageExists eq 'y'}<a class="link" href="tiki-index.php?page={$prefs.feature_wiki_userpage_prefix}{$userinfo.login}" title="{tr}View{/tr}">{$prefs.feature_wiki_userpage_prefix}{$userinfo.login}</a> 
...
{else}{$prefs.feature_wiki_userpage_prefix}{$userinfo.login} (<a class="link" href="tiki-editpage.php?page={$prefs.feature_wiki_userpage_prefix}{$userinfo.login}">{tr}Create{/tr}</a>){/if}</td></tr>
  ...
  {if $customfields[ir].show}
    <tr><td class="{cycle advance=false}">{$customfields[ir].label}:</td>
        <td class="{cycle}"><input type="{$customfields[ir].type}" name="{$customfields[ir].prefName}" value="{$customfields[ir].value}" size="{$customfields[ir].size}" /></td>
    </tr>
  {/if}
  {/section}
...
{/if}
  ...
  <tr><td class="{cycle advance=false}">{tr}Does your mail reader need a special charset{/tr}</td>
  <td class="{cycle}">
  ...
  <tr><td class="{cycle advance=false}">{tr}Theme{/tr}:</td><td class="{cycle}"><select name="mystyle">
  ...
  <tr><td  class="{cycle advance=false}">{tr}Language{/tr}:</td><td class="{cycle}">
  ...
  {/if}
  <tr><td class="{cycle advance=false}">{tr}Number of visited pages to remember{/tr}:</td><td class="{cycle}">
  ...
  <tr><td class="{cycle advance=false}">{tr}Use double-click to edit pages{/tr}:</td>
  <td class="{cycle}">
  <input type="checkbox" name="user_dbl" {if $user_prefs.user_dbl eq 'y'}checked="checked"{/if} />
  </td>
  </tr>
  {* not used {if $prefs.feature_history eq 'y'}
  <tr><td class="{cycle advance=false}">Use new diff any version interface:</td>
  <td class="{cycle}">
  ...
  <tr><td class="{cycle advance=false}">{tr}Show user's info on mouseover{/tr}:</td>
  <td class="{cycle}">
...
<tr>
  ...
  <td class="{cycle}"><input type="checkbox" name="mess_sendReadStatus" {if $user_prefs.mess_sendReadStatus eq 'y'}checked="checked"{/if}/></td>
</tr>
<tr>
...
{if $prefs.feature_wiki eq 'y'}
<tr><td class="{cycle advance=false}">{tr}My pages{/tr}</td><td class="{cycle}"><input type="checkbox" name="mytiki_pages" {if $user_prefs.mytiki_pages eq 'y'}checked="checked"{/if} /></td></tr>
{/if}

{if $prefs.feature_blogs eq 'y'}
<tr><td class="{cycle advance=false}">{tr}My blogs{/tr}</td><td class="{cycle}"><input type="checkbox" name="mytiki_blogs" {if $user_prefs.mytiki_blogs eq 'y'}checked="checked"{/if} /></td></tr>
{/if}

{if $prefs.feature_galleries eq 'y'}
<tr><td class="{cycle advance=false}">{tr}My galleries{/tr}</td><td class="{cycle}"><input type="checkbox" name="mytiki_gals" {if $user_prefs.mytiki_gals eq 'y'}checked="checked"{/if} /></td></tr>
{/if}

{if $prefs.feature_messages eq 'y'and $tiki_p_messages eq 'y'}
<tr><td class="{cycle advance=false}">{tr}My messages{/tr}</td><td class="{cycle}"><input type="checkbox" name="mytiki_msgs" {if $user_prefs.mytiki_msgs eq 'y'}checked="checked"{/if} /></td></tr>
{/if}

{if $prefs.feature_tasks eq 'y' and $tiki_p_tasks eq 'y'}
<tr><td class="{cycle advance=false}">{tr}My tasks{/tr}</td><td class="{cycle}"><input type="checkbox" name="mytiki_tasks" {if $user_prefs.mytiki_tasks eq 'y'}checked="checked"{/if} /></td></tr>
{/if}

{if $prefs.feature_trackers eq 'y'}
<tr><td class="{cycle advance=false}">{tr}My items{/tr}</td><td class="{cycle}"><input type="checkbox" name="mytiki_items" {if $user_prefs.mytiki_items eq 'y'}checked="checked"{/if} /></td></tr>
{/if}

{if $prefs.feature_workflow eq 'y'}
  {if $tiki_p_use_workflow eq 'y'}
    <tr><td class="{cycle advance=false}">{tr}My workflow{/tr}</td><td class="{cycle}"><input type="checkbox" name="mytiki_workflow" {if $user_prefs.mytiki_workflow eq 'y'}checked="checked"{/if} /></td></tr>
  {/if}
{/if}
