{* $Header: /cvsroot/tikiwiki/tiki/templates/modules/mod-login_box.tpl,v 1.65 2007-10-30 21:33:35 rlpowell Exp $ *}
    ...
    {if $user}
      ...
      {if $tiki_p_admin eq 'y'}
        <form action="{if $prefs.https_login eq 'encouraged' || $prefs.https_login eq 'required' || $prefs.https_login eq 'force_nocheck'}{$base_url_https}{/if}{$prefs.login_url}" method="post"{if $prefs.desactive_login_autocomplete eq 'y'} autocomplete="off"{/if}>
         ...
         {/if}
          <p style="text-align: center"><input type="submit" value="{tr}Switch{/tr}"</input></p>
         </fieldset>
        </form>
      {/if}
	  ...
	  <script type="text/javascript">document.getElementById('login-user').focus();</script>
		...
		<script type="text/javascript">document.getElementById('login-user').focus();</script>
          ...
          {if $prefs.rememberme ne 'disabled'}
            ...
            {/if}
          {/if}
          <p style="text-align: center"><input type="submit" value="{tr}Login{/tr}" /></p>
       </fieldset>
