<a href="tiki-admin.php?page=features" class="link"><img border='0'
   src='img/icons/admin_features.png' alt='{tr}Features{/tr}' /></a>
<a href="tiki-admin.php?page=general" class="link"><img border='0'
   src='img/icons/admin_general.png' alt='{tr}General{/tr}' /></a>
<a href="tiki-admin.php?page=login" class="link"><img border='0'
   src='img/icons/admin_login.png' alt='{tr}Login{/tr}' /></a>
<a href="tiki-admin.php?page=wiki" class="link"><img border='0'
   src='img/icons/admin_wiki.png' alt='{tr}Wiki{/tr}' /></a>
<a href="tiki-admin.php?page=gal" class="link"><img border='0'
   src='img/icons/admin_imagegal.png' alt='{tr}Image Galleries{/tr}' /></a>
<a href="tiki-admin.php?page=fgal" class="link"><img border='0'
   src='img/icons/admin_filegal.png' alt='{tr}File Galleries{/tr}' /></a>
<a href="tiki-admin.php?page=blogs" class="link"><img border='0'
   src='img/icons/admin_blogs.png' alt='{tr}Blogs{/tr}' /></a>
<a href="tiki-admin.php?page=forums" class="link"><img border='0'
   src='img/icons/admin_forums.png' alt='{tr}Forums{/tr}' /></a>
<a href="tiki-admin.php?page=polls" class="link"><img border='0'
   src='img/icons/admin_polls.png' alt='{tr}Polls{/tr}' /></a>
<a href="tiki-admin.php?page=rss" class="link"><img border='0'
   src='img/icons/admin_rss.png' alt='{tr}RSS{/tr}' /></a>
<a href="tiki-admin.php?page=cms" class="link"><img border='0'
   src='img/icons/admin_articles.png' alt='{tr}Articles{/tr}' /></a>
<a href="tiki-admin.php?page=faqs" class="link"><img border='0'
   src='img/icons/admin_faqs.png' alt='{tr}FAQs{/tr}' /></a>
<a href="tiki-admin.php?page=trackers" class="link"><img border='0'
   src='img/icons/admin_trackers.png' alt='{tr}Trackers{/tr}' /></a>
<a href="tiki-admin.php?page=webmail" class="link"><img border='0'
   src='img/icons/admin_webmail.png' alt='{tr}Webmail{/tr}' /></a>
<a href="tiki-admin.php?page=directory" class="link"><img border='0'
   src='img/icons/admin_directory.png' alt='{tr}Directory{/tr}' /></a>
<a href="tiki-admin.php?page=userfiles" class="link"><img border='0'
   src='img/icons/admin_userfiles.png' alt='{tr}User files{/tr}' /></a>
<a href="tiki-admin.php?page=maps" class="link"><img border='0'
   src='img/icons/admin_maps.png' alt='{tr}Maps{/tr}' /></a>