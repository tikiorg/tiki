...
<form enctype="multipart/form-data" method="post" action="tiki-quiz_edit.php?quizId={$quiz.quizId}">
	<table class="normal">
		...
		<tr class="formcolor">
			<td>{tr}After-test Options{/tr}</td>
			<td {if $cols} colspan="{$cols}"{/if}>
				[ <a class="link" href="javascript:show('after-test');">{tr}show{/tr}</a>
 				...
 				<div id="after-test" style="display:none;">
					<table class="normal">
						<tr>
							<td class="formcolor"><input type="checkbox" name="immediateFeedback" id="immediate-feedback" {if $immediateFeedback eq 'y'}checked="checked"{/if} /><label for="immediate-feedback">{tr}Immediate feedback{/tr}</td>
							<td class="formcolor"><input type="checkbox" name="showAnswers" id="show-answers" {if $showAnswers eq 'y'}checked="checked"{/if} /><label for="show-answers">{tr}Show correct answers{/tr}</td>
						</tr>
						<tr>
  ...
  <td class="formcolor"><input type="checkbox" name="shareStats" id="share-stats" {if $shareStats eq 'y'}checked="checked"{/if} /><label for="share-results">{tr}Share stats{/tr}</td>
					...
					<table class="normal">
						...
						<tr>
							<td colspan="2" class="formcolor"><input type="checkbox" name="canRepeat" id="quiz-repeat" {if $canRepeat eq 'y'}checked="checked"{/if} /><label for="quiz-repeat">{tr}Allow students to retake this quiz {/tr}
    ...
    <tr>
      ...
      <td class="formcolor">
        <textarea class="wikiedit" name="input_data" rows="25" cols="80" id='subheading' wrap="virtual" ></textarea>
      </td>
    </tr>
		<tr class="formcolor">
      <td class="formcolor">
