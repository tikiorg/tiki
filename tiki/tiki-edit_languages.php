...
// $Header: /cvsroot/tikiwiki/tiki/tiki-edit_languages.php,v 1.5 2003-10-08 03:53:08 dheltzel Exp $
		...
		for ($i = $tr_recnum; $i <= $tr_recnum + $maxRecords; $i++) {
			...
			if (isset($_REQUEST["edit_rec_$i"])) {
				if (strlen($_REQUEST["edit_rec_tran_$i"]) > 0 && strlen($_REQUEST["edit_rec_source_$i"]) > 0) {
					$_REQUEST["edit_rec_source_$i"]
						= addslashes(htmlentities(strip_tags($_REQUEST["edit_rec_source_$i"]), ENT_NOQUOTES));

					$_REQUEST["edit_rec_tran_$i"]
						= addslashes(htmlentities(strip_tags($_REQUEST["edit_rec_tran_$i"]), ENT_NOQUOTES));
				...
				if (strlen($_REQUEST["edit_edt_tran_$i"]) > 0 && strlen($_REQUEST["edit_edt_source_$i"]) > 0) {
					$_REQUEST["edit_edt_tran_$i"]
						= addslashes(htmlentities(strip_tags($_REQUEST["edit_edt_tran_$i"]), ENT_NOQUOTES));

					$_REQUEST["edit_edt_source_$i"]
						= addslashes(htmlentities(strip_tags($_REQUEST["edit_edt_source_$i"]), ENT_NOQUOTES));
				...
				if (strlen($_REQUEST["edit_edt_source_$i"]) > 0) {
					$_REQUEST["edit_edt_source_$i"]
						= addslashes(htmlentities(strip_tags($_REQUEST["edit_etd_source_$i"]), ENT_NOQUOTES));
