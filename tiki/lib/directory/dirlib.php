...
class DirLib extends TikiLib {
	...
	function dir_get_all_categories($offset, $maxRecords, $sort_mode, $find, $siteId = 0) {
		...
		while ($res = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
			...
			if ($siteId) {
				$belongs = $this->db->getOne(
					"select count(*) from tiki_category_sites where siteId=$siteId and categId=" . $res["categId"]);
	...
	function dir_get_all_categories_np($offset, $maxRecords, $sort_mode, $find, $parent) {
		...
		$query
			= "select * from tiki_directory_categories where categId<>$parent $mid order by $sort_mode limit $offset,$maxRecords";
	...
	// Replace
	function dir_replace_category($parent, $categId, $name, $description, $childrenType, $viewableChildren, $allowSites, $showCount,
		$editorGroup) {
		$name = addslashes($name);
