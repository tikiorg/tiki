<?php
/** \file
 * $Header: /cvsroot/tikiwiki/tiki/lib/debug/debug-command_watch.php,v 1.2 2003-08-01 10:30:55 redflo Exp $
 *
...
class DbgCmd_Watch extends DebuggerCommand
{
  ...
  function DbgCmd_Watch()
  {
    ...
    if (is_readable($this->watchfile()))
    {
      ...
      $a = unserialize($s);
      if (count($a) > 0) $this->watches = (array)$a;
    }
  }
  ...
  function draw_interface()
  {
    ...
    foreach ($this->watches as $v)
      ...
      $result[] = array("var"   => $v,
                        "value" => ((substr($v, 0, 1) == '$')
                                 ? $this->value_of_php_var($v) 
                                 : $this->value_of_smarty_var($v)));
    //
    global $smarty;
