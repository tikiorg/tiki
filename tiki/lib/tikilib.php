...
class TikiLib {
    ...
    */
	...
	while ($res = $result->fetchRow()) {
	    $res["questions"] = $this->getOne("select count(*) from `tiki_quiz_questions` where `quizId`=?",array($res["quizId"]));

	    $res["results"] = $this->getOne("select count(*) from `tiki_quiz_results` where `quizId`=?",array($res["quizId"]));
	    $ret[] = $res;
	}
