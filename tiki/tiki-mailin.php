    ...
    if($acc['type']=='wiki-put') {
      ...
      if(isset($body)) {
        if(!$tikilib->page_exists($page)) {
          ...
          $tikilib->create_page($page, 0, $body, date('U'), "Created by email from ".$acc["account"], $aux["sender"]["email"], '0.0.0.0','');
        } else {
          $tikilib->update_page($page,$body,"Created by email from ".$acc["account"], $aux["sender"]["email"], '0.0.0.0','');
    ...
    if($acc['type']=='wiki-append') {
      ...
      if(isset($body)) {
        if(!$tikilib->page_exists($page)) {
          ...
          $info = $tikilib->get_page_info($page);  	
          $tikilib->update_page($page,$info['data'].$body,"Created by email from ".$acc["account"], $aux["sender"]["email"], '0.0.0.0','');
          print("Page: $page has been updated");
        }
      }
    }
