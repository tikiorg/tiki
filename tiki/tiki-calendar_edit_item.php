<?php
// $Header: /cvsroot/tikiwiki/tiki/tiki-calendar_edit_item.php,v 1.9 2006-12-15 02:40:17 mose Exp $
...
if (isset($_POST['act'])) {
	...
	or (!empty($save['calitemId']) and $caladd["$newcalid"]['tiki_p_change_events'])) {
		...
		$save['start'] = (ceil($save['date_start']/(60*60*24))*60*60*24) - 60*60 + $_REQUEST['start_Hour']*60*60 + $_REQUEST['start_Minute']*60;
		...
		if ($save['end_or_duration'] == 'duration') {
			...
			$save['end'] = (ceil($save['date_end']/(60*60*24))*60*60*24) - 60*60 + $_REQUEST['end_Hour']*60*60 + $_REQUEST['end_Minute']*60;
