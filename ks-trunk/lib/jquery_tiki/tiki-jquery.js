...
function convertOverlib(element, tip, params) {	// process modified overlib event fn to cluetip from {popup} smarty func
	...
	if (element.tipWidth) {
		...
		if ($el.width() > $window.width()) {	// this fails when the tip contains images which end up with zero width
			$el.width($window.width() * 0.8);
		}
		options.width = $el.width() + 26;		// add space for css padding
		$el.remove();
		options.width = Math.max(options.width, 150);	// make min width enough for standard thumbnail width - TODO better
