...
// $Id: include_webmail.php 35618 2011-07-23 13:59:19Z lphuberdeau $
...
// building overview
	$areas_table = TikiDb::get()->table('tiki_areas');
	$conditions = array();

	// if count zero, table probably not up-to-date
	if($areas_table->fetchCount($conditions)==0) update_areas();
	...
	foreach($result as $item){
		...
		$area['categName'] = $categlib->get_category_name($item['categId']);	
		$area['description'] = $categlib->get_category_description($item['categId']);
		$areas[] = $area;
	}
