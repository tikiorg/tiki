...
$access->check_feature('feature_invit');
$access->check_permission('tiki_p_invit');
...
$previous=array();
$res=$tikilib->query("SELECT * FROM `tiki_invit` ORDER by `ts` DESC");
...
if (isset($_REQUEST['loadprevious']) && !empty($_REQUEST['loadprevious']) && isset($previous[(int)$_REQUEST['loadprevious']])) {
    ...
    $res=$tikilib->query("SELECT * FROM `tiki_invited` WHERE id_invit=?", array((int)$_REQUEST['loadprevious']));
    $prev_invited="";
    ...
    $_REQUEST['invitgroups'] = explode(',', $prev['groups']);
}
...
foreach($allgroups['data'] as $agroup)
    $invitgroups[$agroup['groupName']] = $agroup['groupDesc'];
$smarty->assign("invitgroups", $invitgroups);
...
if (isset($_REQUEST['send'])) {
    ...
    $igroups=$_REQUEST['invitgroups'];
    
    if (!empty($_REQUEST['confirm'])) {
        
        $tikilib->query("INSERT INTO `tiki_invit` (inviter, groups, ts, emailsubject,emailcontent,wikicontent,wikipageafter) VALUES (?,?,?,?,?,?,?)",
        ...
        $res=$tikilib->query("SELECT MAX(id) as `id` FROM `tiki_invit` WHERE `inviter`=? AND `ts`=?", array($user, $tikilib->now));
        $row=$res->fetchRow(); $id=$row['id'];
        
        foreach($emails as $m)
            $tikilib->query("INSERT INTO `tiki_invited` (id_invit, email, firstname, lastname, used) VALUES (?,?,?,?,?)",
        ...
        foreach($emails as $m) {
            ...
            $mail->setCrlf("\n");
            $url=str_replace('tiki-invit.php', 'tiki-invited.php', $_SERVER['SCRIPT_URI'])
                .'?invit='.$id.'&email='.urlencode($m['email']);
            $text=$_text;
