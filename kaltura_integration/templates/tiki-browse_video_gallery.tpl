...
<div class="navbar">
	...
	{if $system eq 'n'}
		{if $tiki_p_admin_galleries eq 'y' or ($user and $user eq $owner)}
			...
			{button href="tiki-browse_video_gallery.php?galleryId=$galleryId&amp;rebuild=$galleryId" _text="{tr}Rebuild Thumbnails{/tr}"}
		{/if}
		
		{if $tiki_p_upload_images eq 'y'}
			{if $tiki_p_admin_galleries eq 'y' or ($user and $user eq $owner) or $public eq 'y'}
				{button href="tiki-upload_image.php?galleryId=$galleryId" _text="{tr}Upload Video{/tr}"}
			{/if}
		{/if}
	...
	{if $tiki_p_admin_galleries eq 'y'}
		...
		{button href="tiki-show_all_images.php?id=$galleryId" _text="{tr}All Videos{/tr}"}
	{/if}
	...
	{if $user and $prefs.feature_user_watches eq 'y'}
		{if $user_watching_gal eq 'n'}
			<a href="tiki-browse_video_gallery.php?galleryId={$galleryId}&amp;watch_event=image_gallery_changed&amp;watch_object={$galleryId}&amp;watch_action=add" title="{tr}Monitor this Gallery{/tr}" class="icon">{icon _id='eye' alt="{tr}Monitor this Gallery{/tr}" align='right' hspace="1"}</a>
		{else}
			<a href="tiki-browse_video_gallery.php?galleryId={$galleryId}&amp;watch_event=image_gallery_changed&amp;watch_object={$galleryId}&amp;watch_action=remove" title="{tr}Stop Monitoring this Gallery{/tr}" class="icon">{icon _id='no_eye' alt="{tr}Stop Monitoring this Gallery{/tr}" align='right' hspace="1"}</a>
		{/if}
	{/if}
</div>
