...
<div>
<form action="tiki-video_galleries.php" method="get">
<select name="filter">
...
<th><a href="tiki-video_galleries.php?offset={$offset}&amp;sort_mode={if $sort_mode eq 'name_desc'}name_asc{else}name_desc{/if}">{tr}Name{/tr}</a></th>
{/if}
{if $prefs.gal_list_parent eq 'y'}
<th><a href="tiki-video_galleries.php?offset={$offset}&amp;sort_mode={if $sort_mode eq 'parentgallery_desc'}parentgallery_asc{else}parentgallery_desc{/if}">{tr}Parent{/tr}</th>
{/if}
{if $prefs.gal_list_description eq 'y'}
<th><a href="tiki-video_galleries.php?offset={$offset}&amp;sort_mode={if $sort_mode eq 'description_desc'}description_asc{else}description_desc{/if}">{tr}Description{/tr}</a></th>
{/if}
{if $prefs.gal_list_created eq 'y'}
<th><a href="tiki-video_galleries.php?offset={$offset}&amp;sort_mode={if $sort_mode eq 'created_desc'}created_asc{else}created_desc{/if}">{tr}Created{/tr}</a></th>
{/if}
{if $prefs.gal_list_lastmodif eq 'y'}
<th><a href="tiki-video_galleries.php?offset={$offset}&amp;sort_mode={if $sort_mode eq 'lastModif_desc'}lastModif_asc{else}lastModif_desc{/if}">{tr}Last modified{/tr}</a></th>
{/if}
...
{if $tiki_p_admin_galleries eq 'y' or ($user and $galleries[changes].user eq $user)}
  {if ($tiki_p_admin eq 'y') or ($galleries[changes].perms.has_special_perms eq 'n') or ($galleries[changes].perms.tiki_p_create_galleries eq 'y' ) }
    &nbsp;&nbsp;<a class="gallink" title="{tr}Delete{/tr}" href="tiki-video_galleries.php?offset={$offset}&amp;sort_mode={$sort_mode}&amp;removegal={$galleries[changes].galleryId}">{icon _id='cross' alt='{tr}Delete{/tr}'}</a>
  {/if}
  {/if}
