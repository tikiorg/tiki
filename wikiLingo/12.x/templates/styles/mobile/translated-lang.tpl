...
{*<ul class="clearfix cssmenu_horiz">
	...
	<a href="#tanslations" data-role="button" data-inline="true" data-rel="popup">{* mobile *}
	...
	{* ..than on hover first show the list of translations including the current language highlighted *}
        {if empty($trads[0].lang)}
            ...
            {*<li class="tabcontent">*}
                ...
                {if $object_type eq 'wiki page' and ($tiki_p_edit eq 'y' or (!$user and $prefs.wiki_encourage_contribution eq 'y')) and !$lock}
                    <a data-role="button" data-inline="true" href="tiki-edit_translation.php?page={$page|escape}">{tr}Please select a language before performing translation.{/tr}</a>
                {elseif $object_type eq 'article' and $tiki_p_edit_article eq 'y'}
                    <a data-role="button" data-inline="true" href="tiki-edit_article.php?articleId={$articleId|escape}">{tr}Please select a language before performing translation.{/tr}</a>
                {/if}
            {*</li>*}
			</div>
        {else}
		<div id="tanslations" data-role="popup">{* mobile
			...
			{if $object_type eq 'wiki page'}
				...
				<a data-role="button" data-inline="true" href="tiki-index.php?page={$trads[0].objName|escape}&no_bl=y" title="{$trads[0].langName|escape} ({$trads[0].lang|escape}): {$trads[0].objName}" class="selected">
					{$trads[0].langName|escape} ({$trads[0].lang|escape})
				</a>
			{elseif $object_type eq 'article'}
				<a data-role="button" data-inline="true" href="tiki-read_article.php?articleId={$trads[0].objId}" title="{$trads[0].langName|escape} ({$trads[0].lang|escape}): {$trads[0].objName}" class="selected">
					{$trads[0].langName|escape} ({$trads[0].lang|escape})
				</a>
			{/if}
			...
			{* Show the list of available translations *}
				...
				{* For wiki pages *}		
					{if $object_type eq 'wiki page' and $trads[i] neq $trads[0]}
						<a data-role="button" data-inline="true" href="tiki-index.php?page={$trads[i].objName|escape}&no_bl=y" title="{$trads[i].langName|escape} ({$trads[i].lang|escape}): {$trads[i].objName}" class="linkmodule {$trads[i].class}">
							{$trads[i].langName|escape} ({$trads[i].lang|escape})
						</a>
					{/if}
				{* For articles *}
					{if $object_type eq 'article' and $trads[i] neq $trads[0]}
						<a data-role="button" data-inline="true" href="tiki-read_article.php?articleId={$trads[i].objId}" title="{$trads[i].langName|escape} ({$trads[i].lang|escape}): {$trads[i].objName}" class="linkmodule {$trads[i].class}">
							{$trads[i].langName|escape} ({$trads[i].lang|escape})
						</a>
					{/if}
				{/section}
			{* For wiki pages only: Show a link to view all translations on a single page *}
				{if $object_type eq 'wiki page' and $prefs.feature_multilingual_one_page eq 'y' and $translationsCount gt 1}
					<h1>
						<a data-role="button" data-inline="true" href="tiki-all_languages.php?page={$trads[0].objName|escape:url}&no_bl=y" title="{tr}Show all translations of this page on a single page{/tr}">{icon _id=application_view_columns title=""}
							{tr}All languages{/tr}
						</a>
					</h1>
				{/if}
			{* For wiki pages only: List of machine translation candidates if feature is switched on *}
				...
				{* List machine translation candidates for available language of the site *}
					{foreach from=$langsCandidatesForMachineTranslation item=mtl}
						<a data-role="button" data-inline="true" href="tiki-index.php?machine_translate_to_lang={$mtl.lang|escape}&page={$page|escape:"quotes"}&no_bl=y" title="{$mtl.langName|escape} ({$mtl.lang|escape})">
							{$mtl.langName|escape} *
						</a>
					{/foreach}
				{/if}
			{* For all object types: Translation maintenance *}
				{capture}{if $object_type eq 'wiki page' and $tiki_p_edit eq 'y'}
					<a data-role="button" data-inline="true" href="tiki-edit_translation.php?page={$trads[0].objName|escape:url}&no_bl=y" title="{tr}Translate page{/tr}">
						{tr}Translate{/tr}
					</a>
					<a data-role="button" data-inline="true" href="{service controller=translation action=manage type='wiki page' source=$page}" class="attach_detach_translation" data-object_type="wiki page" data-object_id="{$page|escape:'quotes'}" title="{tr}Manage page translations{/tr}">
						{tr}Manage translations{/tr}
					</a>
				{elseif $object_type eq 'article' and $tiki_p_edit_article eq 'y'}
					<a data-role="button" data-inline="true" href="tiki-edit_article.php?translationOf={$articleId}" title="{tr}Translate article{/tr}">
						{tr}Translate{/tr}
					</a>
					<a data-role="button" data-inline="true" href="{service controller=translation action=manage type=article source=$articleId}" class="attach_detach_translation" data-object_id="{$articleId|escape:'quotes'}" data-object_type="article" title="{tr}Manage article translations{/tr}">
						{tr}Manage translations{/tr}
					</a>
				{/if}{/capture}
