...
function setUpClueTips() {
	...
	$('.tips[title!=""]').cluetip($.extend({}, ctOptions, {width: 'auto'}));
	$('.titletips[title!=""]').cluetip($.extend({}, ctOptions, {width: 'auto'}));
	$('.tikihelp[title!=""]').cluetip($.extend({}, ctOptions, {splitTitle: ':', width: 'auto'}));
	$('.ajaxtips[rel!=""]').cluetip($.extend({}, ctOptions, {
		splitTitle: false, sticky: true, attribute: 'rel', mouseOutClose: 'cluetip', width: 'auto',
			...
			if (jqueryTiki.chosen) {
				...
				$.extend(opts, options);
		 		return this.each(function() {
					...
					$(this).chosen(opts2).on("liszt:ready", function () {
						var $this = $(this);
					});
				});
			}
...
(function($) {
	...
	$.fn.showError = function (message) {
		...
		this.each(function () {
			...
			var validate = $(that).closest('form').validate(), errors = {}, field, $additional = $('<ul>');
			...
			$('.form-group').removeClass('has-error')
				...
				.addClass('help-block')
				.prepend('<span class="glyphicon glyphicon-flag"/> ')
					...
					filterField.keypress(function (e) {
						...
						this.searching = setTimeout(function () {
							...
							if ((loaded === "" || loaded.length >= 3) && loaded !== $select.data('loaded')) {
								$spinner = $(field).parent().tikiModal(" ");
...
$.fn.registerFacet = function () {
	this.each(function () {
		...
		var selected = $('option:selected', this).size(),
			all = $('option', this).size();
...
$(document).on('tiki.modal.redraw', '#bootstrap-modal', function () {
	...
	if ($.fn.flexibleSyntaxHighlighter) {
		$('textarea', this).flexibleSyntaxHighlighter();
	}
});
...
$.openModal = function (options) {
	$('#bootstrap-modal')
		...
		.one('shown.bs.modal', options.open || function () {})
