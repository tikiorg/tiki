	...
	if ($top.length && !$("#toc-off").length && location.href.indexOf("tiki-print.php") == -1) {
		...
		if ($headers.length > 1) {
			...
			if (!jqueryTiki.autoToc_inline) { 
				...
				//Get left offset var for affix (This is needed, specially when there are col-md-pull//push interacting with page-data)
				var leftoffset = $('#autotoc').offset().left;
				$("> .nav", "#autotoc").css('left',leftoffset);
				affix();

				//Fix fixed position when resize window
				...
				function affix() {
					...
					.on('affix.bs.affix', function (e) {
						$(e.target).width(e.target.offsetWidth - 2)
					});
				}
