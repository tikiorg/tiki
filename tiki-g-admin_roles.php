...
if(!isset($_REQUEST['roleId'])) $_REQUEST['roleId'] = 0;
if($_REQUEST["roleId"]) {
...
$cant_pages = ceil($mapitems["cant"] / $maxRecords);$smarty->assign_by_ref('cant_pages',$cant_pages);$smarty->assign('actual_page',1+($offset/$maxRecords));
if($mapitems["cant"] > ($offset+$maxRecords)) {  $smarty->assign('next_offset',$offset + $maxRecords);} else {  $smarty->assign('next_offset',-1); }
...
if(!isset($_REQUEST['sort_mode2'])) $_REQUEST['sort_mode2']='name_asc';
