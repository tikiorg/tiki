...
// {{{
{
	...
	function add($type, $object, $options) {
		...
		switch ($type) {
			case 'wiki page':
				if (array_key_exists('languages', $options)) $renderer = new ObjectRenderer_MultilingualWiki($type, $object, $options);
				else $renderer = new ObjectRenderer_Wiki($type, $object, $options);
				break;
			...
			if ($renderer && $renderer->isValid()) {
				...
				foreach($this->customIndexes as $key => & $data) {
					if ($prop = $renderer->getIndexValue($key)) {
						if (isset($data[$prop])) $data[$prop][] = $index;
						else $data[$prop] = array($index);
					}
				}
			}
		}
		function finalize() {
			foreach($this->customIndexes as & $data) ksort($data);
		}
		function render($smarty, $key, $options) {
			foreach($this->customIndexes[$key] as $indexes) foreach($indexes as $index) {
...
// {{{
{
		...
		function _render($smarty, $options) {
			...
			foreach($languages as $lang) {
				if ($this->renderers[$lang]) $out.= $this->renderers[$lang]->_render($smarty, $options);
			}
			return $out;
		}
		function getIndexValue($key) {
			if (strpos($key, 'lang_') === 0) {
				...
				if (isset($this->renderers[$lang]) && $this->renderers[$lang]) return $this->renderers[$lang]->getIndexValue($key);
				return;
			}
...
// End of classes }}}
	...
	switch ($_GET['list']) {
		...
		case 'glossary':
			if (isset($_REQUEST['languages'])) $languages = (array)$_REQUEST['languages'];
			else $languages = array($prefs['language']);
			...
			foreach($languages as $num => $code) {
				...
				$indexPages[] = array('key' => $key, 'indextitle' => tra('Index (' . $code . ')'), 'options' => array('decorator' => 'indexrow', 'display' => 'title', 'languages' => array($code),),);
			}
		...
		// Display all data
		$objectList->render($smarty, 'title', array('decorator' => 'indexed', 'display' => 'object'));
