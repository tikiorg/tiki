...
"Apache has the right to write in your file tree, which enables the installation, removal or upgrade of packages. When you are done with those operations, think to fix those permissions back to a safe state (by using \"./fixperms.sh fix\" for example)." => "Apachella on oikeus kirjoittaa hakemistopuuhusi, mikä sallii asennuksen, poiston tai pakettien päivityksen. Kun nuo asennukset on tehty, kannattaa oikeudet palauttaa turvalliseen tilaan (käyttäen \"./fixperms.sh fix\" esimerkiksi).",
"To install, remove or upgrade packages you need to give the apache user the right to write files in your web tree (you can use \"./fixperms.sh open\" to set it up). After installation you need to remove that permission (using \"./fixperms.sh fix\")." => "Asentaaksesi, poistaaksesi tai päivittääksesi paketteja sinun on annettava apache käyttäjälle oikeudet kirjoittaa tiedostoja web puuhun (voit käyttää \"./fixperms.sh open\" komentoa). Asennuksen jälkeen palauta oikeudet (käyttäen \"./fixperms.sh fix\").",
