...
{if !isset($tpl_module_title)}
	{capture assign=tpl_module_title}{tr}Switch Theme{/tr} {if !empty($tc_theme) or $group_theme or (($section eq 'admin' or !$section) and $prefs.theme_admin neq '') or $css_theme}{icon name="information" class="tips btn btn-sm btn-link" title="{tr}Not allowed here{/tr}:{tr}Displayed theme{/tr}: {$prefs.theme_active}{if !empty($prefs.theme_option_active)}/{$prefs.theme_option_active}{/if} ({if !empty($tc_theme)}{tr}Theme Control{/tr}{elseif ($section eq 'admin' or !$section) and $prefs.theme_admin neq ''}{tr}Admin Theme{/tr}{elseif $group_theme}{tr}Group Theme{/tr}{elseif $css_theme}CSS {tr}Editor{/tr}{/if})"}{/if}{/capture}
{/if}
