...
{if $prefs.feature_categories eq 'y' and $page and $showmodule}
	{if !isset($tpl_module_title)}
		{assign var=tpl_module_title value="{tr}$modcattitle{/tr}"}
	{/if}
					...
					{if $i.incat eq 'y'}
						<tr>
							<td class="{cycle advance=false}">
								{if $module_params.path eq 'n'}{$i.name}{else}{$i.categpath}{/if}
							</td>
			...
			<div align="center">
				<form method="post" action="{$smarty.server.REQUEST_URI}" target="_self">
					...
					{if $module_params.add ne 'n'}
						{if $module_params.detail eq 'y'}
							<option value="0" style="font-style: italic;">
								{if $module_params.categorize}{tr}{$module_params.categorize}{/tr}{else}{tr}Categorize{/tr}{/if}
							</option>
						{elseif !isset($module_params.notop)}
					...
					{foreach key=k item=i from=$modcatlist}
						{if $module_params.detail ne 'y' or $i.incat ne 'y'}
							{if !($module_params.add eq 'n' and $i.incat ne 'y')}
								<option value="{$k}"{if $i.incat eq 'y'} selected="selected"{/if}>
									{if $module_params.path eq 'n'}{$i.name}{else}{$i.categpath}{/if}
								</option>
							{/if}
						{/if}
					...
					{if $module_params.multiple eq 'y' and $modules_params.add ne 'n'}
						<div align="center">
							<input type="submit" name="categorize" value="{if $module_params.categorize}{tr}{$module_params.categorize}{/tr}{else}{tr}Categorize{/tr}{/if}" />
						</div>
					{/if}
					</select>
				</form>
