...
$lasttimeout=(int)$_REQUEST['lasttimeout']; if ($lasttimeout < $timeout_min) $lasttimeout = $timeout_min;
...
foreach($chans as $chan) {
	...
	if (($msgon == $channel) && (!is_null($channel))) {
		...
		if (substr($msg, 0, 1) == '/') {
			...
			$tikilib->query("INSERT INTO tiki_minichat (nick,user,ts,channel,msg) VALUES (?,?,?,?,?)",
							array($user,$user,$tikilib->now, $channel, $msg));
	...
	if ($lastid > 0) {
		...
		if ($maxid != $lastid) {
			$lastid=0; $lasttimeout=$timeout_min;
