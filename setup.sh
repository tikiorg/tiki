...
composer_core()
{
	...
	then
		if [ ${LOGCOMPOSERFLAG} = "0" ] ; then
			...
			until php -dmemory_limit=-1 temp/composer.phar install --prefer-dist --no-dev 2>&1 | sed '/Warning: Ambiguous class resolution/d'
		...
		if [ ${LOGCOMPOSERFLAG} = "1" ] ; then
			until php -dmemory_limit=-1 temp/composer.phar install --prefer-dist --no-dev > ${TIKI_COMPOSER_INSTALL_LOG}
			# setting memory_limit here prevents suhosin ALERT - script tried to increase memory_limit to 536870912 bytes
			do
				if [ $N -eq 7 ];
				then
