...
while ($file = readdir($h)) {
	...
	if (is_file("games/thumbs/$file") && $file != '.' && $file != '..' && !ereg('\.txt$', $file) && $file != 'index.php' && $file != 'README') {
...
if (isset($_POST["upload"]) && $tiki_p_admin_games == 'y') {
	...
	if (isset($_FILES['flashfile']) && is_uploaded_file($_FILES['flashfile']['tmp_name']) && isset($_FILES['imagefile']) && is_uploaded_file($_FILES['imagefile']['tmp_name'])) {
		$name1 = basename($_FILES['flashfile']['name']);
		$name2 = basename($_FILES['imagefile']['name']);
		$parts = explode('.', $name2);
		$namec = $parts[0] . '.' . $parts[1];
		if (!ereg("\.(swf|dcr)$", $name1)) {
			$smarty->assign('msg', tra("The game file must have .swf or .dcr extension"));
			$smarty->display("error.tpl");
			die;
		}
		if (!$namec || $namec != $name1 || !isset($parts[2]) || !in_array($parts[2], array('gif', 'png', 'jpg'))) {
			$smarty->assign('msg', tra("The thumbnail name must be") . " $name1.gif, $name1.jpg " . tra("or") . " $name1.png");
			$smarty->display("error.tpl");
			die;
		}
		...
		$smarty->assign('msg', tra("Please supply both files"));
		$smarty->display("error.tpl");
		die;
	}
}
...
if (isset($_REQUEST["edit"]) && $tiki_p_admin_games == 'y') {
	...
	if (array_key_exists($file, $games)) {
		...
		$data = '';
		if (is_file($descfile)) $data = file_get_contents("games/thumbs/$file" . '.txt');
