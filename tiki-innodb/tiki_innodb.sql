...
-- 3. Move all deletes to the top of the script (in reverse order of creation). Tables must be dropped in the reverse order of creation when FKs are defined,
--			and the is 1(!!) defined on the table tiki_calendar_items
--			tiki.sql should probably be broken up into 3 scripts (in order of running)
--				1. tiki_dropdb.sql		Drop the schema
--				2. tiki_createdb.sql	Create the schema
--				3. tiki_inidb.sql		Add data to the new database
-- 	Problems (definistion commented out for now)
--		tiki_user_bookmarks_folders - Incorrect table definition; there can be only one auto column and it must be defined as a key
--			The create of this table has been commented out
--
/*
	Notes installation and test
	-----------------------------
	* Installation
	1) Create the database using this script
	2) Create a new Tiki filestructure from trunk
	3) Manually create db/local.php
	
	* Tiki test
	1) Tiki starts OK
	2) In the admin panel: Database Version Problem is displays
	3) Create a wiki page
		-> Fatal error: Call to a member function get_object_id() on a non-object in <path-to-tiki>\lib\filegals\filegallib.php on line 1599
*/
-- --------------------------------------------------------
