...
if(isset($_REQUEST['save_act'])) {
	...
	$rid = 0;
	if(isset($_REQUEST['userole']) && $_REQUEST['userole']) $rid = $_REQUEST['userole'];
...
if(!isset($_REQUEST['sort_mode'])) $_REQUEST['sort_mode']='flowNum_asc';
if(!isset($_REQUEST['find'])) $_REQUEST['find']='';
if(!isset($_REQUEST['were'])) $_REQUEST['where']=$where;
...
if(isset($_REQUEST['activityId'])) {
  for($i=0;$i<count($activities["data"]);$i++) {
    ...
    $activities["data"][$i]['to'] = $activityManager->transition_exists($_REQUEST['pid'],$_REQUEST['activityId'],$id) ? 'y' :'n';
    $activities["data"][$i]['from'] = $activityManager->transition_exists($_REQUEST['pid'],$id,$_REQUEST['activityId']) ? 'y' :'n';
  }
}
