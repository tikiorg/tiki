...
<tr><td>
{if !$pointx || !$pointy}<small>{tr}route view not yet saved{/tr}</small>{else}<small>{tr}route of pages in this structure{/tr}</small>{/if}
...
function cload() {literal}{{/literal}
  ...
  {elseif $mode == 'satellite'}cmap.setCenter(new GLatLng({$gmap_defaulty}, {$gmap_defaultx}), {$gmap_defaultz}, G_SATELLITE_MAP);
	...
	var tempCoord = new GLatLng({$u[0]},{$u[1]});	
  ...
  clastClicked = {$i};
	{literal}});{/literal}		
	{/foreach}
	cpolyline = new GPolyline(tempCoordArr, "#FF0000", 4);
	cmap.addOverlay(cpolyline);
