<?php
// $Header: /cvsroot/tikiwiki/_mods/lib/html2pdf/css.list-style-type.inc.php,v 1.1 2008-01-15 09:20:35 mose Exp $
...
class CSSListStyleType extends CSSSubProperty {
  ...
  function value2ps($value) {
    if ($value === LST_DISC)    { return "/disc"; }
    if ($value === LST_CIRCLE)  { return "/circle"; }
    if ($value === LST_SQUARE)  { return "/square"; }
    if ($value === LST_NONE)    { return "/none"; }
    if ($value === LST_DECIMAL) { return "/string"; }
    if ($value === LST_DECIMAL_LEADING_ZERO) { return "/string"; }
    if ($value === LST_LOWER_ROMAN) { return "/string"; }
    if ($value === LST_UPPER_ROMAN) { return "/string"; }
    if ($value === LST_LOWER_LATIN) { return "/string"; }
    if ($value === LST_UPPER_LATIN) { return "/string"; }
