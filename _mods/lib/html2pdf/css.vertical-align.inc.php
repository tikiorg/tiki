<?php
// $Header: /cvsroot/tikiwiki/_mods/lib/html2pdf/css.vertical-align.inc.php,v 1.1 2008-01-15 09:20:48 mose Exp $
...
class CSSVerticalAlign extends CSSProperty {
  ...
  function value2ps($value) {
    if ($value === VA_SUPER)       { return "/super"; }
    if ($value === VA_SUB)         { return "/sub"; }
    if ($value === VA_TOP)         { return "/top"; }
    if ($value === VA_MIDDLE)      { return "/middle"; }
    if ($value === VA_BOTTOM)      { return "/bottom"; }
    if ($value === VA_BASELINE)    { return "/baseline"; }
    if ($value === VA_TEXT_TOP)    { return "/top"; }
    if ($value === VA_TEXT_BOTTOM) { return "/bottom"; }
    return "/baseline";
  }
  ...
  function ps($writer) {  
    if ($this->applicable()) {
      $writer->write($this->value2ps($this->get())." 1 index put-vertical-align\n");    
    };
  }
