...
if (!$exit_module){
	if (!isset ($_REQUEST["objectCreated"]) && isset ($_REQUEST["createObject"]) && isset ($_REQUEST["createObjectCategoryId"])) {
		if($tiki_p_admin != 'y' && !$userlib->object_has_permission($user, $workspace["workspaceId"], 'workspace', "aulawiki_p_create_resources")){
			...
			$categUtil->redirect($id, $wscode."-".$_REQUEST["createObjectName"], $_REQUEST["createObjectType"]);
	...
	$categObjects = $categUtil->get_category_objects($selectedCategory);
	...
	foreach ($categObjects as $key => $categObject) {
		...
		$categObjects[$key]["removeURL"] = $categUtil->get_url_remove($categObject["objId"], $categObject["type"]);
	}
