...
*/
class CategUtilLib extends TikiDB {
	var $db; // The PEAR db object used to access the database
	function CategUtilLib($db) {
		if (!$db) {
	...
	function get_url_admin_assignments($id, $name) {
		return "./aulawiki-view_module.php?module=aulawiki_assignments_admin&workspaceId=".$id;
	}

	function get_url_admin_workspace($id, $name) {
		return "./aulawiki-workspaces.php?find=".$name."&search=find&edit=".$id;
	}
	...
	function get_url_remove_filegallery($id) {
		return "./tiki-file_galleries?removegal=".$id;
	}
	...
	function get_url_remove_workspace($id) {
		return "./aulawiki-workspaces.php?viewWS=0&delete=".$id;
	}
	...
	*/
	...
	function create_assignments($name, $desc, $parentCategoryId, $resourceId) {
		...
		$idCatObj = $categlib->add_categorized_object("assignments", $resourceId, $desc, $name, "aulawiki-view_module.php?module=aulawiki_assignments_admin&workspaceId=".$resourceId);
	...
	function create_blog($name, $desc, $parentCategoryId) {
		...
		$head = '{include file="aulawiki-blog.tpl"}';
	...
	function remove_structure($name) {
	...
	function create_faq($name, $desc, $parentCategoryId) {
		...
		$idCatObj = $categlib->add_categorized_object("faq", $fid, $desc, $name, "./tiki-take_quiz.php?quizId=$qid");
		$categlib->categorize($idCatObj, $parentCategoryId);
		return $fid;
	}
