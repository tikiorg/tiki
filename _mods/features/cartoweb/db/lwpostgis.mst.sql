...
-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
-- 
-- $Id: lwpostgis.mst.sql,v 1.1 2006-07-01 23:37:05 tombombadilom Exp $
--
...
CREATE OR REPLACE FUNCTION translate(geometry,float8,float8,float8)
	RETURNS geometry
	AS '/usr/lib/postgresql/8.1/lib/liblwgeom.so.1.1', 'LWGEOM_translate'
	LANGUAGE 'C' IMMUTABLE STRICT; -- WITH (isstrict) ;
...
-- Availability: 1.1.0CVS
CREATE OR REPLACE FUNCTION scale(geometry,float8,float8,float8)
	RETURNS geometry
	AS '/usr/lib/postgresql/8.1/lib/liblwgeom.so.1.1', 'LWGEOM_scale'
	LANGUAGE 'C' IMMUTABLE STRICT; -- WITH (isstrict) ;
...
CREATE OR REPLACE FUNCTION transscale(geometry,float8,float8,float8,float8)
	RETURNS geometry
	AS '/usr/lib/postgresql/8.1/lib/liblwgeom.so.1.1', 'LWGEOM_transscale'
	LANGUAGE 'C' IMMUTABLE STRICT; -- WITH (isstrict) ;
...
CREATE OR REPLACE FUNCTION postgis_scripts_installed() RETURNS text
        AS 'SELECT \'1.1.1\'::text AS version'
        LANGUAGE 'sql' IMMUTABLE;
...
CREATE OR REPLACE FUNCTION postgis_scripts_build_date() RETURNS text
        AS 'SELECT \'2006-05-08 10:31:19\'::text AS version'
        LANGUAGE 'sql' IMMUTABLE;
