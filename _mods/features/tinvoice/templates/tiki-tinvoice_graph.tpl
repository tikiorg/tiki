...
<div class="navbar" style="height: 36px;">
	...
	htmlelement=tinvoice-graph}tiki-tinvoice_graph.php?graphPeriod=week&todate={$todate}{/ajax_href}>{tr}Week{/tr}</a>&nbsp;
	...
	{ajax_href template=tiki-tinvoice_graph.tpl
	htmlelement=tinvoice-graph}tiki-tinvoice_graph.php?graphPeriod=month&todate={$todate}{/ajax_href}>{tr}Month{/tr}</a>&nbsp;
	</div><div class="button2" style="text-align: center; width: 70px; float: left;"><a class="linkbut"  {if $graphPeriod eq "trimester"} style="background-color: orange" {/if} href="tiki-tinvoice_graph.php?graphPeriod=trimester&todate={$todate}" onclick="toggle('xtype');">{tr}Trimester{/tr}</a>&nbsp;
...
</div><div  class="button2" style="text-align: center; width: 70px; float: left;"><a class="linkbut"  {if $graphPeriod eq "semester"} style="background-color: orange" {/if} href="tiki-tinvoice_graph.php?graphPeriod=semester&todate={$todate}" onclick="toggle('xtype');">{tr}Semester{/tr}</a>&nbsp;
	</div><div class="button2"  style="text-align: center; width: 70px; float: left;"><a class="linkbut"  {if $graphPeriod eq "year"} style="background-color: orange" {/if} href="tiki-tinvoice_graph.php?graphPeriod=year&todate={$todate}" onclick="toggle('xtype');">{tr}Year{/tr}</a>&nbsp;
...
<div class="navbar">
	<a class="linkbut" href="tiki-tinvoice_graph.php?graphPeriod={$graphPeriod}&todate={$prev}" alt="{tr}previous{/tr}">{tr}<< Previous {$graphPeriod}{/tr}</a>&nbsp;
	<a class="linkbut" href="tiki-tinvoice_graph.php?graphPeriod={$graphPeriod}&todate={$next}" alt="{tr}next{/tr}">{tr}Next {$graphPeriod}>>{/tr}</a>&nbsp;
</div>

<div align="center">
	<img id="chart" src="tiki-tinvoice_chart.php?graphPeriod={$graphPeriod}&todate={$todate}"  border=0 alt="tinvoice graphs" />
</div>
</div>
