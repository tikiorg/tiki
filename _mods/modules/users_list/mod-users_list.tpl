{* $Header: /cvsroot/tikiwiki/_mods/modules/users_list/mod-users_list.tpl,v 1.6 2007-11-22 17:05:43 sylvieg Exp $ *}
<table class="normal">
...
<tr class="{cycle}">
{if $module_params_users_list.login ne 'n'}<td>{if $users[user].info_public ne 'n'}<a class="link" href="tiki-user_information.php?view_user={$users[user].user|escape:"url"}" title="{tr}view{/tr}">{$users[user].user}</a>{else}{$users[user].user}{/if}</td>{/if}
{if $module_params_users_list.realName eq 'y'}<td>{if $users[user].info_public ne 'n'}<a class="link" href="tiki-user_information.php?view_user={$users[user].user|escape:"url"}" title="{tr}view{/tr}">{$users[user].realName}</a>{else}{$users[user].realName}{/if}</td>{/if}
...
{if $module_params_users_list.avatar eq 'y'}<td>{if $users[user].info_public ne 'n'}<a class="link" href="tiki-user_information.php?view_user={$users[user].user|escape:"url"}" title="{tr}view{/tr}">{$users[user].avatar}</a>{else>{$users[user].avatar}{/if}</td>{/if}
