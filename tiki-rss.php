...
if (($rss_version > 1) && ($rss_version < 5)) {
$datenow = htmlspecialchars(gmdate('D, d M Y H:i:s T', date("U")));
} else $datenow = htmlspecialchars($tikilib->iso_8601(date("U")));
...
if ($rss_version >= 5)
{
	...
	if ($output == "")
	{
	  ...
	  foreach ($changes["data"] as $chg) {
	    ...
	    if (($title == "") && array_key_exists("created", $chg)) {
    		$title = gmdate('D, d M Y H:i:s T', $chg["created"]); 
	    }
