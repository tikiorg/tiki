{* $Id $ *}
{* params:
 *   autoloaddelay in seconds (default = 2, -1 = off)
 *
 *}
{strip}
  ...
  <form action="javascript:void(null);" onsubmit="return submitWebmail();" id="webmail_form" name="webmail_form">
	<div class="mod_webmail_heading">
		{if $prefs.feature_ajax}
			<!--  a title="{tr}Refresh{/tr}" onclick="doRefreshWebmail();return false;">{icon _id='arrow_refresh'}</a-->
			<a title="{tr}Refresh{/tr}" {ajax_href template="modules/mod-webmail_inbox.tpl" htmlelement="mod-webmail_inbox$module_position$module_ord"|escape function="doRefreshWebmail"}tiki-webmail_ajax.php{/ajax_href}>
...
setTimeout("doRefreshWebmail()", {$autoRefresh * 1000});
</script>	
	{/if}
</form>
{else}
