...
# 		$query = "select tiki_workspaces.*,tiki_workspace_types.name as typename from tiki_workspaces,tiki_workspace_types where tiki_workspaces.type=tiki_workspace_types.id and tiki_workspaces.closed=? and tiki_workspaces.startDate<? and tiki_workspaces.endDate>?";
	...
	function get_child_workspaces_groups0 ($topWS, $WSgrp) {
                if (!$topWS) 
                	return false;
		$wsUserLib = new WorkspaceUserLib($this->db);
		$grps = $wsUserLib->get_descendant_groups($WSgrp, TRUE);
		foreach ($grps as $k => $d) {
	        	$groups["data"][]["groupName"]=$k;
               }
		$wsData = $this->get_child_workspaces(0,-1,'name_desc',$topWS);
		foreach ($wsData["data"] as $k => $d) {
			$descgroups= $wsUserLib->get_descendant_groups("WSGRP".$d["code"], TRUE);
        		foreach ($descgroups as $k1 => $d1) {
		        	$groups["data"][]["groupName"]=$k1;
		               }
		}
	return $groups;
	}	

	function get_child_workspaces_groups ($topWS, $WSgrp) {
                if (!$topWS) 
                	return false;
		$topworkspace=$this->get_workspace_by_id($topWS);
		$wsUserLib = new WorkspaceUserLib($this->db);
		$grps = $wsUserLib->get_descendant_groups("WSGRP".$topworkspace["code"], TRUE);
		foreach ($grps as $k => $d) {
	        	$groups[]=$k;
               }
		$wsData = $this->get_child_workspaces(0,-1,'name_desc',$topWS);
