{if $row.tracker_field_product_stock|nonp gt 0}
	...
	{else}
		{wikiplugin _name='addtocart' code=$row.object_id description=$row.name|nonp price=$row.tracker_field_product_price ajaxaddtocart='y' href="tiki-index.php?page=cart+product&itemId="|cat:$row.object_id weight=$row.tracker_field_product_weight}{/wikiplugin}
	{/if}
{/if}
