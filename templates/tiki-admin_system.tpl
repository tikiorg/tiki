...
{if $tiki_p_admin eq 'y'}
	{remarksbox type="warning" title="{tr}Advanced feature{/tr}"}
		...
		<div id="fixutf8" {if $advanced_features ne 'y'}style="display:none;"{else}style="display:block;"{/if}>
			...
			<table class="normal">
				<tr>
					<td class="form" colspan="4">{tr}Warning: Make a backup of your Database before using this function!{/tr}</td>
				</tr>
				<tr>
					<td class="form" colspan="4">{tr}Warning: If you try to convert large tables, raise the maximum execution time in your php.ini!{/tr}</td>
				</tr>
				<tr>
					<td class="form" colspan="4">{tr}This function converts ISO-8859-1 encoded strings in your tables to UTF-8{/tr}</td>
				</tr>
					<tr>
					<td class="form" colspan="4">{tr}This may be necessary if you created content with tiki &lt; 1.8.4 and Default Charset settings in apache set to ISO-8859-1{/tr}</td>
				</tr>
				<tr>
					<td colspan="4">&nbsp;</td>
				</tr>
			...
			<table class="sortable" id="tablefix" width="100%">
				...
				{foreach key=key item=item from=$tabfields}
					<tr>
						<td class="{cycle advance=false}">{$item.table}</td>
							<td class="{cycle advance=false}">{$item.field}</td>
