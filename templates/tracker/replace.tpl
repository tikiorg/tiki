...
<div>
	...
	<label>
		<input type="checkbox" name="descriptionIsParsed" {if $info.descriptionIsParsed eq 'y'}checked="checked"{/if}
			   value="1">
...
<div>
	...
	<label class="depends" data-on="useRatings">
		...
		<input type="text" name="ratingOptions"
			   value="{if $info.ratingOptions}{$info.ratingOptions|escape}{else}-2,-1,0,1,2{/if}">
	</label>
	<label class="depends" data-on="useRatings">
	...
	<fieldset class="depends sortable" data-on="useAttachments" data-selector="label">
		...
		{foreach from=$attachmentAttributes key=name item=att}
			<label>
				<input type="checkbox" name="orderAttachments[]"
					   value="{$name|escape}" {if $att.selected} checked="checked"{/if}>
				{$att.label|escape}
			</label>
		{/foreach}
	</fieldset>
...
<div>
	...
	<label class="depends" data-on="showCreated">
		...
		<div class="description">
			<a class="link" target="strftime"
			   href="http://www.php.net/manual/en/function.strftime.php">{tr}Date and Time Format Help{/tr}</a>
		</div>
	</label>
	<label class="depends" data-on="showCreated">
	...
	<label>
		...
		<select name="defaultOrderDir">
			...
			<option value="desc"
					{if $info.defaultOrderDir eq 'desc'}selected="selected"{/if}>{tr}descending{/tr}</option>
		</select>
	</label>
	<label>
...
<div>
	...
	<fieldset>
		...
		{foreach key=st item=stdata from=$statusTypes}
			<label>
				<input type="checkbox" name="defaultStatus[]"
					   value="{$st|escape}"{if in_array($st, $statusList)} checked="checked"{/if}>
				{$stdata.label|escape}
			</label>
		{/foreach}
	</fieldset>
	...
	<div>
		...
		<label class="depends" data-on="autoCreateGroup">
			...
			<select name="autoCreateGroupInc">
				<option value=""></option>
				{foreach from=$groupList item=g}
					<option value="{$g|escape}"
							{if $g eq $info.autoCreateGroupInc}selected="selected"{/if}>{$g|escape}</option>
				{/foreach}
			</select>
		</label>
		<label class="depends" data-on="autoCreateGroup">
