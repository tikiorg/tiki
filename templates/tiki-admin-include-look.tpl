...
toggle_brosho = function() {
	...
	$jq("#brosho-controls").append($jq("<li id='brosho-copy' />").text("{tr}Copy to Custom CSS{/tr}: ").append($jq("<a href='#'>{tr}Copy{/tr}</a>")).click(function(){
		$jq("textarea[name=header_custom_css]").val($jq("textarea[name=header_custom_css]").val() + "\n" + brosho_get_code());
	}));
	$jq("#brosho-generate").remove();
	return false;
};
var brosho_get_code = function() {
		...
		if (el.length) {
			...
			if(brosho_css !== undefined || brosho_css) { //does this element have changed css
				...
				for(var i = 0; i < properties.length; i++) { //loop through every property
					if(properties[i].length) temp_css += '\t' + $jq.trim(properties[i]) + ';\n'; //trim and add the property
				}
