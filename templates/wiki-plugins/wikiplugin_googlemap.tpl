...
<div class="wikitext" id="gmap{$gmapname|escape}container" style="display: {if $gmaphidden}none{else}block{/if}">
	...
	{if $gmaptype eq 'locator'}
		...
		{/jq}
		<input type="submit" class="btn btn-default" name="cancel" value="{tr}Find address{/tr}" onclick="showAddress{$gmapname|escape}(this.form.address.value);return false;"><br>
	...
	{if $gmap_defaultset && $user}
		<input type="submit" class="btn btn-default" name="cancel" onclick="document.getElementById('gmap{$gmapname|escape}_ajax_msg').innerHTML = '{tr}saving...{/tr}';saveGmapDefaultxyz{$gmapname|escape}();return false;" value="{tr}Save current map view as user default{/tr}">
	{/if}
	{if $gmaptype eq 'locator' && $gmapitemtype eq 'user'}
		<input type="submit" class="btn btn-default" name="cancel" onclick="document.getElementById('gmap{$gmapname|escape}_ajax_msg').innerHTML = '{tr}saving...{/tr}';saveGmapUser{$gmapname|escape}();return false;" value="{tr}Save as user location{/tr}">
	{/if}
	{if $gmaptype eq 'locator' && $gmapitemtype neq 'user' && $gmapitemtype neq 'trackeritem'}
		<input type="submit" class="btn btn-default" name="cancel" onclick="document.getElementById('gmap{$gmapname|escape}_ajax_msg').innerHTML = '{tr}saving...{/tr}';saveGmapItem{$gmapname|escape}();return false;" value="{tr}Save as object location{/tr}">
	{/if}
	{if !$gmap_in_form && ($gmaptype eq 'locator' || $gmap_defaultset && $user)}
		</form>
	{/if}
