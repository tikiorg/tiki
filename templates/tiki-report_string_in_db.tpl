				...
				{elseif $tableName=='tiki_comments'}
					...
					{elseif $row['objectType']=='wiki page'}
						{if ($column=='objectType' || $column=='data' || $column=='object')}
							<td><a href="tiki-index.php?page={$row['object']|escape}&threadId={$row['threadId']}&comzone=show#threadId{$row['threadId']}" class="link tips" title="{$row['data']|escape}:{tr}View page{/tr}" target="_blank">{$val|escape}</a></td>
						{else}
							<td>{$val|escape}</td>
						{/if}
					{else}
