	...
	{* --- tab with editor --- *}
		...
		<form action="tiki-send_newsletters.php" method="post" id='editpageform' enctype='multipart/form-data'>
			...
			<table class="formcolor" id="newstable">
				...
				<tr>
					<td><label for="sendfrom">{tr}Send From Email{/tr}</label></td>
					<td><input type="text" name="sendfrom" id="sendfrom" value="{$sendfrom|escape}"> {tr}if not:{/tr} {$prefs.sender_email|escape|default:"<em>{tr}Sender email not set{/tr}</em>"}</td>
				</tr>
