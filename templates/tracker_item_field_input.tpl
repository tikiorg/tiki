...
{elseif $field_value.type eq 'A'}
	...
	{if !isset($input_err) and $field_value.value ne ''}
		...
		{if $tiki_p_admin_trackers eq 'y' or $field_value.owner eq $user}
...
{elseif $field_value.type eq 'f'}
	{html_select_date prefix=$field_value.ins_id time=$field_value.value start_year=$prefs.calendar_start_year end_year=$prefs.calendar_end_year field_order=$prefs.display_field_order}
...
{elseif $field_value.type eq 'c'}
	<input type="checkbox" name="{$field_value.ins_id}" {if $field_value.value eq 'y' or $field_value.value eq 'on' or $field_value.defaultvalue eq 'y'}checked="checked"}checked="checked"{/if}/>
