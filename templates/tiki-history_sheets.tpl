		...
		<tr>
			{section name=date loop=$grid_content}
				<td style="width: 50%;">
					...
					{button href="tiki-view_sheets.php?sheetId=$sheetId&readdate=`$history[$smarty.section.date.index].stamp`" _id="edit_button" _text="{tr}Edit{/tr}" _htmlelement="role_main" sheetId="$sheetId" _class="" parse="edit" editSheet="y" _auto_args="*" _title="{tr}Edit{/tr}"}
				</td>
			{/section}
		</tr>
	</table>
	...
	{section name=revision_date loop=$history key=i}
		<tr>
	    	...
	    	<td style="vertical-align: middle; text-align: center;">
			   	<input type="radio" name="compareSheet1" class="compareSheet1" value="{$history[revision_date].index}" />
	    	</td>
	    	<td style="vertical-align: middle; text-align: center;">
			   	<input type="radio" name="compareSheet2" class="compareSheet2" value="{$history[revision_date].index}" />
	    	</td>
    		<td>
