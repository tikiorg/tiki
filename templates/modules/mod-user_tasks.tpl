...
<input style="font-size: 9px;" type="submit" class="btn btn-sm" name="modTasksSave" value="{tr}Add{/tr}" />
</form>
</div>
<form action="{$ownurl}" method="post">
...
{/section}
{if $nonums != 'y'}</ol>{else}</ul>{/if}
<input style="font-size: 9px;" type="submit" class="btn btn-sm" name="modTasksCom" value="{tr}Done{/tr}" />
<input style="font-size: 9px;" type="submit" class="btn btn-sm" name="modTasksDel" value="{tr}Del{/tr}" />
</form>
{/tikimodule}
{/if}
