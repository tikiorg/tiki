...
{if !empty($cart_content)}
	{if $module_params.showItems neq 'n'}
		<form method="post" action="{if $module_params.ajax eq 'n'}{query _keepall='y' _type='relative'}{else}{service controller='module' action='execute'}{/if}"{if $module_params.ajax eq 'y'} class="mod-cart-item-form"{/if}>
		<table>
			<tr>
				<th>{tr}Product{/tr}</th>
	...
	{if not empty($cart_weight)}
		...
		<input type="hidden" name="cart_weight" value="{$module_params.weightUnit}">
	{/if}
