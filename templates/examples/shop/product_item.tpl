		...
		<td rowspan="14" style="padding-right: 2em; min-height: 600px;">
			...
			{if $img}
				<div class="item-img-outer" style="min-width:360px;min-height:360px;">
					<a class="item-img" href="tiki-download_file.php?fileId={$img[0]}&display" title="{$row.title}" rel="[p]box">
		...
		<td colspan="3">
			{if count($img) > 1}
				{for $i = 1; $i< count($img); $i++}
					<a class="item-thumb" href="tiki-download_file.php?fileId={$img[$i]}&display" title="{$row.name}" rel="[p]box">
