...
{if $field.isMultilingual ne 'y'}
	...
	{else}
		{if $field.options_array[7] == 'y'}
			...
			{/textarea}
		{else}
			{textarea id=$data.element_id name=$field.ins_id _toolbars=$data.toolbar _simple='y' cols=$data.cols rows=$data.rows onkeyup=$data.keyup _wysiwyg='n' section="trackers"}
				{$field.value}
			{/textarea}
		{/if}
	{/if}
