...
{if $context.list_mode eq 'n'}
	...
	{if $user}
		...
		<form method="post" action="{$smarty.server.PHP_SELF}?{query}">
			<input type="submit" class="btn btn-default" name="user_subscribe" value="{tr}Subscribe{/tr}"> {tr}with{/tr}
			{if $field.list}
			...
			{/if}
			{if $field.user_subscription}<br><input type="submit" class="btn btn-default" name="user_unsubscribe" value="{tr}Unsubscribe{/tr}">{/if}
		</form>
	{/if}
{/if}