	...
	<div class="table-responsive">
		<table class="table" id="selectable">
			<tr>
				...
				<th width="20"><a href='#'{popup delay="0|2000" fullhtml="1" text=$smarty.capture.other_sorts|escape:"javascript"|escape:"html"} title="{tr}Other Sorts{/tr}">{icon name='list' alt="{tr}Other Sorts{/tr}"}</a></th>
			</tr>
			...
			{foreach from=$klist key=key item=item}
				{if $item->id ne ''}
					<tr{if ($key % 2)} class="odd"{else} class="even"{/if}>
						...
						<td class="text"><a href="#" title="{tr}Thumbnail{/tr}" {popup delay="0|2000" fullhtml="1" text=$smarty.capture.actions|escape:"javascript"|escape:"html"}><img class="athumb" src="{$item->thumbnailUrl}" alt="{$item->description}" height="80" width="120"></a></td>
