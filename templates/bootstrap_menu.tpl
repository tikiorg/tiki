<ul class="nav">
	{foreach from=$list item=item}
		{if !empty($item.children)}
			<li class="{$item.class|escape}{if !empty($item.selected)} active{/if}">
				...
				<ul id="menu_option{$item.optionId|escape}" class="nav collapse">
					{foreach from=$item.children item=sub}
						<li class="{$sub.class|escape}{if !empty($sub.selected)} active{/if}">
		...
		{else}
			<li class="{$item.class|escape}{if !empty($item.selected)}active{/if}"><a href="{$item.sefurl|escape}">{tr}{$item.name|escape}{/tr}</a></li>
		{/if}
	{/foreach}
</ul>
