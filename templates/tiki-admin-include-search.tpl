...
<div class="cbox">
  ...
  <div class="cbox-data">
	...
	<tr>
          <td class="form">
        {if $prefs.feature_help eq 'y'}<a href="{$prefs.helpurl}Referer+Search+Highlighting" target="tikihelp" class="tikihelp" title="{tr}Referer Search Highlighting{/tr}">{/if}
                {tr}Referer Search Highlighting{/tr}:
                {if $prefs.feature_help eq 'y'}</a>{/if}
                </td>
        <td><input type="checkbox" name="feature_referer_highlight"
                {if $prefs.feature_referer_highlight eq 'y'}checked="checked"{/if}/></td>
        </tr>
<tr><th class="heading" colspan="2" align="center">{tr}Database Search using MySQL 'Full-Text' Feature{/tr}</th></tr>
        <tr>
	  <td class="form">
	{if $prefs.feature_help eq 'y'}<a href="{$prefs.helpurl}Search+Admin" target="tikihelp" class="tikihelp" title="{tr}Activate MySQL Full-Text feature{/tr}">{/if}
		{tr}Activate MySQL 'Full-Text' feature{/tr}:
		{if $prefs.feature_help eq 'y'}</a>{/if}
	        </td>
          <td><input type="checkbox" name="feature_search_fulltext"
                {if $prefs.feature_search_fulltext eq 'y'}checked="checked"{/if}/></td>
        </tr>
	<tr>
          <td class="heading" colspan="2">{tr}Performance issues{/tr}</td>
        </tr><tr>
	<td class="form">
	{if $prefs.feature_help eq 'y'}<a href="{$prefs.helpurl}Search+Admin" target="tikihelp" class="tikihelp" title="{tr}Search may show forbidden results. Much better performance though.{/tr}">{/if}
		{tr}Ignore individual object permissions{/tr}:
		{if $prefs.feature_help eq 'y'}</a>{/if}
	        </td>
          <td><input type="checkbox" name="feature_search_show_forbidden_obj"
                {if $prefs.feature_search_show_forbidden_obj eq 'y'}checked="checked"{/if}/></td>
        </tr><tr>
	<td class="form">
	{if $prefs.feature_help eq 'y'}<a href="{$prefs.helpurl}Search+Admin" target="tikihelp" class="tikihelp" title="{tr}Search may show forbidden results. Much better performance though.{/tr}">{/if}
...
<a href="tiki-admin.php?page=search&refresh_index_now=y" class="link" title="{tr}Refresh wiki search index now{/tr}">
	...
	<td class="form">
{if $prefs.feature_help eq 'y'}<a href="{$prefs.helpurl}Search+Stats" target="tikihelp" class="tikihelp" title="{tr}SearchStats{/tr}">{/if}
                        {tr}Search stats{/tr}:
                        {if $prefs.feature_help eq 'y'}</a>{/if}
                        </td>
        <td><input type="checkbox" name="feature_search_stats"
            {if $prefs.feature_search_stats eq 'y'}checked="checked"{/if}/>
	</td>
	</tr>
<tr>
	  ...
	  <td><input size="5" type="text" name="search_refresh_rate" id="search-refresh"
	      value="{$prefs.search_refresh_rate|escape}" /> {tr}0 for no refresh{/tr}</td>
        </tr><tr>
          ...
          <td><input size="5" type="text" name="search_min_wordlength" id="search-min_length"
              value="{$prefs.search_min_wordlength|escape}" /></td>
        </tr><tr>
	  ...
	  value="{$prefs.search_max_syllwords|escape}" /></td>
	</tr><tr>
          ...
          value="{$prefs.search_syll_age|escape}" /></td>
        </tr><tr>
          ...
          value="{$prefs.search_lru_purge_rate|escape}" /></td>
        </tr><tr>
          ...
          value="{$prefs.search_lru_length|escape}" /></td>
        </tr><tr>
