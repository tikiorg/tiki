...
{section name=ix loop=$listpages}
	...
	{if $listpages[ix].disp_article eq 'y'}
		...
		<article class="article{if !empty($container_class)} {$container_class}{/if} article{$smarty.section.ix.index}">
			...
			{/if}
			<div class="articleheading">
						<div {if $listpages[ix].isfloat eq 'n'}class="media-left pull-left"{/if}>
							{if $listpages[ix].show_image eq 'y'}
								{if $listpages[ix].useImage eq 'y'}
									{if $listpages[ix].hasImage eq 'y'}
										<a href="{$smarty.capture.href}" class="thumbnail{if $listpages[ix].isfloat eq 'y'} pull-left{/if}" {*if $listpages[ix].isfloat eq 'y'*} style="margin-right: 10px;"{*/if*}
											...
											{$style=''}
											<img {*{if $listpages[ix].isfloat eq 'y'}{$style="margin-right:4px;float:left;"}{else}*}class="arti cleimage"{*{/if}*}
                        ...
                        <div class="media-body">
							{/if}
							<div class="articleheadingtext media">{$listpages[ix].parsed_heading}</div>
