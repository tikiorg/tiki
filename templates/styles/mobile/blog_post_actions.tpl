<div class="actions">
	<a {if $prefs.mobile_mode eq "y"}data-role="button" data-inline="true" {/if}href='tiki-print_blog_post.php?postId={$post_info.postId}'>{icon _id='printer' alt="{tr}Print{/tr}"}</a> {* mobile *}
	{if $prefs.feature_blog_sharethis eq "y"}
		...
		{literal}<script language="javascript" type="text/javascript">
			...
			document.write('<span class="share" id="share{/literal}{$postId}{literal}"><a title="{/literal}{$smarty.capture.shared_link_title|replace:'\'':'\\\''}{literal}" href="javascript:void(0);"><img src="https://ws.sharethis.com/images/share-icon-16x16.png?CXNID=1000014.0NXC"></a></span>');
