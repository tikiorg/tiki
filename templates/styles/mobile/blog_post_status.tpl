<div class="status">
	<a {if $prefs.mobile_mode eq "y"}data-role="button" data-inline="true" {/if}class="link" href="{$post_info.postId|sefurl:blogpost}">{tr}Permalink{/tr}</a>
	{if $blog_post_context eq 'view_blog'}
		{if $allow_comments eq 'y' and $prefs.feature_blogposts_comments eq 'y' && $tiki_p_read_comments eq 'y'}
			 | 
 			<a {if $prefs.mobile_mode eq "y"}data-role="button" data-inline="true" {/if}class="link linkcomments" href="{$post_info.postId|sefurl:blogpost}#comments">
