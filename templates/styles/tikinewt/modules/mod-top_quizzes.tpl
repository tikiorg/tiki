{* based on /cvsroot/tikiwiki/tiki/templates/modules/mod-top_quizzes.tpl,v 1.7.10.1 2005/02/23 15:56:12 michael_davey *}
...
{if $nonums != 'y'}<ol>{else}<ul>{/if}
	...
	<li>
	<a class="linkmodule" href="tiki-take_quiz.php?quizId={$modTopQuizzes[ix].quizId}">{$modTopQuizzes[ix].quizName}</a>
	</li>
    {/section}
	{if $nonums != 'y'}</ol>{else}</ul>{/if}
    {/tikimodule}
