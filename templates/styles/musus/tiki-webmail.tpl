...
<tr><td>{tr}Password{/tr}</td><td colspan="3"><input type="password" name="pass" value="{$info.pass|escape}" /></td></tr>
<tr><td>{tr}Messages per page{/tr}</td><td colspan="3"><input type="text" name="msgs" size="7" value="{$info.msgs|escape}" /></td></tr>
...
[<a href="tiki-webmail.php?section=settings&amp;remove={$accounts[ix].accountId}" 
...
<tr>
<td>
<a href="tiki-webmail.php?section=mailbox">{tr}View All{/tr}</a> | <a href="tiki-webmail.php?section=mailbox&amp;filter=unread">{tr}Unread{/tr}</a> | <a href="tiki-webmail.php?section=mailbox&amp;filter=flagged">{tr}Flagged{/tr}</a>
</td>
<td align="right">
{tr}Msg{/tr} {$showstart}-{$showend} {tr}of{/tr} {$total} 
{if $first}| <a href="tiki-webmail.php?section=mailbox&amp;start={$first}{if $filter}&amp;filter={$filter}{/if}">{tr}First{/tr}</a>{/if} 
{if $prevstart}| <a href="tiki-webmail.php?section=mailbox&amp;start={$prevstart}{if $filter}&amp;filter={$filter}{/if}">{tr}Prev{/tr}</a>{/if} 
{if $nextstart}| <a href="tiki-webmail.php?section=mailbox&start={$nextstart}{if $filter}&amp;filter={$filter}{/if}">{tr}Next{/tr}</a>{/if} 
{if $last}| <a href="tiki-webmail.php?section=mailbox&amp;start={$last}{if $filter}&amp;filter={$filter}{/if}">{tr}Last{/tr}</a>{/if}
</td>
</tr>
</table><br />
<form action="tiki-webmail.php" method="post">
...
<td style="background:{$class};"><a href="tiki-webmail.php?section=read&amp;msgid={$list[ix].msgid}">{$list[ix].subject}</a>{if $list[ix].has_attachment}<img src="img/webmail/clip.gif" alt='{tr}clip{/tr}'/>{/if}</td>
...
{if $section eq 'read'}
{if $prev}<a href="tiki-webmail.php?section=read&amp;msgid={$prev}">{tr}Prev{/tr}</a> |{/if}
{if $next}<a href="tiki-webmail.php?section=read&amp;msgid={$next}">{tr}Next{/tr}</a> |{/if}
 <a href="tiki-webmail.php?section=mailbox">{tr}back to mailbox{/tr}</a> |
{if $fullheaders eq 'n'} 
 <a href="tiki-webmail.php?section=read&amp;msgid={$msgid}&amp;fullheaders=1">{tr}full headers{/tr}</a>
{else}
 <a href="tiki-webmail.php?section=read&amp;msgid={$msgid}">{tr}normal headers{/tr}</a>
{/if} 
<table>
<tr>
  <td>
...
<div class="simplebox">
<a href="tiki-webmail_download_attachment.php?section=read&amp;msgid={$msgid}&amp;getpart={$attachs[ix].part}">{$attachs[ix].name|iconify}{$attachs[ix].name}</a>
</div>
{/section}
{/if}
...
<table>
<tr><td>{tr}First Name{/tr}:</td><td><input type="text" maxlength="80" size="20" name="firstName" value="{$info.firstName|escape}" /></td></tr>
<tr><td>{tr}Last Name{/tr}:</td><td><input type="text" maxlength="80" size="20" name="lastName" value="{$info.lastName|escape}" /></td></tr>
<tr><td>{tr}Email{/tr}:</td><td><input type="text" maxlength="80" size="20" name="email" value="{$info.email|escape}" /></td></tr>
<tr><td>{tr}Nickname{/tr}:</td><td><input type="text" maxlength="80" size="20" name="nickname" value="{$info.nickname|escape}" /></td></tr>
...
<a href="tiki-webmail.php?section=contacts&amp;offset={$offset}&amp;sort_mode={$sort_mode}&amp;find={$find}">{tr}all{/tr}</a>
{section name=ix loop=$letters}
<a href="tiki-webmail.php?section=contacts&amp;offset={$offset}&amp;sort_mode={$sort_mode}&amp;find={$find}&amp;letter={$letters[ix]}">{$letters[ix]}</a>
{/section}
<table>
<tr>
<td class="heading"><a href="tiki-webmail.php?section=contacts&amp;offset={$offset}&amp;sort_mode={if $sort_mode eq 'firstName_desc'}firstName_asc{else}firstName_desc{/if}">{tr}First Name{/tr}</a></td>
<td class="heading"><a href="tiki-webmail.php?section=contacts&amp;offset={$offset}&amp;sort_mode={if $sort_mode eq 'lastName_desc'}lastName_asc{else}lastName_desc{/if}">{tr}Last Name{/tr}</a></td>
<td class="heading"><a href="tiki-webmail.php?section=contacts&amp;offset={$offset}&amp;sort_mode={if $sort_mode eq 'email_desc'}email_asc{else}email_desc{/if}">{tr}Email{/tr}</a></td>
<td class="heading"><a href="tiki-webmail.php?section=contacts&amp;offset={$offset}&amp;sort_mode={if $sort_mode eq 'nickname_desc'}nickname_asc{else}nickname_desc{/if}">{tr}Nickname{/tr}</a></td>
</tr>
{cycle values="odd,even" print=false}
...
<td class="{cycle advance=false}"><a href="tiki-webmail.php?section=contacts&amp;offset={$offset}&amp;sort_mode={$sort_mode}&amp;find={$find}&amp;contactId={$channels[user].contactId}">{$channels[user].email}</a>
[<a href="tiki-webmail.php?section=contacts&amp;offset={$offset}&amp;sort_mode={$sort_mode}&amp;find={$find}&amp;remove={$channels[user].contactId}" 
onclick="return confirmTheLink(this,'{tr}Are you sure you want to delete this contact?{/tr}')" 
...
{if $attaching eq 'n'}
  {if $sent eq 'n'}
    ...
    <table >
    <tr><td><a title="{tr}select from address book{/tr}" href="#" onClick="javascript:window.open('tiki-webmail_contacts.php?element=to','','menubar=no,width=452,height=550');">{tr}To{/tr}</a>:</td><td colspan="3"><input size="69" type="text" id="to" name="to" value="{$to|escape}" /></td></tr>
    <tr><td>{tr}cc{/tr}</td><td><input id="cc" type="text" name="cc" value="{$cc|escape}" /></td><td>{tr}bcc{/tr}</td><td><input type="text" name="bcc" value="{$bcc}" id="bcc" /></td></tr>
    <tr><td>{tr}Subject{/tr}</td><td colspan="3"><input size="69" type="text" name="subject" value="{$subject|escape}" /></td></tr>
    <tr><td>{tr}Attachments{/tr}</td><td colspan="3">
    {if $attach1}
    ({$attach1})
    {/if}
    {if $attach2}
    ...
    <tr><td>{tr}Use HTML mail{/tr}</td><td colspan="3"><input type="checkbox" name="useHTML" /></td></tr>
    </td></tr>
    </table>
    </form>
  {else}
...
{else}
  <form enctype="multipart/form-data" action="tiki-webmail.php" method="post">
    ...
    <table>
    {if $attach1}
    <tr><td>{tr}Attachment 1{/tr}</td><td>{$attach1} <input type="submit" name="remove_attach1" value="{tr}remove{/tr}" /></td></tr>
    {else}
    <tr><td>{tr}Attachment 1{/tr}</td><td><input type="hidden" name="MAX_FILE_SIZE" value="1500000" /><input name="userfile1" type="file" /></td></tr>
    {/if}
    {if $attach2}
    <tr><td>{tr}Attachment 2{/tr}</td><td>{$attach2} <input type="submit" name="remove_attach2" value="{tr}remove{/tr}" /></td></tr>
    {else}
    <tr><td>{tr}Attachment 2{/tr}</td><td><input type="hidden" name="MAX_FILE_SIZE" value="1500000" /><input name="userfile2" type="file" /></td></tr>
    {/if}
    {if $attach3}
    <tr><td>{tr}Attachment 3{/tr}</td><td>{$attach3} <input type="submit" name="remove_attach3" value="{tr}remove{/tr}" /></td></tr>
    {else}
    <tr><td>{tr}Attachment 3{/tr}</td><td><input type="hidden" name="MAX_FILE_SIZE" value="1500000" /><input name="userfile3" type="file" /></td></tr>
    {/if}
    <tr><td>&nbsp;</td><td><input type="submit" name="attached" value="{tr}done{/tr}" /></td></tr>
    </table>
    </form>
