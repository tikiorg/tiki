{* $Header: /cvsroot/tikiwiki/tiki/templates/styles/musus/tiki-assignuser.tpl,v 1.2 2004-01-13 19:26:59 musus Exp $ *}
...
<table>
<tr><td>{tr}Login{/tr}:</td><td>{$user_info.login}</td></tr>
<tr><td>{tr}Email{/tr}:</td><td>{$user_info.email}</td></tr>
<tr><td>{tr}Groups{/tr}:</td><td>
{foreach from=$user_info.groups item=grp}
{$grp}
{if $grp != "Anonymous"}
(<a href="tiki-assignuser.php?offset={$offset}&amp;sort_mode={$sort_mode}&amp;assign_user={$assign_user}&amp;action=removegroup&amp;group={$grp}">x</a>)
{/if}&nbsp;&nbsp;
{/foreach}
</td></tr>
<form method="post" action="tiki-assignuser.php?assign_user={$assign_user}">
<tr><td>{tr}Default Group{/tr}:</td><td>
...
<table>
<tr>
<td class="heading"><a href="tiki-assignuser.php?assign_user={$assign_user}&amp;offset={$offset}&amp;sort_mode={if $sort_mode eq 'groupName_desc'}groupName_asc{else}groupName_desc{/if}">{tr}name{/tr}</a></td>
<td class="heading"><a href="tiki-assignuser.php?assign_user={$assign_user}&amp;offset={$offset}&amp;sort_mode={if $sort_mode eq 'groupDesc_desc'}groupDesc_asc{else}groupDesc_desc{/if}">{tr}desc{/tr}</a></td>
...
<a href="tiki-assignuser.php?offset={$offset}&amp;sort_mode={$sort_mode}&amp;action=assign&amp;group={$users[user].groupName}&amp;assign_user={$assign_user}">{tr}assign{/tr}</a></td>
{/if}
</tr>
{/section}
</table>
...
<a class="prevnext" href="tiki-assignuser.php?find={$find}&amp;assign_user={$assign_user}&amp;offset={$selector_offset}&amp;sort_mode={$sort_mode}">{$smarty.section.foo.index_next}</a>&nbsp;
{/section}
{/if}
</div>