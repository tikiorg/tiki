...
{if $prefs.feature_layoutshadows eq 'y'}<div class="box-shadow">{$prefs.box_shadow_start}{/if}
	...
	{elseif $module_notitle ne 'y'}
		...
		<!--[if IE]><br class="clear" style="height: 1px !important" /><![endif]--></h3>
	{/if}
		<div id="mod-{$smarty.capture.name}" style="display: {if !isset($module_display) or $module_display}block{else}none{/if};{$module_params.style}" class="clearfix box-data{if !empty($module_params.class)} {$module_params.class}{/if} clearfix">
