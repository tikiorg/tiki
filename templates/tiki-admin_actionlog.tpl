	...
	{else}
		<table class="smallnormal">
			...
			<tr class="formcolor">
				...
				<td>
					{html_select_date time=$endDate prefix="endDate_" end_year="-10" field_order=$prefs.display_field_order}
				</td>
			</tr>
				...
				<td>
					<select name="categId">
						...
						{section name=ix loop=$categories}
							<option value="{$categories[ix].categId|escape}" {if $reportCateg eq $categories[ix].name}selected="selected"{/if}>
								{$categories[ix].name|escape}
							</option>
						{/section}
					</select>
				</td>
			...
			<tr class="formcolor">
				...
				<td>
					{tr}bytes{/tr}
					<input type="radio" name="unit" value="bytes"{if $unit ne 'kb'} checked="checked"{/if} />
					{tr}kb{/tr}
			...
			<tr class="formcolor">
				<td colspan="2" class="input_submit_container">
					<input type="submit" name="list" value="{tr}Report{/tr}" />
				</td>
			</tr>
					...
					<td colspan="2">
						...
						{if $prefs.feature_jpgraph eq 'y'}
							...
							<select name="legendBgcolor">
								{foreach item=color from=$bgcolors}
									<option value="{$color|escape}"{if $defaultLegendBgcolor eq $color} selected="selected"{/if}>
										{tr}{$color}{/tr}
									</option>
								{/foreach}
							</select>
...
<h2>{tr}List{/tr}
	{if $selectedUsers}
		&nbsp;&mdash;&nbsp;
		{tr}User:{/tr}
		{foreach key=ix item=auser from=$users}{if $selectedUsers[$ix] eq 'y'} {$auser|escape}{/if}{/foreach}
	{/if}
	{if $selectedGroups}
		&nbsp;&mdash;&nbsp;
		{tr}Group:{/tr}
		...
		{section name=ix loop=$actionlogs}
			<tr>
				...
				{if $prefs.feature_contribution eq 'y'}
					...
					{/if}
					{if $prefs.feature_contribution eq 'y' and $tiki_p_admin eq 'y'}
						<td class="{cycle advance=false}">
							{if $actionlogs[ix].actionId}
								<a class="link" href="tiki-admin_actionlog.php?actionId={$actionlogs[ix].actionId}&amp;startDate={$startDate}&amp;endDate={$endDate}#action" title="{tr}Edit Contribution{/tr}">{icon _id='page_edit'}</a>
							{else}
								&nbsp;
							{/if}
						</td>
...
{if $action}
	...
	<form method="post" action="tiki-admin_actionlog.php">
		...
		{/if}
		{if $selectedGroups}
			<input type="hidden" name="selectedGroups" value="{$selectedGroups}" />
		{/if}
		{if $startDate}
	...
	<table class="smallnormal">
		...
		<tr>
			...
			{foreach item=type from=$typeVol}
				<th>
					{$type} (+{if $unit eq 'kb'}{tr}kb{/tr}{else}{tr}bytes{/tr}{/if})
				</th>
				<th>
					{$type} (-{if $unit eq 'kb'}{tr}kb{/tr}{else}{tr}bytes{/tr}{/if})
				</th>
		...
		{foreach key=categId item=vol from=$volCateg}
			<tr>
				...
				{foreach item=type from=$typeVol} {* math equation="round(a/b)" a=$vol[$type].del b=1024 *}
					<td class="{cycle advance=false}{if $vol[$type].add} diffadded{/if}">
						{if $vol[$type].add}{$vol[$type].add}{else}0{/if}
					</td>
					<td class="{cycle advance=false}{if $vol[$type].del} diffdeleted{/if}">
						{if $vol[$type].del}{$vol[$type].del}{else}0{/if}
					</td>
					<td class="{cycle advance=false}{if $vol[$type].dif > 0} diffadded{elseif $vol[$type].dif < 0} diffdeleted{/if}">
						{if $vol[$type].dif}{$vol[$type].dif}{else}0{/if}
					</td>
				{/foreach}
				<!-- {cycle} -->
			</tr>
...
{if $objectActions|@count ne 0}
	<table class="normal">
		...
		{foreach item=stat from=$objectActions name=objectActions}
			<tr>
				<td class="{cycle advance=false}">{if $stat.link}<a href="{$stat.link}" target="_blank" title="{tr}View{/tr}">
					{$stat.object|escape}</a>{else}{$stat.object|escape}{/if}
				</td>
				{foreach key=a item=nb from=$stat}
...
{if $prefs.feature_contribution eq 'y' && isset($contributionStat)}
	<table>
		<caption>
			{if $selectedUsers}
				{tr}Volumn per users'contribution and time{/tr}
			{else}
				{tr}Volumn per contribution and time{/tr}
			{/if}
		</caption>
		<tr>
			<th>{tr}Contribution{/tr}</th>
