	...
	<div class="wikiactions icons btn-group pull-right">
			...
			{if $prefs.flaggedrev_approval neq 'y' or ! $revision_approval or $lastVersion eq $revision_displayed}
				{if $editable and ($tiki_p_edit eq 'y' or $page|lower eq 'sandbox') and $beingEdited ne 'y' and $machine_translate_to_lang eq ''}
					<a class="btn btn-default" title="{tr}Edit this page{/tr}" {ajax_href template="tiki-editpage.tpl"}tiki-editpage.php?page={$page|escape:"url"}{if !empty($page_ref_id) and $needsStaging neq 'y'}&amp;page_ref_id={$page_ref_id}{/if}{/ajax_href}>{glyph name='edit' class='text-primary' title="{tr}Edit this page{/tr}"}</a>
			...
			{/if}
			{if $cached_page eq 'y'}
				<a class="btn btn-default" title="{tr}Refresh{/tr}" href="tiki-index.php?page={$page|escape:"url"}&amp;refresh=1">{glyph name='refresh' class='text-primary' title="{tr}Refresh{/tr}"}</a>
			{/if}
			{if $prefs.feature_wiki_print eq 'y'}
				<a class="btn btn-default" title="{tr}Print{/tr}" href="tiki-print.php?{query _keepall='y'}">{glyph name='print' class='text-primary' title="{tr}Print{/tr}"}</a>
			{/if}
	
			{if $prefs.feature_share eq 'y' && $tiki_p_share eq 'y'}
				<a class="btn btn-default" title="{tr}Share this page{/tr}" href="tiki-share.php?url={$smarty.server.REQUEST_URI|escape:'url'}" class='text-primary'>{icon _id='share_link' alt="{tr}Share this page{/tr}"}</a>
			{/if}
			...
			{if !empty($user) and $prefs.feature_user_watches eq 'y'}
				{if $user_watching_page eq 'n'}
					<a class="btn btn-default" href="tiki-index.php?page={$page|escape:"url"}&amp;watch_event=wiki_page_changed&amp;watch_object={$page|escape:"url"}&amp;watch_action=add{if $structure eq 'y'}&amp;structure={$home_info.pageName|escape:'url'}{/if}" class="icon">{glyph name='eye-open' class='text-primary' title="{tr}Page is NOT being monitored. Click icon to START monitoring.{/tr}"}</a>
				{else}
					<a class="btn btn-default" href="tiki-index.php?page={$page|escape:"url"}&amp;watch_event=wiki_page_changed&amp;watch_object={$page|escape:"url"}&amp;watch_action=remove{if $structure eq 'y'}&amp;structure={$home_info.pageName|escape:'url'}{/if}" class="icon">{glyph _id='eye-close' class='text-primary' alt="{tr}Page IS being monitored. Click icon to STOP monitoring.{/tr}"}</a>
				{/if}
			...
			{if $prefs.feature_group_watches eq 'y' and ( $tiki_p_admin_users eq 'y' or $tiki_p_admin eq 'y' )}
				<a class="btn btn-default" href="tiki-object_watches.php?objectId={$page|escape:"url"}&amp;watch_event=wiki_page_changed&amp;objectType=wiki+page&amp;objectName={$page|escape:"url"}&amp;objectHref={'tiki-index.php?page='|cat:$page|escape:"url"}" class="icon">{glyph name='eye' class='text-primary' title="{tr}Group Monitor{/tr}"}</a>
