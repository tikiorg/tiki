...
{foreach from=$icons key=page item=info}
	{if ! $info.disabled and $info.icon}
		{self_link _icon_class="reflect" _width="32" _height="32" _alt=$info.title page=$page _class="icon tips bottom btn btn-default" _title="`$info.title`|`$info.description`"}
			<span class="{$info.icon_class}"></span>
		{/self_link}
