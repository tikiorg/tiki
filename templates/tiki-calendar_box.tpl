...
{if $calendar_view_tab eq "y"}<a href="{$cellurl|replace:"editmode=1":"editmode=details"}{if $feature_tabs ne 'y'}#details{/if}" title="{tr}details{/tr}"><img src="img/icons/zoom.gif" border=0 width="16" height="16" alt="{tr}zoom{/tr}" /></a>&nbsp;{/if}{if $tiki_p_change_events eq "y"}<a href="{$cellurl}{if $feature_tabs ne 'y'}#add{/if}" title="{tr}edit{/tr}"><img src="img/icons/edit.gif" border="0"  width="20" height="16" alt="{tr}edit{/tr}" /></a><a href="tiki-calendar.php?calitemId={$cellid}&amp;delete=1" title="{tr}remove{/tr}"><img src="img/icons2/delete.gif" border="0" width="16" height="16" alt="{tr}remove{/tr}" /></a>{/if}
