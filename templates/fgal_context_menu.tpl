...
{else}
	{if $prefs.javascript_enabled eq 'y'}
		{if $menu_text eq 'y' or $menu_icon eq 'y'}
			...
			<form class="upform" name="form{$files[changes].fileId}" method="post" action="{$smarty.server.PHP_SELF}?galleryId={$gal_info.galleryId}{if $filegals_manager eq 'y'}&amp;filegals_manager=y{/if}{if $prefs.fgal_asynchronous_indexing eq 'y'}&amp;fast{/if}" enctype="multipart/form-data">
	...
	{if $files[changes].perms.tiki_p_admin_file_galleries eq 'y'
		...
		{if $files[changes].archiveId == 0}
			...
			{else}
				...
				<a href="tiki-upload_file.php?galleryId={$gal_info.galleryId}&amp;fileId={$files[changes].id}{if $filegals_manager eq 'y'}&amp;filegals_manager=y{/if}">{icon _menu_text=$menu_text _menu_icon=$menu_icon _id='database_refresh' alt=$replace_action_title}</a>
