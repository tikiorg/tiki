{* $Header: /cvsroot/tikiwiki/tiki/templates/tiki-quiz_edit.tpl,v 1.11 2004-05-24 20:53:06 ggeller Exp $ *}
...
<form enctype="multipart/form-data" method="post" action="tiki-quiz_edit.php">
 				...
 				<div id="status" style="display:none;">
					<table class="normal">
						<tr>
							<td class="formcolor">
								{if $quiz->online eq 'y'}
									{html_radios name="online" options=$online_choices selected=choice_online}
								{else}
									{html_radios name="online" options=$online_choices selected=choice_offline}
								{/if}
							</td>
						</tr>
						{if $quiz->taken eq 'y'}
        ...
        [ <a class="link" href="javascript:show('general');">{tr}show{/tr}</a>
 				...
 				<div id="general" style="display:none;">
					<table class="normal">
      				...
      				<td>
								{html_select_date prefix="publish_" time=$quiz->datePub start_year="-5" end_year="+10"} {tr}at {/tr}{html_select_time prefix="publish_" time=$quiz->datePub display_seconds=false} HRS&nbsp;{$siteTimeZone} 
							</td>
						</tr>
						<tr class="formcolor">
				...
				[ <a class="link" href="javascript:show('test-time');">{tr}show{/tr}</a>
 				...
 				<div id="test-time" style="display:none;">
					<table class="normal">
						...
						<tr>
  						<td class="formcolor"><input type="checkbox" name="timeLimited" id="timelimit" {if $quiz->timeLimited eq 'y'}checked="checked"{/if} /><label for="timelimit">{tr}Impose a time limit of {/tr}</label><select name="timeLimit" id="quiz-maxtime">{html_options values=$mins selected=$quiz->timeLimit output=$mins}</select> {tr}minutes{/tr}</td>
						</tr>
						<tr>
						...
						<tr>
							<td class="formcolor"><input type="checkbox" name="canRepeat" id="repeat" {if $quiz->canRepeat eq 'y'}checked="checked"{/if} /><label for="repeat">{tr}Allow students to retake this quiz {/tr}</label>
							<select name="repetitions" id="quiz-repeat">{html_options values=$repetitions selected=$quiz->repetitions output=$repetitions}</select> {tr}times{/tr}</td>
						</tr>
					</table>
			  </div>
			</td>
			...
			<td {if $cols} colspan="{$cols}"{/if}>
				[ <a class="link" href="javascript:show('feedback');">{tr}show{/tr}</a>
 				...
 				<div id="feedback" style="display:none;">
					<table class="normal">
						<tr>
							<td colspan=2 class="formcolor"><label for="grading-method">{tr}Grading method {/tr}</label><select name="grading-method" id="grading-method">{html_options values=$optionsGrading selected=$quiz->gradingMethod output=$optionsGrading}</select>
              </td>
						</tr>
						<tr>
			...
			<td {if $cols} colspan="{$cols}"{/if}>
				[ <a class="link" href="javascript:show('after-test');">{tr}show{/tr}</a>
 				...
 				<div id="after-test" style="display:none;">
					<table class="normal">
						<tr>
							<td class="formcolor"><input type="checkbox" name="additionalQuestions" id="additionalQuestions" {if $quiz->additionalQuestions eq 'y'}checked="checked"{/if} /><label for="additional-questions">{tr}Solicit additional questions from students{/tr}</td>
						</tr>
					</table>
					<table class="normal">
						<tr>
							<td colspan="2" class="formcolor"><input type="checkbox" name="forum" id="forum" {if $quiz->forum eq 'y'}checked="checked"{/if} /><label for="forum">{tr}Link quiz to forum named: {/tr}</label><input type="text" name="forumName" id="forumName" value="{$quiz->nameForum|escape}" size="40"></td>
						</tr>
				  </table>
			  </div>
			</td>
    ...
    <tr>
      ...
      <td class="formcolor">
        <textarea class="wikiedit" name="input_data" rows="10" {* rows="20" *} cols="80" id='subheading' wrap="virtual" >{$quiz->data}</textarea>
      </td>
    </tr>
