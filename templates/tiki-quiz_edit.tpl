				...
				<div id="general" style="display:none;">
					<table class="formcolor">
						...
						<tr>
							...
							<td>
								<input type="checkbox" name=quiz.nQuestion id="nQuestion" {if $quiz->nQuestion eq 'y'}checked="checked"{/if}>
								<label for="nQuestions">{tr}Use {/tr}</label>
								<select name=quiz.nQuestions id="nQuestions">
						...
						<table class="table normal">
							...
							<tr>
								<td>
									...
									<select name=quiz.questionsPerPage id="quiz-perpage">{html_options values=$tpl.qpp selected=$quiz->questionsPerPage output=$tpl.qpp}</select> {tr}question(s).{/tr}
								</td>
							</tr>
							<tr>
								<td>
									...
									<select name=quiz.timeLimit id="quiz-maxtime">{html_options values=$tpl.mins selected=$quiz->timeLimit output=$tpl.mins}</select> {tr}minutes{/tr}
								</td>
							</tr>
							<tr>
								<td>
							...
							<tr>
								<td>
									...
									<select name=quiz.repetitions id="quiz-repeat">{html_options values=$tpl.repetitions selected=$quiz->repetitions output=$tpl.repetitions}</select> {tr}times{/tr}
								</td>
							</tr>
						</table>
					</div>
						...
						<table class="table normal">
							<tr>
								<td colspan=2>
									...
									<select name=quiz.gradingMethod" id="grading-method">{html_options values=$tpl.optionsGrading selected=$quiz->gradingMethod output=$tpl.optionsGrading}</select>
								</td>
							</tr>
							<tr>
								<td colspan=2>
									<label>{tr}Show students their score {/tr}</label>
									<select name=quiz.showScore id="showScore">{html_options values=$tpl.optionsShowScore selected=$quiz->showScore output=$tpl.optionsShowScore}</select>
								</td>
							</tr>
							<tr>
								<td>
									<label>{tr}Show students the correct answers {/tr}</label>
									<select name=quiz.showCorrectAnswers>{html_options values=$tpl.optionsShowScore selected=$quiz->showCorrectAnswers output=$tpl.optionsShowScore}</select>
								</td>
							</tr>
							<tr>
								<td>
									<label>{tr}Publish statistics {/tr}</label>
									<select name=quiz.publishStats>{html_options values=$tpl.optionsShowScore selected=$quiz->publishStats output=$tpl.optionsShowScore}</select>
							</tr>
						</table>
					</div>
				</div>
						...
						<table class="table normal">
							<tr>
								<td>
									...
									<label>{tr}Link quiz to forum named: {/tr}</label>
									<input type="text" name="quiz.forumName" value="{$quiz->nameForum|escape}" size="40">
								</td>
							</tr>
						</table>
					</div>
		...
		<table class="table normal">
			...
			<tr>
				...
				<td>
					...
					<a class="link" href="tiki-index.php?page={$page|escape:"url"}">{tr}Cancel Edit{/tr}</a>
				</td>
			</tr>
		</table>
	</div>
