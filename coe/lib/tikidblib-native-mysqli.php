...
class TikiResult{
	...
	function fetchRow($mode) {
		...
		//echo "TITI=".print_r($value,true)."<br/>";
		return $value;
	}
...
class TikiDB {
    ...
    foreach ($values as $key => $value) {
		...
		//echo "Resultat=".$result->num_rows."<br/>";
//		print_r($result);
		$this->stopTimer($starttime);

		if (!$result) {
			$this->sql_error_msg = $pq->error;
			$tmp = new TikiResult(NULL,0);
			return $tmp;
		} else {
			$this->sql_error_msg = "";
			$tmp = new TikiResult($result);
		//	echo "TOTO<br/>";
			return $tmp;
		}
	}
	...
	function getOne($query, $values = null, $reporterrors = true, $offset = 0) {
		...
		//echo "GETONE<br/>";
		$result = $this->_query($query, $values, 1, $offset);
		$res = $result->fetchRow();
		//echo "RES".print_r($res,true)."<br/>";
		if ($result) {
			$tmp = each($res);
			return $tmp['value'];
		} else {
