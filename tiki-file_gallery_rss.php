<?php
// $Header: /cvsroot/tikiwiki/tiki/tiki-file_gallery_rss.php,v 1.13 2003-10-11 09:43:20 ohertel Exp $
...
$title = "Tiki RSS feed for the file gallery: ".$info["name"];;
$now = date("U");
...
$read = httpPrefix().str_replace($pagename, "tiki-download_file.php", $urlarray["path"]);
...
if ($rss_version == 1) {
	...
	foreach ($changes["data"] as $chg) {
		print ('        <rdf:li resource="'.htmlspecialchars($read."?fileId=".$chg["fileId"]).'" />'."\n");
	}
	print "</rdf:Seq>\n";
	print "</items>\n";
...
foreach ($changes["data"] as $chg) {
	if ($rss_version == 2) {
		...
		print ('<item rdf:about="'.htmlspecialchars($read."?fileId=".$chg["fileId"]).'">'."\n");
	}
	print ('  <title>'.rawurlencode($chg["filename"].': '.$tikilib -> date_format($tikilib -> get_short_datetime_format(), $chg["created"])).'</title>'."\n");
	print ('  <link>'.htmlspecialchars($read."?fileId=".$chg["fileId"]).'</link>'."\n");
  ...
  $date = $tikilib -> date_format($tikilib -> get_short_datetime_format(), $chg["created"]);
	if ($rss_version == 2) {
		...
		print ('<guid isPermaLink="true">'.htmlspecialchars($read."?fileId=".$chg["fileId"]).'</guid>'."\n");
