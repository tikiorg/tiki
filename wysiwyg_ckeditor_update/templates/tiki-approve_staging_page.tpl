...
<table>
	...
	<tr class="{cycle}">
		...
		<td class="button">{$staging_info.version}<br />{tr}Current{/tr}</td>
		<td class="button">&nbsp;<a class="link" href="tiki-pagehistory.php?page={$staging_page|escape:"url"}&amp;preview={$staging_info.version}" title="{tr}View{/tr}" target="_blank">v</a>
	...
	{section name=hist loop=$history}
		<tr class="{cycle}">
			...
			<td class="button">{$history[hist].version}</td>
			<td class="button">
